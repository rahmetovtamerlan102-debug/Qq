...
async def depsearch_search(query: str):
    ...
    if re.match(r'^\+?\d{10,15}$', query):
        ...
        logger.info(f"DepSearch normalized phone: {query}")
    
    # Домены для проверки (второй домен - основной)
    domains = [
        "depsearch.sbs",          # Основной домен
        "api.depsearch.sbs",      # API поддомен
        "depsearch.lol",          # Запасной
        "api.depsearch.lol",      # API поддомен
    ]
    ...
    # Пробуем каждый домен
    for domain in domains:
        # Пробуем разные форматы URL
        url_formats = [
            f"https://{domain}/quest={query}&token={DEPSEARCH_TOKEN}&lang=ru",
            f"https://{domain}/api/quest={query}&token={DEPSEARCH_TOKEN}&lang=ru",
            f"https://{domain}/search?q={query}&token={DEPSEARCH_TOKEN}",
        ]
        
        for url in url_formats:
            logger.info(f"DepSearch trying: {url}")
            ...
            try:
                async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                    ...
                    logger.info(f"DepSearch {domain}: HTTP {resp.status}, length: {len(text)}")
                    logger.info(f"DepSearch response preview: {text[:200]}")
                    
                    # Проверяем на Cloudflare
                    if "Just a moment" in text or "cf-browser-verification" in text:
                        logger.warning(f"DepSearch {domain}: Cloudflare detected")
                        continue
                    
                    if resp.status == 200:
                        try:
                            data = json.loads(text)
                            logger.info(f"DepSearch {domain}: Successfully parsed JSON")
                            
                            if isinstance(data, dict):
                                # Проверяем наличие результатов
                                if data.get('results') and len(data.get('results', [])) > 0:
                                    logger.info(f"DepSearch {domain}: Found {len(data['results'])} results!")
                                    return data
                                
                                if data.get('phone_info'):
                                    logger.info(f"DepSearch {domain}: Found phone_info")
                                    return data
                                
                                # Если есть любые данные - возвращаем
                                if data:
                                    logger.info(f"DepSearch {domain}: Returned data with keys: {list(data.keys())}")
                                    return data
                                
                        except json.JSONDecodeError as e:
                            logger.error(f"DepSearch {domain} JSON decode error: {e}")
                            # Пытаемся найти JSON в тексте
                            json_match = re.search(r'\{.*\}', text, re.DOTALL)
                            if json_match:
                                try:
                                    data = json.loads(json_match.group())
                                    logger.info(f"DepSearch {domain}: Extracted JSON from response")
                                    if data:
                                        return data
                                except:
                                    pass
                        ...
                        logger.warning(f"DepSearch {domain}: 403 Forbidden")
                        continue
                    elif resp.status == 404:
                        logger.warning(f"DepSearch {domain}: 404 Not Found")
                        continue
                    else:
                        logger.warning(f"DepSearch {domain}: HTTP {resp.status}")
                        continue
                        
            except asyncio.TimeoutError:
                logger.warning(f"DepSearch {domain}: Timeout")
                continue
            except aiohttp.ClientError as e:
                logger.warning(f"DepSearch {domain}: Client error - {e}")
                continue
            except Exception as e:
                logger.error(f"DepSearch {domain}: Unexpected error - {e}")
                continue
    
    # Если ничего не нашлось - пробуем без токена
    logger.info("DepSearch: Trying without token...")
    for domain in ["depsearch.sbs", "api.depsearch.sbs"]:
        url = f"https://{domain}/quest={query}&lang=ru"
        try:
                ...
                if resp.status == 200:
                    ...
                    if "Just a moment" not in text:
                        try:
                            data = json.loads(text)
                            if data:
                                logger.info(f"DepSearch {domain}: Success without token!")
                                return data
                        except:
                            pass
        except:
    ...
    logger.error("DepSearch: All domains failed")
    return {}
...
def parse_depsearch(data):
    if not data or not isinstance(data, dict):
        logger.warning("parse_depsearch: no data or not dict")
        return []
    ...
    if phone_info:
        logger.info(f"DepSearch phone_info: {phone_info}")
        parsed.append({
            "type": "phone_info",
            "data": phone_info
        })
    ...
    if not results:
        ...
        for key in ['data', 'response', 'items', 'result']:
            if key in data:
                if isinstance(data[key], list):
                    results = data[key]
                    logger.info(f"Found results in '{key}' field")
                    break
                elif isinstance(data[key], dict):
                    for subkey in ['results', 'data', 'items']:
                        if subkey in data[key] and isinstance(data[key][subkey], list):
                            results = data[key][subkey]
                            logger.info(f"Found results in '{key}.{subkey}'")
                            break
    
    if not results:
        logger.warning("No results found in DepSearch response")
        # Пытаемся найти любые данные
        for key, value in data.items():
            if isinstance(value, list) and value:
                logger.info(f"Found list in '{key}', using as results")
                results = value
                break
    ...
    for idx, item in enumerate(results):
        if isinstance(item, dict):
            ...
            if fields:
                ...
                logger.info(f"Added result {idx} with {len(fields)} fields")
    
    logger.info(f"DepSearch parsed {len(parsed)} blocks")
    return parsed
...
async def collect_general_data(query: str, search_type: str = "phone"):
    ...
    for block in dep_parsed:
        if block['type'] == 'phone_info':
            ...
            if record_key not in seen_records:
                ...
                for key in ['👤ФИО', '👤Полное имя', 'full_name', 'fio', 'ФИО', 'Полное имя', '👤Имя', 'Имя', 'NAME']:
                ...
                for key in ['🎂Дата рождения', 'birthdate', 'Дата рождения', 'BIRTHDATE']:
    ...
    for src, src_name in [(seon, 'SEON'), (snusbase, 'Snusbase')]:
        ...
        emails = deep_find_all(src, 'email') + deep_find_all(src, 'mail')
        for email in emails:
...
async def test_depsearch(message: Message):
    ...
    args = message.text.split()
    if len(args) < 2:
        await message.reply("Укажите номер: /testdep 79141345653")
        return
    ...
    try:
        ...
        if result and isinstance(result, dict):
            ...
            has_phone_info = bool(result.get('phone_info'))
            results_count = len(result.get('results', []))
            
            text = f"✅ DepSearch ответил!\n"
            text += f"📊 Результатов: {results_count}\n"
            text += f"📱 Phone info: {'есть' if has_phone_info else 'нет'}\n"
            text += f"🔑 Ключи ответа: {', '.join(result.keys())}\n\n"
            
            if has_results:
                text += f"📝 Первая запись:\n"
