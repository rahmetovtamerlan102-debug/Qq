...
# === КРИПТОТОКЕН (вставлен, но лучше хранить в .env) ===
CRYPTOPAY_TOKEN = os.getenv("CRYPTOPAY_TOKEN")
if not CRYPTOPAY_TOKEN:
    # Если не найден в .env, используем встроенный (НО ЭТО НЕБЕЗОПАСНО!)
    CRYPTOPAY_TOKEN = "626190:AAkRdsFEHPdfZc6yIACB52hh1mVKkOdT0qc"
    logger.warning("CRYPTOPAY_TOKEN взят из кода (небезопасно). Рекомендуется задать в .env и перевыпустить токен.")

CHANNEL_USERNAME = "@dataseekerinfo"
CHANNEL_LINK = "tg://resolve?domain=dataseekerinfo"
ADMIN_IDS = [8559629118]  # замените на свой ID
...
async def depsearch_search(query: str):
    ...
    try:
        async with session.get(url, timeout=aiohttp.ClientTimeout(total=API_TIMEOUTS["depsearch"])) as resp:
            ...
            if resp.status != 200:
                logger.warning(f"DepSearch статус {resp.status}")
                return {}
            try:
                return json.loads(text)
            except json.JSONDecodeError as e:
                logger.error(f"DepSearch JSON error: {e}")
                return {}
...
async def jitler_search_with_balancer(query: str, search_type: str = "number"):
    ...
    for attempt in range(2):
        ...
        if not token:
            logger.warning("Jitler: нет токенов")
            return {}
        ...
        try:
            async with session.post(url, json=payload, headers=headers,
                ...
                if resp.status == 200:
                    ...
                    logger.warning(f"Jitler статус {resp.status}")
                    return {}
        except asyncio.TimeoutError:
...
async def collect_phone_data(query: str):
    ...
    logger.info(f"DepSearch результат: {json.dumps(depsearch, ensure_ascii=False)[:500]}")
...
# === ГЕНЕРАТОР HTML-ОТЧЁТА (с основными данными, соцсетями и телефонными книгами) ===
def generate_html_report(data: dict, views: int) -> str:
    ...
    # Формируем блок структуры (левая колонка)
    structure_items = ""
...
async def create_crypto_pay_invoice(user_id: int, amount_usd: float, requests_count: int):
    if not CRYPTOPAY_TOKEN:
        return None, "❌ CRYPTOPAY_TOKEN не настроен. Пожалуйста, обратитесь к администратору."
    session = await get_http_session()
        ...
        async with session.post(url, json=payload, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as resp:
            if resp.status == 200:
                ...
                if data.get('ok'):
                    ...
                    return None, f"❌ Ошибка CryptoPay: {error_msg}"
            else:
...
async def process_crypto_pay_payment(invoice_id: str):
    ...
    async with pool.acquire() as conn:
        ...
        try:
            ...
            logger.error(f"Ошибка отправки сообщения об оплате: {e}")
...
Каждые 24 часа выдаётся по 5 бесплатных запросов."""
    ...
    # Закрепляем ТОЛЬКО сообщение с "Вечная ссылка"
...
async def pay_usdt_callback(callback: CallbackQuery):
    ...
    user_id = callback.from_user.id
    pay_url, error = await create_crypto_pay_invoice(user_id, amount_usd, requests_count)
    if not pay_url:
        await callback.message.edit_text(
...
async def admin_callback(callback: CallbackQuery, state: FSMContext = None):
    ...
    if action == "stats":
        ...
        await callback.message.edit_text(
            f"📊 Статистика\n👤 Пользователей: {users}\n📄 Отчётов: {reports}\n🎫 Промокодов: {promos}\n💰 Платежей: {payments}",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Назад", callback_data="admin_back")]])
        )
    elif action == "back":
        ...
        await callback.message.edit_text("Введите промокод:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Назад", callback_data="admin_back")]]))
        await state.set_state(PromoCreation.waiting_for_code)
    elif action == "broadcast":
        await callback.message.edit_text("Введите текст рассылки:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Назад", callback_data="admin_back")]]))
        await state.set_state(Broadcast.waiting_for_text)
    elif action == "give":
        await callback.message.edit_text("Введите ID пользователя:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Назад", callback_data="admin_back")]]))
        ...
        await callback.message.edit_text(text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Назад", callback_data="admin_back")]]))
    elif action == "delete_promo":
        promos = await get_all_promo_codes()
        if not promos:
            await callback.message.edit_text("Нет промокодов для удаления.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Назад", callback_data="admin_back")]]))
        else:
            keyboard = InlineKeyboardMarkup(inline_keyboard=[])
            for p in promos:
        ...
        await callback.message.edit_text(text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Назад", callback_data="admin_back")]]))
    await callback.answer()
...
async def process_promo_code(message: Message, state: FSMContext):
    ...
    if not re.match(r'^[A-Za-z0-9_\-]+$', code):
        await message.reply("❌ Некорректный промокод (только буквы, цифры, _ и -).")
        return
...
async def universal_handler(message: Message):
    ...
    if re.match(r'^\+?\d{10,15}$', cleaned):
        ...
        search_type = "email"
        await message.reply("❌ Поиск по email пока не реализован. Работаем только с номерами телефонов.")
        return
    else:
        await message.reply("❌ Неизвестный тип запроса. Поддерживается только номер телефона в формате +79999999999.")
        return
    ...
    try:
        ...
        # Проверка, есть ли хоть что-то
        has_data = any([
            data.get('operator'), data.get('region'), data.get('country'),
            data.get('fio'), data.get('birthdate'), data.get('age') is not None,
            data.get('address'), data.get('inn'), data.get('snils'), data.get('passport'),
            data.get('phone_books'), data.get('vk'), data.get('ok'), data.get('instagram'), data.get('tiktok'),
            data.get('emails'), data.get('telegrams'), data.get('extra')
        ])
...
# === ЗАПУСК ===
async def main():
    await asyncio.sleep(2)  # задержка для предотвращения конфликтов
    await init_db()
    app = web.Application()
