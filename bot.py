...
async def create_crypto_pay_invoice(user_id: int, amount_usdt: float, requests_count: int) -> str:
    session = await get_http_session()
...
async def buy_package_callback(callback: types.CallbackQuery):
    ...
    user_id = callback.from_user.id
    pay_url = await create_crypto_pay_invoice(user_id, amount_usdt, requests_count)
    if not pay_url:
        await callback.message.edit_text(
            "❌ Ошибка при создании счёта. Проверьте токен CryptoPay и вебхук.\n"
    ...
    await callback.message.edit_text(
        ...
        f"Сумма: {amount_usdt} USDT (TRC20)\n\n"
