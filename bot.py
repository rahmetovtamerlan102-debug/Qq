...
async def collect_phone_data(query: str):
    # Запускаем все запросы параллельно с короткими таймаутами
    ...
    async def fetch_jitler():
        try:
            return await asyncio.wait_for(jitler_search_with_balancer(query, "number"), timeout=3.0)
        except:
            return {}
    
    # Запускаем все параллельно с общим таймаутом 8 секунд
    try:
