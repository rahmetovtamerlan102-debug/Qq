...
async def collect_phone_data(query: str):
        ...
        for record in bigbase.get('records', []):
            ...
            for item in base_record:
                if isinstance(item, list) and len(item) >= 2:
                    ...
                    value = item[1]
                    if value:
                        if 'вконтакте' in key or 'vk' in key or 'vkontakte' in key:
                            if not result['vk']:
                                result['vk'] = value
                        elif 'одноклассники' in key or 'ok' in key or 'odnoklassniki' in key:
                            if not result['ok']:
                                result['ok'] = value
                            ...
                            if not result['tiktok']:
                                result['tiktok'] = value
        for obj in bigbase.get('connections', {}).get('person', []):
            if not result['vk']:
                vk = deep_find(obj, 'vk') or deep_find(obj, 'vkontakte')
                if vk:
                    result['vk'] = vk
            if not result['ok']:
                ok = deep_find(obj, 'ok') or deep_find(obj, 'odnoklassniki')
                if ok:
                    result['ok'] = ok
            if not result['instagram']:
                inst = deep_find(obj, 'instagram')
                if inst:
                    result['instagram'] = inst
            if not result['tiktok']:
                tt = deep_find(obj, 'tiktok')
                if tt:
                    result['tiktok'] = tt
        for node in graph.get('nodes', []):
            if node.get('type') in ('social', 'vk'):
                if not result['vk'] and node.get('title'):
                    result['vk'] = node['title']
            if node.get('type') == 'ok':
                if not result['ok'] and node.get('title'):
                    result['ok'] = node['title']
            if node.get('type') == 'instagram':
                if not result['instagram'] and node.get('title'):
                    result['instagram'] = node['title']
            if node.get('type') == 'tiktok':
                if not result['tiktok'] and node.get('title'):
                    result['tiktok'] = node['title']
    ...
    # Преобразуем соцсети в чистый URL/текст
...
def format_phone_report(data: dict, views: int) -> str:
    ...
    # Соцсети – сразу после основных данных
    if data.get('vk'):
        lines.append(f"\n🧑‍💻 Вконтакте: {data['vk']}")
    if data.get('ok'):
...
async def main():
    await init_db()
    print("🚀 Бот запущен (соцсети сверху email)")
