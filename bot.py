...
async def jitler_search_with_balancer(query: str, search_type: str = "number"):
    ...
    Максимум 10 попыток с задержкой 0.5 сек.
    """
                ...
                async with session.post(url, json=payload, headers=headers, timeout=create_timeout) as resp:
                    if resp.status == 200:
                        ...
                        if data.get('result'):
                            if 'id' in data:
                                task_id = data['id']
                                for poll_attempt in range(10):  # максимум 10 попыток
...
# ===== СБОР ДАННЫХ ДЛЯ ID (ОПТИМИЗИРОВАННЫЙ) =====
async def collect_id_data(query: str, search_type: str):
    """
    Сбор данных для ID с таймаутом на всё выполнение.
    """
    try:
        bigbase, jitler = await asyncio.wait_for(
            asyncio.gather(
                bigbase_search(query),
...
async def main():
    await init_db()
    print("🚀 Бот запущен (оптимизированная скорость ID-поиска)")
