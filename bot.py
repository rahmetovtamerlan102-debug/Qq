...
# === CRYPTOPAY ===
CRYPTOPAY_TOKEN = os.getenv("CRYPTOPAY_TOKEN", "626190:AAkRdsFEHPdfZc6yIACB52hh1mVKkOdT0qc")

CHANNEL_USERNAME = "@dataseekerinfo"
CHANNEL_LINK = "tg://resolve?domain=dataseekerinfo"
ADMIN_IDS = [int(id.strip()) for id in os.getenv("ADMIN_IDS", "8559629118").split(",") if id.strip()]
...
async def depsearch_search(query: str):
    ...
    try:
        async with session.get(
            ...
            logger.info(f"DepSearch: HTTP {response.status}, length={len(text)}")
            
            if response.status != 200:
                logger.warning(f"DepSearch HTTP error: {response.status}")
                return {}
            
            try:
                data = json.loads(text)
            except json.JSONDecodeError:
                logger.error("DepSearch вернул невалидный JSON")
                return {}
            
            if not isinstance(data, dict):
                return {}
            
            logger.info(f"DepSearch RAW keys: {list(data.keys())}")
            if data.get('results'):
                logger.info(f"DepSearch RAW results count: {len(data['results'])}")
                if data['results']:
                    logger.info(f"DepSearch RAW first result: {data['results'][0]}")
        ...
        logger.error(f"DepSearch client error: {e}")
        return {}
    except Exception as e:
        logger.exception(f"DepSearch error: {e}")
        return {}
...
async def collect_general_data(query: str, search_type: str = "phone"):
    ...
    if cache_key in cache:
        logger.info(f"Cache hit for {query}, but skipping for debug")
    ...
    for name, task in tasks.items():
        try:
            timeout = 16.0 if name == "depsearch" else 10.0
    ...
    for idx, item in enumerate(dep_results, 1):
        ...
        if record_key not in seen_records:
            ...
            for key in ['👤ФИО', 'ФИО', 'full_name', 'fio', 'Полное имя', 'Имя']:
                if key in item and not result['fio']:
                    val = str(item[key]).strip()
                    if val and val not in ['None', 'null', '']:
                        result['fio'] = val
                        break
            
            for key in ['🎂Дата рождения', 'Дата рождения', 'birthdate']:
                if key in item and not all_birthdates:
            ...
            for key in ['📍Адрес', 'Адрес', 'address']:
                if key in item and not result['address']:
                    addr = str(item[key]).strip()
                    if not re.match(r'^\d{4}-\d{2}-\d{2}', addr):
                        result['address'] = addr
                    break
            
            for key in ['✉️Почта', 'email', 'Email']:
                if key in item and '@' in str(item[key]):
                    email = str(item[key]).strip()
                    if email and email not in result['emails']:
                        result['emails'].append(email)
                    break
            
            for key in ['📞Телефон', 'Телефон', 'phone']:
                if key in item:
                    phone_val = str(item[key]).strip()
            ...
            if not result['vk']:
                for key in ['🧑‍💻Вконтакте', 'vk', 'VK']:
                    if key in item:
                        result['vk'] = get_social_url(item[key])
                        break
            
            if not result['ok']:
                for key in ['👨‍🦳Одноклассники', 'ok', 'OK']:
                    if key in item:
                        result['ok'] = get_social_url(item[key])
                        break
            
            if not result['instagram']:
                for key in ['📷Instagram', 'instagram']:
                    if key in item:
                        result['instagram'] = get_social_url(item[key])
                        break
            
            if not result['tiktok']:
                for key in ['👩‍🦲TikTok', 'tiktok']:
                    if key in item:
                        result['tiktok'] = get_social_url(item[key])
                        break
        ...
        if block['type'] == 'result':
            ...
            if record_key not in seen_records:
                ...
                if not all_birthdates:
                    for key in ['Дата рождения', 'birthdate']:
                        if key in fields:
    ...
    for block in jitler_parsed:
        if block['type'] == 'phonebook':
            ...
            if record_key not in seen_records:
                ...
                if 'vk' in profile_data.get('Ссылка', '').lower() and not result['vk']:
                    result['vk'] = profile_data.get('Ссылка', '')
                elif 'ok' in profile_data.get('Ссылка', '').lower() and not result['ok']:
                    result['ok'] = profile_data.get('Ссылка', '')
                elif 'instagram' in profile_data.get('Ссылка', '').lower() and not result['instagram']:
                    result['instagram'] = profile_data.get('Ссылка', '')
                elif 'tiktok' in profile_data.get('Ссылка', '').lower() and not result['tiktok']:
                    result['tiktok'] = profile_data.get('Ссылка', '')
...
def generate_html_report(data: dict, views: int = 0) -> str:
    ...
    if data.get('phone_books'):
        main_info.append(("Телефоны", ', '.join(data['phone_books'][:5])))
    if data.get('vk'):
