...
def clean_value(value):
    ...
    if isinstance(value, dict):
        if set(value.keys()) <= {'registered', 'photo', 'lastseen'}:
            return None
        for k in ['value', 'username', 'id', 'url', 'email', 'name', 'full_name']:
            if k in value and value[k]:
                return str(value[k])
        if 'registered' in value and not value['registered']:
            return None
...
def extract_email(email_data):
    if isinstance(email_data, dict):
        return email_data.get('value') or email_data.get('email')
    return email_data

async def collect_data(query: str, search_type: str = "number", is_phone: bool = True):
    bigbase = await bigbase_search(query)
    nightsearch = await nightsearch_search(query) if is_phone else {}
    seon = await seon_search(query) if is_phone else {}
    jitler = await jitler_search_with_balancer(query, search_type)
    ...
    if bigbase and isinstance(bigbase, dict):
        ...
        result['emails'].extend(deep_find_all(bigbase, 'email') + deep_find_all(bigbase, 'mail') + deep_find_all(bigbase, 'e-mail'))
        result['telegrams'].extend(deep_find_all(bigbase, 'telegram') + deep_find_all(bigbase, 'tg'))
        
        vk_data = deep_find_all(bigbase, 'vk') + deep_find_all(bigbase, 'vkontakte')
        if vk_data and clean_value(vk_data[0]):
            result['vk'] = vk_data[0]
        
        inst_data = deep_find_all(bigbase, 'instagram')
        if inst_data and clean_value(inst_data[0]):
            result['instagram'] = inst_data[0]
        
        tt_data = deep_find_all(bigbase, 'tiktok')
        if tt_data and clean_value(tt_data[0]):
            result['tiktok'] = tt_data[0]
        
        ok_data = deep_find_all(bigbase, 'ok') + deep_find_all(bigbase, 'odnoklassniki')
        if ok_data and clean_value(ok_data[0]):
            result['ok'] = ok_data[0]
        
        result['banks'].extend(deep_find_all(bigbase, 'bank') + deep_find_all(bigbase, 'banks') + deep_find_all(bigbase, 'account'))
        
        # Для ID - дополнительные поля
        if not is_phone:
            groups = deep_find_all(bigbase, 'groups') + deep_find_all(bigbase, 'chats')
            if groups:
                result['groups'] = [g for g in groups if clean_value(g)]
            interests = deep_find_all(bigbase, 'interests')
            if interests:
                result['interests'] = [i for i in interests if clean_value(i)]
            history = deep_find_all(bigbase, 'username_history') + deep_find_all(bigbase, 'history')
            if history:
                result['username_history'] = history
            gifts = deep_find_all(bigbase, 'gifts') + deep_find_all(bigbase, 'presents')
            if gifts:
                result['gift_ids'] = [g for g in gifts if clean_value(g)]
    ...
    for src in [nightsearch, seon, jitler]:
        ...
        # Для ID из JITLER
        if not is_phone and src == jitler:
            groups = deep_find_all(src, 'groups') + deep_find_all(src, 'chats')
            if groups:
                result['groups'].extend(groups)
            interests = deep_find_all(src, 'interests')
            if interests:
                result['interests'].extend(interests)
            history = deep_find_all(src, 'username_history') + deep_find_all(src, 'history') + deep_find_all(src, 'name_history')
            if history:
                result['username_history'].extend(history)
            gifts = deep_find_all(src, 'gifts') + deep_find_all(src, 'presents')
            if gifts:
                result['gift_ids'].extend(gifts)
    
    # Очистка и удаление дублей
    result['emails'] = list(dict.fromkeys([e for e in result['emails'] if clean_value(e)]))
    result['telegrams'] = list(dict.fromkeys([t for t in result['telegrams'] if clean_value(t)]))
    result['banks'] = list(dict.fromkeys([b for b in result['banks'] if clean_value(b)]))
    result['groups'] = list(dict.fromkeys([g for g in result['groups'] if clean_value(g)]))
    result['interests'] = list(dict.fromkeys([i for i in result['interests'] if clean_value(i)]))
    result['gift_ids'] = list(dict.fromkeys([g for g in result['gift_ids'] if clean_value(g)]))
    
    if result['vk'] and not clean_value(result['vk']):
        result['vk'] = None
    if result['instagram'] and not clean_value(result['instagram']):
        result['instagram'] = None
    if result['tiktok'] and not clean_value(result['tiktok']):
        result['tiktok'] = None
    if result['ok'] and not clean_value(result['ok']):
        result['ok'] = None
...
def format_report(data: dict) -> str:
    ...
    if data.get('emails'):
        ...
        for email in data['emails']:
            cleaned = extract_email(email)
            if cleaned and cleaned != 'None':
                clean_emails.append(str(cleaned))
        if clean_emails:
            lines.append(f"\n📧 E-mail: {', '.join(clean_emails)}")
    ...
    if data.get('instagram'):
        inst_clean = clean_value(data['instagram'])
        if inst_clean:
            lines.append(f"\n📷 Instagram: {inst_clean}")
    
    if data.get('tiktok'):
        tt_clean = clean_value(data['tiktok'])
        if tt_clean:
            lines.append(f"\n🎵 TikTok: {tt_clean}")
    ...
    if data.get('banks'):
        ...
        for bank in data['banks']:
            cleaned = clean_value(bank)
            if cleaned:
                clean_banks.append(str(cleaned))
        if clean_banks:
            lines.append(f"\n🏦 Банки: {', '.join(clean_banks)}")
...
def format_id_report(data: dict, views: int) -> str:
    ...
    if data.get('gift_ids'):
        ...
        lines.append(", ".join([str(g) for g in data['gift_ids']]))
...
# ----- ПОКАЗ КЛАВИАТУРЫ ВЫБОРА НАПРАВЛЕНИЯ ДЛЯ ID -----
...
async def direction_callback(callback: types.CallbackQuery):
    ...
    if cached:
        ...
        detailed = format_id_report(merged, views)
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
    ...
    if has_data:
        await save_id_report(cache_key, {'merged': data})
...
async def number_handler(message: types.Message):
    ...
    if is_phone:
        ...
        # Это ID - показываем выбор направления
