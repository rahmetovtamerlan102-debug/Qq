...
async def collect_phone_data(query: str):
    ...
    for name, task in tasks.items():
        try:
            ...
            logger.warning(f"Timeout {name} для {query}")
            results[name] = {}
            task.cancel()
...
async def admin_callback(callback: CallbackQuery, state: FSMContext = None):
    ...
    if action == "stats":
        ...
        await callback.message.edit_text("Введите текст рассылки:", reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton(text="Назад", callback_data="admin_back")]
        ]))
        await state.set_state(Broadcast.waiting_for_text)
    elif action == "give":
        await callback.message.edit_text("Введите ID пользователя:", reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton(text="Назад", callback_data="admin_back")]
        ]))
        ...
        promos = await get_all_promo_codes()
        if not promos:
            await callback.message.edit_text("Нет промокодов для удаления.", reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(text="Назад", callback_data="admin_back")]
            ]))
        else:
            keyboard = InlineKeyboardMarkup(inline_keyboard=[])
