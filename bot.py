...
# ⚠️ ЭТОТ ТОКЕН НЕ РАБОТАЕТ ДЛЯ CRYPTOPAY – замените на токен приложения
...
# ===== ФУНКЦИИ ДЛЯ ПОКУПКИ ЗАПРОСОВ (FIAT USD) =====
...
async def buy_requests_callback(callback: types.CallbackQuery):
    ...
    for pkg in PACKAGES:
        ...
        keyboard.inline_keyboard.append([
            InlineKeyboardButton(
                text=f"{pkg['requests']} запросов — ${pkg['usd']} USD",
    ...
    await callback.message.edit_text(
        ...
        "Оплата производится в USD – вы сможете оплатить любой криптовалютой.",
        parse_mode="Markdown",
        reply_markup=keyboard
    )
    await callback.answer()
...
async def buy_package_callback(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        f"Нажмите «Оплатить счёт» – выберите удобную криптовалюту.\n"
