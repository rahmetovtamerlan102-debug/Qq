...
async def collect_data(query: str, search_type: str, is_phone: bool = False):
    ...
    # ---- Оператор, регион, страна (для телефона) ----
    if is_phone:
        op = None
        reg = None
        country = None
...
async def referral_callback(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(text)  # parse_mode не указан, значит plain text
    await callback.answer()
...
async def raw_cmd(message: types.Message):
    ...
    if not re.fullmatch(r'\d{10,12}', query):
        await message.reply("❌ Введите номер телефона (10-12 цифр).")
        return
    data = await collect_data(query, "number", is_phone=True)
...
async def main():
    await init_db()
    print("🚀 Бот запущен (исправленный, с командой /raw)")
