    ...
    async with pool.acquire() as conn:
        await conn.execute('''
            CREATE TABLE IF NOT EXISTS users (
                ...
                bonus_requests INTEGER DEFAULT 0   -- бонусные запросы за рефералов
            )
        ''')
        await conn.execute('''
...
async def create_user(user_id: int, username: str = None, referred_by: int = None):
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute('''
            INSERT INTO users (user_id, username, referral_code, referred_by, created_at, daily_requests, last_request_date, bonus_requests)
            VALUES ($1, $2, $3, $4, NOW(), 0, CURRENT_DATE, 0)
...
async def get_bonus_requests(user_id: int) -> int:
    pool = await get_pool()
    async with pool.acquire() as conn:
        row = await conn.fetchrow('SELECT bonus_requests FROM users WHERE user_id = $1', user_id)
...
async def get_daily_limit(user_id: int) -> int:
    """Общий лимит на сегодня: 5 + бонусные запросы"""
    return 5 + await get_bonus_requests(user_id)
...
# ===== ТЕКСТОВЫЕ ФОРМАТТЕРЫ (без изменений) =====
    ...
    if not user:
        ...
        if referrer_id:
            ...
            async with pool.acquire() as conn:
                ...
                await conn.execute('UPDATE referrals SET bonus_given = TRUE WHERE referrer_id = $1 AND referred_id = $2', 
                                   referrer_id, message.from_user.id)
            # Уведомление рефереру с именем и ID
            username_display = f"@{message.from_user.username}" if message.from_user.username else f"ID {message.from_user.id}"
            try:
                await bot.send_message(
                    referrer_id,
                    f"🎉 Пользователь {username_display} (ID: {message.from_user.id}) перешёл по вашей реферальной ссылке!\n"
                    f"Вы получили +1 бонусный запрос (всего бонусов: {await get_bonus_requests(referrer_id)})"
                )
            except Exception:
                pass

    # Кнопка "Информация о боте" и "Реферальная система"
...
Приглашайте друзей — получайте дополнительные запросы!"""
    await message.reply(text, parse_mode="Markdown")

# Обработчик инлайн-кнопки "Реферальная система"
...
async def referral_info_callback(callback: types.CallbackQuery):
    ...
    if not ref_code:
        await callback.answer("Ошибка: не найден код", show_alert=True)
        return
    ...
    text = (
        ...
        f"{ref_link}\n\n"
        f"👥 Приглашено: {stats} чел.\n"
        f"➕ Бонусных запросов: {bonus}\n\n"
        f"За каждого приглашённого, который начнёт пользоваться ботом, вы получаете +1 запрос в день (максимум не ограничен)."
    )
...
async def universal_handler(message: types.Message):
    ...
    # Проверка лимита (5 + бонусы)
    daily_requests = await get_daily_requests(message.from_user.id)
    limit = await get_daily_limit(message.from_user.id)
    if daily_requests >= limit:
        await message.reply(
            f"❌ Вы исчерпали лимит запросов на сегодня ({limit} в день).\n"
            "Попробуйте завтра или пригласите друзей для увеличения лимита! ⏰",
            parse_mode="Markdown"
        )
        return
