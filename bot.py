...
def generate_html_report(data: dict, views: int, title: str) -> str:
    # Упрощённый HTML для демонстрации
    html = f"""
    <!DOCTYPE html>
    <html>
...
async def universal_handler(message: types.Message):
    ...
    has_data = False
    for key, val in data.items():
        if val and key not in ['query', 'raw_data']:
            has_data = True
            break
    if not has_data and not data.get('raw_data'):
        await status_msg.edit_text(f"❌ По запросу `{query}` ничего не найдено.\n\n💡 Попробуйте другой запрос.")
        return
