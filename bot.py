    ...
    if depsearch:
        ...
        if isinstance(results_list, list):
            for idx, item in enumerate(results_list):
                ...
                # Собираем все поля, кроме служебных
                record_data = {}
                record_key = ""
                for k, v in item.items():
                    ...
                    if v and v not in (None, '', [], {}):
                        # Убираем эмодзи из ключа для красоты
...
# === ГЕНЕРАТОР HTML-ОТЧЁТА (БЕЗ ЭМОДЗИ) ===
...
# === ФОРМАТТЕР ТЕКСТОВОГО ОТЧЁТА (БЕЗ ЭМОДЗИ) ===
def format_general_report(data: dict, views: int = 0) -> str:
    if data.get('records_count', 0) == 0 and not any([data.get('fio'), data.get('operator')]):
        return f"Ничего не найдено\n\nПо вашему запросу `{data.get('query')}` данных нет."

    lines = []
    query = data.get('query', '')
    search_type = data.get('type', 'unknown').upper()
    lines.append(f"{search_type}: {query}")
    lines.append("")
    ...
    if data.get('snils'):
        lines.append(f"СНИЛС: {data['snils']}")
    if data.get('passport'):
        lines.append(f"Паспорт: {data['passport']}")
    ...
    if data.get('phone_books'):
        lines.append("")
        lines.append(f"Телефонные книги: {', '.join(data['phone_books'][:5])}")
    if data.get('vk'):
        lines.append("")
    ...
    if data.get('extra'):
        ...
        for key, rec in data['extra'].items():
    ...
    lines.append(f"\nНайдено записей: {data.get('records_count', 0)}")
    if data.get('sources'):
        lines.append(f"Источники: {', '.join(data['sources'])}")
    return "\n".join(lines)
...
async def start_cmd(message: Message):
    ...
    main_text = (
        "dataseeker — твой бесплатный цифровой детектив.\n\n"
        "Типы поиска:\n\n"
        "Контакты:\n"
        "  Телефон → +79999999999\n"
...
async def process_general_query(message: Message, query: str, search_type: str):
    ...
    try:
        ...
        if search_type == "phone":
            ...
            report = format_general_report(data)
...
# === УНИВЕРСАЛЬНЫЙ ОБРАБОТЧИК (запускает detect_type) ===
