...
# Токен TeleLog (вставлен по вашему запросу)
...
async def collect_telelog_only(query: str):
    print(f"🔍 Быстрый поиск Telegram ID: {query} (только TeleLog + BigBase)")
...
async def collect_id_data(query: str, search_type: str):
    ...
    # ---- ОБРАБОТКА DEPSEARCH, NIGHTSEARCH, SEON, SNUSBASE (похоже на collect_phone_data) ----
    for src in [depsearch, nightsearch, seon, snusbase]:
        if not src:
            continue
        # Здесь можно добавить парсинг нужных полей, если они есть
        # Например, извлечение email, телеграм, соцсетей и т.д.
        # Для простоты пока пропускаем, но можно расширить
...
async def main():
    await init_db()
    print("🚀 Бот запущен (разделены TeleLog и полный поиск)")
