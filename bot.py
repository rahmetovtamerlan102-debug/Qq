...
async def phone_handler(message: types.Message):
    ...
    # Клавиатура: HTML-отчёт, Telegram, WhatsApp
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📄 Полный отчёт (HTML)", callback_data=f"html_report_{digits}")],
...
@dp.callback_query(lambda c: c.data and c.data.startswith("html_report_"))
async def html_report_callback(callback: types.CallbackQuery):
    digits = callback.data.replace("html_report_", "")
    data = await get_report(digits)
    if not data:
        await callback.answer("Отчёт не найден, повторите поиск")
        return
    views = await get_unique_views_phone(digits, callback.from_user.id)
    ...
    await callback.message.reply_document(html_file, caption="📄 Красивый OSINT-отчёт")
