...
CHANNEL_ID = None  # Будет получен автоматически
...
# ===== ПОЛУЧЕНИЕ ID КАНАЛА ПО ЮЗЕРНЕЙМУ =====
async def get_channel_id():
    ...
    if CHANNEL_ID is None:
        try:
            ...
            CHANNEL_ID = chat.id
            print(f"✅ ID канала получен: {CHANNEL_ID}")
...
async def start_cmd(message: types.Message):
    ...
    # Если подписан - создаём пользователя и показываем главное меню
...
async def check_subscription_callback(callback: types.CallbackQuery):
    ...
    if is_subscribed:
        ...
        # Закрепляем вечную ссылку
        msg_link = await callback.message.reply(
            "🔮 *Вечная ссылка на информацию:*\nЕсли удалят этого бота — то новую ссылку на него найдёте по кнопке ниже.",
            reply_markup=keyboard,
            parse_mode="Markdown"
        )
