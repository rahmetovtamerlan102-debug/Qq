...
async def collect_phone_data(query: str):
    bigbase, nightsearch, seon, jitler = await asyncio.gather(
...
def format_phone_report(data: dict, views: int) -> str:
    ...
    if data.get('fio') or data.get('birthdate') or data.get('age'):
        lines.append("\n👤 Основные данные")
        if data.get('fio'):
    ...
    if data.get('ok') and clean_value(data['ok']):
        lines.append(f"\n👥 Одноклассники: {clean_value(data['ok'])}")
    if data.get('banks'):
        lines.append(f"\n🏦 Банки: {', '.join([b for b in data['banks'] if clean_value(b)])}")
...
async def main():
    await init_db()
    print("🚀 Бот запущен (БЫСТРЫЙ ПОИСК)")
