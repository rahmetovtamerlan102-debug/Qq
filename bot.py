...
# ===== ФУНКЦИИ РАБОТЫ С БАЗОЙ (безлимитные) =====
...
async def deduct_request(user_id: int) -> bool:
    return True  # бесконечные запросы
...
# ===== ВСПОМОГАТЕЛЬНАЯ ФУНКЦИЯ ДЛЯ ГЛУБОКОГО ПОИСКА =====
def deep_search(obj, key, default=None):
    """
    Рекурсивно ищет ключ в любых вложенных словарях и списках.
            ...
            try:
                async with session.post(url, json=payload, headers=headers) as resp:
                    if resp.status == 200:
                        ...
                        print(f"JITLER токен {token} превысил лимит, переключаем")
                        continue
                    else:
                        error_text = await resp.text()
...
# ===== СБОР ДАННЫХ (универсальный, с глубоким поиском) =====
async def collect_data(query: str, search_type: str, is_phone: bool = False):
    ...
    # Логирование сырых ответов (для отладки)
    print("\n=== BIGBASE ===")
    print(json.dumps(bigbase, ensure_ascii=False, indent=2))
    print("\n=== NIGHTSEARCH ===")
    print(json.dumps(nightsearch, ensure_ascii=False, indent=2))
    print("\n=== SEON ===")
    print(json.dumps(seon, ensure_ascii=False, indent=2))
    print("\n=== JITLER ===")
    print(json.dumps(jitler, ensure_ascii=False, indent=2))

    sources = [bigbase, nightsearch, seon, jitler]

    # ---- Извлечение оператора, региона, страны (для телефона) ----
    if is_phone:
        op = None
        reg = None
        country = None
    ...
    # ---- Контакты (телефонные книги) и Telegram ----
    contacts = []
    telegram_raw = None
    for src in sources:
        if not src:
            continue
        # Ищем contacts/phone_books в любой глубине
    ...
    # ---- Для ID собираем дополнительные данные (группы, интересы, история, подарки) ----
    telegram_id = None
    reg_date = None
    username = None
    groups = []
    ...
    if not is_phone:
        # Ищем в BigBase, Nightsearch, Seon (они обычно дают структуру)
        for src in [bigbase, nightsearch, seon]:
            if not src:
                continue
            # telegram_id
            if not telegram_id:
                telegram_id = deep_search(src, 'telegram_id') or deep_search(src, 'tg_id') or deep_search(src, 'id')
            # reg_date
            if not reg_date:
                reg_date = deep_search(src, 'registration_date') or deep_search(src, 'reg_date') or deep_search(src, 'date')
            # username
            if not username:
                username = deep_search(src, 'username') or deep_search(src, 'nick')
            # groups
            if not groups:
    ...
    # Парсим Telegram-аккаунты из сырого поля
...
def format_phone_report(merged: dict, full_data: dict, phone: str, views: int) -> str:
    ...
    # ФИО, дата рождения, возраст – ищем в merged (там уже собрано из deep_search)
    fio = merged.get('fio') or merged.get('full_name') or merged.get('name')
    birthdate = merged.get('birthdate') or merged.get('birth_date') or merged.get('date_of_birth')
    age = merged.get('age')
    ...
    # Соцсети, банки, email – ищем в merged (они добавляются отдельно)
...
async def direction_callback(callback: types.CallbackQuery):
    ...
    # Проверяем кэш
    cached = await get_id_report(id_str)
    if cached:
...
async def number_handler(message: types.Message):
    ...
    if is_phone:
        ...
        # ID – показываем выбор, но сначала проверим кэш (если уже был поиск по этому ID)
        cached = await get_id_report(digits)
        if cached:
            # Если есть кэш, можно сразу показать результат? Но пользователь выбрал ID, поэтому показываем направление.
            # Но чтобы не потерять кэш, можно показать выбор, и при выборе использовать кэш.
            pass
...
async def main():
    await init_db()
    print("🚀 Бот запущен (исправленный, с глубоким парсингом и логами)")
