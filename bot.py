...
CHANNEL_USERNAME = "@dataseekerinfo"          # для проверки подписки
CHANNEL_LINK = "tg://resolve?domain=dataseekerinfo"  # для кнопки (ссылка-приглашение)
...
def generate_html_report(data: dict, views: int, title: str = "Отчёт по номеру телефона") -> str:
    ...
    <head>
        ...
        <style>
            /* все стили как в предыдущей версии (опущены для краткости, но они должны быть) */
...
async def start_cmd(message: types.Message):
    ...
    if not is_subscribed:
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=f"📢 Подписаться на {CHANNEL_USERNAME}", url=CHANNEL_LINK)],
