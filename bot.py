...
# === РАСШИРЕННЫЙ ПАРСИНГ HTML ===
async def parse_html_page(url: str, query: str = None):
    """Расширенный парсинг HTML страницы"""
    ...
    try:
        async with session.get(
            ...
            # 3. Поиск Адресов
            address_patterns = [
                r'(?:г\.?|город)\s*[А-Яа-я\-]+\s*(?:ул\.?|улица)\s*[А-Яа-я\-]+\s*(?:д\.?|дом)\s*\d+[А-Яа-я]?',
                r'(?:ул\.?|улица)\s*[А-Яа-я\-]+\s*(?:д\.?|дом)\s*\d+[А-Яа-я]?\s*(?:кв\.?|квартира)?\s*\d*',
                r'[А-Яа-я\-]+\s*(?:ул\.?|улица)\s*[А-Яа-я\-]+\s*\d+',
                r'(?:пр-т|проспект)\s*[А-Яа-я\-]+\s*\d+',
            ...
            result['snils'] = list(set(re.findall(snils_pattern, text)))[:5]
            ...
            for pattern in inn_patterns:
                matches = re.findall(pattern, text)
                if matches:
                    result['inn'].extend(matches)
            # Фильтруем слишком длинные числа
            result['inn'] = [inn for inn in list(set(result['inn'])) if 10 <= len(inn) <= 12][:5]
            
            # 6. Поиск Паспорта
            passport_patterns = [
                r'\d{4}\s?\d{6}',  # 4 цифры пробел 6 цифр
                r'\d{2}\s?\d{2}\s?\d{6}',
            ...
            for pattern in passport_patterns:
                matches = re.findall(pattern, text, re.IGNORECASE)
                if matches:
                    result['passport'].extend(matches)
            result['passport'] = list(set(result['passport']))[:5]
            
            # 7. Поиск Дат рождения
            birthdate_patterns = [
                r'\d{2}[./-]\d{2}[./-]\d{4}',
                r'\d{4}[./-]\d{2}[./-]\d{2}',
                r'\d{2}\s+(?:янв|фев|мар|апр|май|июн|июл|авг|сен|окт|ноя|дек)\s+\d{4}',
            ]
            for pattern in birthdate_patterns:
                matches = re.findall(pattern, text, re.IGNORECASE)
                if matches:
                    result['birthdates'].extend(matches)
            ...
            # 9. Поиск Telegram (дополнительно)
            tg_patterns = [
                r'@[a-zA-Z0-9_]{5,32}',
                r't\.me/[a-zA-Z0-9_]+',
            ...
            for pattern in tg_patterns:
                matches = re.findall(pattern, text)
                if matches:
                    result['telegrams'].extend(matches)
            ...
            if query:
                ...
                for p in soup.find_all(['p', 'div', 'span', 'li']):
                    ...
                    if query_lower in text_p.lower():
                        found_items.append({
                            'type': 'text',
                            'content': text_p[:200] + '...' if len(text_p) > 200 else text_p
                        })
...
async def parse_html_command(message: Message):
    ...
    try:
        ...
        # Персональные данные
        if result.get('snils') or result.get('inn') or result.get('passport') or result.get('birthdates'):
            text += f"🪪 **Персональные данные:**\n"
            if result.get('snils'):
                text += f"  🆔 СНИЛС: {', '.join(result['snils'][:3])}\n"
            if result.get('inn'):
                text += f"  📄 ИНН: {', '.join(result['inn'][:3])}\n"
            if result.get('passport'):
                text += f"  🪪 Паспорт: {', '.join(result['passport'][:3])}\n"
            if result.get('birthdates'):
                text += f"  🎂 Дата рождения: {', '.join(result['birthdates'][:3])}\n"
            text += "\n"
        
        # Соцсети
        if result.get('socials') or result.get('telegrams'):
            text += f"🌐 **Соцсети:**\n"
            if result.get('socials'):
                for social in result['socials'][:5]:
                    text += f"  • {social}\n"
        ...
        if query and result.get('found_data'):
            ...
            for item in result['found_data'][:5]:
