...
# ===== СБОР ДАННЫХ (ГАРАНТИРОВАННЫЙ ПАРСИНГ) =====
async def collect_data(query: str, search_type: str, is_phone: bool = False):
    ...
    # ---- ПРИНУДИТЕЛЬНЫЙ ПАРСИНГ BIGBASE ----
    op = None
    reg = None
    country = None
    fio = None
    ...
    # Если данные из BigBase не найдены, пробуем другие источники
    if not op:
...
async def main():
    await init_db()
    print("🚀 Бот запущен (финальная версия с парсингом BigBase)")
