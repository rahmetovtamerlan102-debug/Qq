...
if not FUNSTAT_TOKEN:
    print("⚠️ FUNSTAT_TOKEN не задан, Funstat API не будет использоваться")
    ...
    if FUNSTAT_TOKEN:
        try:
            ...
            if names_data and names_data.get('result'):
                result['raw_data']['funstat_names'] = names_data
            ...
            if usernames_data and usernames_data.get('result'):
                result['raw_data']['funstat_usernames'] = usernames_data
            ...
            if gifts_data and gifts_data.get('result'):
                result['raw_data']['funstat_gifts'] = gifts_data
...
async def universal_handler(message: Message):
    ...
    # Проверка на Telegram ID (формат: "id 12345678" или просто число)
    id_match = re.match(r'^id\s+(\d{5,15})$', text, re.IGNORECASE)
    if id_match:
        search_type = "telegram_id"
        query = id_match.group(1)
    elif re.match(r'^\d{5,15}$', cleaned):
        search_type = "telegram_id"
        query = cleaned
