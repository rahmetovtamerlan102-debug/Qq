...
def parse_depsearch(data: dict) -> dict:
    """
    Универсальный парсер DepSearch.
    Забирает ВСЕ поля из результатов, не привязываясь к конкретным ключам.
    """
    result = {}
    ...
    if isinstance(phone_info, dict):
        result["phone_info"] = phone_info.copy()
...
async def depsearch_search(query: str):
    """DepSearch с универсальным парсером"""
    ...
    try:
        async with session.get(
            ...
            logger.info(
                f"DepSearch: HTTP {response.status}, "
                f"length={len(text)}"
            )
            
            if response.status != 200:
                logger.warning(
                    f"DepSearch HTTP error: {response.status}"
                )
                return {}
            ...
            # Применяем универсальный парсер
            return parse_depsearch(data)
...
async def collect_general_data(query: str, search_type: str = "phone"):
    ...
    # ---- DepSearch (универсальный парсер) ----
    if depsearch.get("phone_info"):
        info = depsearch["phone_info"]
    ...
    if depsearch.get("results"):
        for item in depsearch["results"]:
            ...
            if record_key not in seen_records:
                ...
                # Извлекаем основные поля для быстрого доступа (не удаляя остальные)
