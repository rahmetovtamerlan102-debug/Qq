...
# ===== СБОР ДАННЫХ ДЛЯ TELEGRAM (TeleLog + BigBase + Jitler для книг/соцсетей) =====
async def collect_telegram_data(query: str):
    print(f"🔍 Поиск Telegram ID: {query}")
    ...
    # Jitler (телефонные книги, соцсети) — используем тип sherlock
    jitler = await jitler_search_with_balancer(query, "sherlock")
    ...
    # ---- Jitler (телефонные книги, соцсети) ----
    if jitler and isinstance(jitler, dict):
        ...
        # Также можем получить телеграм-аккаунты из Jitler (дополнительно)
    ...
    print(f"📊 Результат Telegram: телеграм={len(result['telegrams'])}, группы={len(result['groups'])}, история имён={len(result['name_history'])}, история юзернеймов={len(result['username_history'])}, книги={len(result['phone_books'])}, подарки={len(result['gifts'])}")
    return result

# ===== СБОР ДАННЫХ ДЛЯ VK (Jitler vks + BigBase + другие API, без TeleLog) =====
async def collect_vk_data(query: str):
    print(f"🔍 Поиск VK: {query}")
    ...
    # ---- Другие API (можно добавить извлечение данных, если нужно) ----
    # Пока просто сохраняем raw от Jitler, если есть
    if jitler and isinstance(jitler, dict):
        raw_text = jitler.get('response', {}).get('raw', '')
        if raw_text:
            result['raw_data'] = raw_text[:500]
    ...
    print(f"📊 Результат VK: телеграм={len(result['telegrams'])}, группы={len(result['groups'])}, книги={len(result['phone_books'])}")
    return result

# ===== СБОР ДАННЫХ ДЛЯ ДОКУМЕНТОВ (ИНН, ПАСПОРТ, ВУ) =====
async def collect_document_data(query: str, search_type: str):
    print(f"🔍 Поиск документа: {query}, тип: {search_type}")

    # Все API
    bigbase = await bigbase_search(query)
    jitler = await jitler_search_with_balancer(query, search_type)  # предполагается, что Jitler поддерживает эти типы
    depsearch = await depsearch_search(query)
    nightsearch = await nightsearch_search(query)
    seon = await seon_search(query)
    snusbase = await snusbase_search(query)
    ...
    # Собираем все сырые тексты в один
    raw_parts = []
    if bigbase and isinstance(bigbase, dict):
        raw_parts.append(f"BigBase: {json.dumps(bigbase, ensure_ascii=False)[:300]}")
    if jitler and isinstance(jitler, dict):
        raw_text = jitler.get('response', {}).get('raw', '')
        if raw_text:
            raw_parts.append(f"Jitler: {raw_text[:300]}")
    if depsearch and isinstance(depsearch, dict):
        raw_parts.append(f"DepSearch: {json.dumps(depsearch, ensure_ascii=False)[:300]}")
    if nightsearch and isinstance(nightsearch, dict):
        raw_parts.append(f"NightSearch: {json.dumps(nightsearch, ensure_ascii=False)[:300]}")
    if seon and isinstance(seon, dict):
        raw_parts.append(f"SEON: {json.dumps(seon, ensure_ascii=False)[:300]}")
    if snusbase and isinstance(snusbase, dict):
        raw_parts.append(f"Snusbase: {json.dumps(snusbase, ensure_ascii=False)[:300]}")

    if raw_parts:
        result['raw_data'] = "\n\n".join(raw_parts)[:1000]

    # Также можно извлечь группы из BigBase, если они есть
    if bigbase and isinstance(bigbase, dict):
        records = bigbase.get('records', [])
        for record in records:
            base_record = record.get('base_record', [])
    ...
    print(f"📊 Результат документа: группы={len(result['groups'])}, raw_data={len(result.get('raw_data', ''))}")
    return result
...
def format_id_report(data: dict, views: int, direction: str = "") -> str:
    lines = []
    if data.get('query'):
        lines.append(f"# {direction}: {data['query']}")
        lines.append("")
...
async def direction_callback(callback: types.CallbackQuery):
    ...
    # Используем новый ключ кэша (v2) чтобы не использовать старые данные
    cache_key = f"v3:{id_str}:{direction}"
    cached = await get_id_report(cache_key)
    if cached:
        data = cached.get('data', {})
        views = await get_unique_views_id(id_str, callback.from_user.id)
        detailed = format_id_report(data, views, direction.upper())
    ...
    try:
        if direction == "tg":
            data = await asyncio.wait_for(collect_telegram_data(id_str), timeout=15.0)
        elif direction == "vk":
            data = await asyncio.wait_for(collect_vk_data(id_str), timeout=20.0)
        elif direction in ("inn", "passport", "driver"):
            data = await asyncio.wait_for(collect_document_data(id_str, direction), timeout=20.0)
        else:
    ...
    # Сохраняем в кэш, если есть хоть какие-то данные
    if any([data.get('telegrams'), data.get('groups'), data.get('name_history'), data.get('username_history'), data.get('gifts'), data.get('raw_data')]):
...
async def number_handler(message: types.Message):
    ...
    if is_phone:
        ...
        data = await collect_phone_data(digits)   # функция collect_phone_data остаётся без изменений
...
async def main():
    await init_db()
    print("🚀 Бот запущен (разделены типы поиска)")
