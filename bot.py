...
async def collect_phone_data(query: str):
    ...
    for email in result['emails']:
        if isinstance(email, dict):
            ...
            # Извлекаем email из строки, если там есть @
...
def format_phone_report(data: dict, views: int) -> str:
    ...
    if has_personal:
        ...
        # Адрес скрыт, показывается только в HTML
    ...
    # Карты скрыты, показываются только в HTML
...
def generate_html_report(data: dict, views: int, title: str = "Отчёт по номеру телефона") -> str:
    ...
    if data.get('country'):
        ...
        """
    # Кнопки WhatsApp и Telegram (если есть номер)
    phone = data.get('query', '')
    if phone:
        phone_clean = re.sub(r'\D', '', phone)
        html += f"""
                <div class="row" style="border-bottom: none; padding-top: 10px;">
                    <span class="label">📱 Связаться</span>
                    <span class="value">
                        <a href="https://wa.me/{phone_clean}" target="_blank" class="contact-btn whatsapp">WhatsApp</a>
                        <a href="https://t.me/{phone_clean}" target="_blank" class="contact-btn telegram">Telegram</a>
                    </span>
                </div>
        """
    html += "</div>"

    # Основные данные (полные, включая адрес)
...
🤝 Реферальная система: за каждого приглашённого друга вы получаете +1 запрос в день."""
...
async def universal_handler(message: types.Message):
    ...
    # Быстрое определение ID (ускорено)
    if text.lower().startswith("id") and len(text) > 2:
        digits = text[2:].strip()
        if digits.isdigit():
            query = digits
        ...
        search_type = "telegram"
    elif re.match(r'^\+?\d{10,15}$', re.sub(r'\s+', '', text)):
    ...
    if search_type == "phone":
        ...
        await status_msg.edit_text("❌ Неизвестный тип запроса. Попробуйте отправить телефон, ID, email, IP, VK, ФИО или используйте /inn.")
...
async def referral_callback(callback: types.CallbackQuery):
    ...
    await callback.message.reply(text, parse_mode="Markdown", reply_markup=keyboard)
    await callback.answer()
