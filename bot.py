...
def depsearch_to_report(data: dict) -> dict:
    """
    Универсальное преобразование DepSearch в отчёт.
    Сохраняет ВСЕ поля из results.
    """
    report = {
        "results": [],
        "phone_info": {}
...
async def depsearch_search(query: str):
    """DepSearch через правильный endpoint /quest"""
...
async def collect_general_data(query: str, search_type: str = "phone"):
    ...
    # Временно удаляем кэш для отладки
