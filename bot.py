...
def detect_query_type(text: str) -> str:
    ...
    if text.isalpha():
        return "tag"
    # Если ничего не подошло, просто универсальный
    return "unknown"
...
async def collect_universal_data(query: str, search_type: str):
    ...
    # Jitler добавляем для номеров, телеграм, соцсетей, личности, тегов
        ...
        for record in records:
            ...
            for item in base_record:
                if isinstance(item, list) and len(item) >= 2:
                    ...
                    elif 'фио' in key or 'фисо' in key or 'full_name' in key:
                        if value:
                            result['fio'].append(str(value))
                    elif 'организация' in key or 'company' in key or 'название' in key:
                        if value:
                            result['organizations'].append(str(value))
                    elif 'дата рождения' in key or 'birthdate' in key:
                        if value:
                            result['person_data']['birthdate'] = str(value)
                    elif 'регистрация' in key or 'address' in key:
                        if value:
                            result['person_data']['address'] = str(value)
    ...
    if seon and isinstance(seon, dict):
        ...
        if data:
            ...
            if data.get('country'):
                result['person_data']['country'] = data['country']
            if data.get('carrier'):
                result['person_data']['carrier'] = data['carrier']
            if data.get('line_type'):
                result['person_data']['line_type'] = data['line_type']
    ...
    # Фильтруем email
    result['emails'] = [e for e in result['emails'] if len(e) < 100 and '@' in e]
    ...
    result['phones'] = list(dict.fromkeys(cleaned_phones))[:10]
    result['fio'] = list(dict.fromkeys([x for x in result['fio'] if x]))[:5]
    result['organizations'] = list(dict.fromkeys([x for x in result['organizations'] if x]))[:5]
...
def format_universal_report(data: dict, views: int) -> str:
    ...
    # Персональные данные
    person = data.get('person_data', {})
    if person:
        if person.get('fio'):
            lines.append(f"👤 ФИО: {person['fio']}")
        if person.get('birthdate'):
            lines.append(f"📅 Дата рождения: {person['birthdate']}")
        if person.get('address'):
            lines.append(f"🏠 Адрес: {person['address']}")
        if person.get('carrier'):
            lines.append(f"📡 Оператор: {person['carrier']}")
        if person.get('line_type'):
            lines.append(f"📞 Тип линии: {person['line_type']}")
        if person.get('country'):
            lines.append(f"🌍 Страна: {person['country']}")
        if person.get('fio') or person.get('birthdate'):
            lines.append("")
    ...
    if data.get('organizations'):
        lines.append(f"🏢 Организации: {', '.join(data['organizations'])}")
        lines.append("")
        ...
        for entry in data['name_history'][:10]:
            if isinstance(entry, dict):
                ...
                if date:
                    ...
                    lines.append(f"  → {name}")
        if len(data['name_history']) > 10:
            lines.append(f"  ... и ещё {len(data['name_history'])-10}")
        lines.append("")
    ...
    if data.get('username_history'):
        ...
        if len(data['username_history']) > 10:
            lines.append(f"  ... и ещё {len(data['username_history'])-10}")
        lines.append("")
    ...
    if data.get('gifts'):
        ...
        for gift in data['gifts'][:10]:
            lines.append(f"  {gift}")
        if len(data['gifts']) > 10:
            lines.append(f"  ... и ещё {len(data['gifts'])-10}")
        lines.append("")
    
    lines.append(f"👁 Интересовались этим: {views}")
    return "\n".join(lines)

# ===== ФУНКЦИЯ ДЛЯ ОБРАБОТКИ НОМЕРА ТЕЛЕФОНА (ОСТАВЛЕНА ДЛЯ СОВМЕСТИМОСТИ) =====
async def collect_phone_data(query: str):
    # Упрощённый вызов универсального поиска
    return await collect_universal_data(query, "phone")
...
async def start_cmd(message: types.Message):
    ...
    await message.reply("Отправьте номер телефона, ID Telegram, ФИО, VIN, email или ссылку для поиска", reply_markup=keyboard)
...
async def universal_handler(message: types.Message):
    ...
    # Определяем views (для цифр считаем как ID, иначе 0)
    if text.isdigit():
