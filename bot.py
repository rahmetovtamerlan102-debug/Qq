...
# TeleLog token (можно заменить через .env)
...
async def collect_id_data(query: str, search_type: str):
    ...
    # Если это специальные типы (ИНН, паспорт, ВУ) — не добавляем Telegram-данные, только raw
    if search_type in ("inn", "passport", "driver"):
        if jitler and isinstance(jitler, dict):
            jitler_data = jitler.get('response', {})
            raw_text = jitler_data.get('raw', '')
            if raw_text:
                result['raw_data'] = raw_text[:1000]
        # Не обрабатываем BigBase для этих типов, чтобы не было лишней информации
        return result

    # ---- ОБРАБОТКА BIGBASE (для VK и других) ----
    if bigbase and isinstance(bigbase, dict):
        records = bigbase.get('records', [])
        for record in records:
            base_record = record.get('base_record', [])
    ...
    # ---- ОБРАБОТКА JITLER (для VK и других) ----
    if jitler and isinstance(jitler, dict):
...
async def direction_callback(callback: types.CallbackQuery):
    ...
    cache_key = f"v2:{id_str}:{direction}"
...
async def main():
    await init_db()
    print("🚀 Бот запущен (исправлен кэш, убраны двойные @, для ИНН/паспорт/ВУ только raw)")
