...
from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton, BufferedInputFile, PreCheckoutQuery, LabeledPrice
...
# ===== ПАКЕТЫ ДЛЯ ПОКУПКИ (только USDT) =====
PACKAGES = [
    {"requests": 5, "usd": 1.50},
    {"requests": 10, "usd": 2.50},
    {"requests": 25, "usd": 5.00},
    {"requests": 50, "usd": 9.00},
    {"requests": 100, "usd": 15.00},
]
...
# ===== ФУНКЦИИ РАБОТЫ С ПОЛЬЗОВАТЕЛЯМИ И РЕФЕРАЛКАМИ =====
...
# ===== ФУНКЦИИ ДЛЯ ПОКУПКИ ЗАПРОСОВ =====
async def create_crypto_pay_invoice(user_id: int, amount_usd: float, requests_count: int) -> str:
    """Создаёт счёт в CryptoPay и возвращает ссылку для оплаты"""
    session = await get_http_session()
    ...
    async with pool.acquire() as conn:
        ...
        try:
            await bot.send_message(
                user_id,
                f"✅ Оплата подтверждена! Вам начислено {requests} дополнительных запросов.\n"
...
async def start_cmd(message: types.Message):
        ...
        if referrer_id:
            try:
                ...
                if ref_user:
                    ...
                    await bot.send_message(
                        referrer_id,
                        f"По вашей реферальной ссылке перешёл новый пользователь {ref_username} (ID: {user_id}). Вы получили +1 бонусный запрос."
                    )
                    referral_message = (
                        f"Вы зарегистрировались по реферальной ссылке пользователя {ref_username}.\n"
                        f"Ваш реферер получил +1 дополнительный запрос.\n"
                        f"Теперь у вас есть 5 бесплатных запросов в сутки."
                    )
            except Exception:
                pass
    else:
                ...
                if not existing:
                    ...
                    try:
                        ...
                        if ref_user:
                            ...
                            await bot.send_message(
                                referrer_id,
                                f"Пользователь {ref_username} (ID: {user_id}) перешёл по вашей реферальной ссылке. Вы получили +1 бонусный запрос."
                            )
                            referral_message = (
                                f"Вы активировали реферальную ссылку пользователя {ref_username}.\n"
                                f"Ваш реферер получил +1 дополнительный запрос."
                            )
                    except Exception:
                        pass
    ...
    keyboard_info = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🌐 Информация о боте", url=CHANNEL_LINK)]
    ])
...
async def my_profile_callback(callback: types.CallbackQuery):
    ...
    days = delta.days
    agent_time = f"{days} {plural_days(days)}"
...
async def referral_system_callback(callback: types.CallbackQuery):
    user_id = callback.from_user.id
    user = await get_user(user_id)
    if not user:
        await callback.answer("❌ Вы не зарегистрированы.")
    ...
    text = (
        ...
        f"Приглашайте друзей и зарабатывайте дополнительные запросы.\n"
        f"За каждого нового пользователя вы получаете +1 запрос.\n\n"
        f"Ваша ссылка для приглашения:\n"
        f"https://t.me/{bot_username}?start=ref{ref_code}\n\n"
        f"Приглашено: {invited}\n"
        f"Бонусов начислено: {bonuses}"
    )
...
async def buy_requests_callback(callback: types.CallbackQuery):
    """Показывает пакеты запросов для покупки"""
    keyboard = InlineKeyboardMarkup(inline_keyboard=[])
    for pkg in PACKAGES:
        keyboard.inline_keyboard.append([
            InlineKeyboardButton(
                text=f"{pkg['requests']} запросов — ${pkg['usd']} USDT",
                callback_data=f"buy_pkg_{pkg['requests']}_{pkg['usd']}"
            )
        ])
    keyboard.inline_keyboard.append([
        InlineKeyboardButton(text="🔙 Назад", callback_data="back_to_menu")
    ...
    await callback.message.edit_text(
        ...
        "Оплата производится в USDT (TRC20) через CryptoPay.\n"
        "После оплаты запросы начисляются автоматически.",
        parse_mode="Markdown",
        reply_markup=keyboard
    )
    await callback.answer()

@dp.callback_query(lambda c: c.data and c.data.startswith("buy_pkg_"))
async def buy_package_callback(callback: types.CallbackQuery):
    """Обрабатывает выбор пакета и создаёт счёт"""
    _, requests_str, usd_str = callback.data.split("_")
    requests_count = int(requests_str)
    amount_usd = float(usd_str)
    user_id = callback.from_user.id

    # Создаём счёт в CryptoPay
    pay_url = await create_crypto_pay_invoice(user_id, amount_usd, requests_count)
    if not pay_url:
        await callback.message.edit_text(
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="✅ Я оплатил (проверить)", callback_data=f"check_payment_{requests_count}")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="buy_requests")]
    ])
    await callback.message.edit_text(
        f"💳 **Счёт для оплаты:**\n\n"
        f"Пакет: {requests_count} запросов\n"
        f"Сумма: {amount_usd} USDT (TRC20)\n\n"
...
async def check_payment_callback(callback: types.CallbackQuery):
    """Проверяет статус оплаты (заглушка, т.к. вебхук обрабатывает автоматически)"""
    user_id = callback.from_user.id
    pool = await get_pool()
    async with pool.acquire() as conn:
        purchase = await conn.fetchrow(
            'SELECT * FROM purchases WHERE user_id = $1 AND status = $2 ORDER BY created_at DESC LIMIT 1',
            user_id, 'pending'
        )
        if not purchase:
            await callback.answer("❌ Нет активных платежей.")
            return
        # Проверяем через API CryptoPay
        session = await get_http_session()
        url = f"https://pay.crypt.bot/api/getInvoices?invoice_id={purchase['invoice_id']}"
                ...
                if resp.status == 200:
                    ...
                    if data.get('ok'):
                        ...
                        if invoices and invoices[0].get('status') == 'paid':
                            await process_crypto_pay_payment(purchase['invoice_id'])
                            await callback.message.edit_text(
                                f"✅ Оплата подтверждена! Вам начислено {purchase['requests']} запросов.",
                                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        ...
        await callback.answer("⏳ Платёж ещё не обработан. Подождите или нажмите позже.", show_alert=True)
...
async def promo_back_callback(callback: types.CallbackQuery, state: FSMContext):
    await state.clear()
...
async def check_subscription_callback(callback: types.CallbackQuery):
    ...
    if is_subscribed:
        user = await get_user(user_id)
        if not user:
            await create_user(user_id, callback.from_user.username, None)
        else:
            ref_code = await get_referral_code(user_id)
            if not ref_code:
                await generate_referral_code(user_id)
...
async def admin_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    if action == "stats":
        ...
        async with pool.acquire() as conn:
            ...
            promo_count = await conn.fetchval('SELECT COUNT(*) FROM promo_codes')
            purchases_count = await conn.fetchval('SELECT COUNT(*) FROM purchases WHERE status = $1', 'confirmed')
        text = (
            f"📊 **Статистика**\n"
            f"👤 Пользователей: {users_count}\n"
            f"📄 Отчётов сохранено: {reports_count}\n"
            f"🎫 Промокодов: {promo_count}\n"
            f"💰 Успешных платежей: {purchases_count}"
        )
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        await callback.message.edit_text(
            "Введите текст для рассылки (можно с Markdown).\nДля отмены нажмите кнопку Назад.",
            reply_markup=keyboard
        )
        ...
        await callback.message.edit_text(
            "Введите ID пользователя, которому хотите выдать запросы.\nДля отмены нажмите кнопку Назад.",
            reply_markup=keyboard
        )
        ...
        if not promos:
            ...
            for p in promos:
                text += f"`{p['code']}` — {p['used_count']}/{p['max_uses']} активаций, даёт {p['requests_granted']} запросов\n"
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 Назад", callback_data="admin_back")]
        ])
        await callback.message.edit_text(text, parse_mode="Markdown", reply_markup=keyboard)
        ...
        for p in promos:
            keyboard.inline_keyboard.append([
                InlineKeyboardButton(text=f"❌ {p['code']}", callback_data=f"delpromo_{p['code']}")
            ])
        ...
        if not rows:
            ...
            for p in rows:
                ...
                text += f"{status_emoji} {p['user_id']} — {p['requests']} запросов, {p['amount']} {p['currency']} — {p['status']}\n"
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 Назад", callback_data="admin_back")]
        ])
        await callback.message.edit_text(text, parse_mode="Markdown", reply_markup=keyboard)
...
async def process_promo_code(message: types.Message, state: FSMContext):
    ...
    if not re.match(r'^[A-Za-z0-9_\-]+$', code):
        await message.reply("❌ Промокод может содержать только буквы, цифры, подчёркивания и дефисы. Попробуйте снова.")
        return
    existing = await get_promo_code(code)
    if existing:
        await message.reply("❌ Такой промокод уже существует. Введите другой.")
        return
...
async def process_max_uses(message: types.Message, state: FSMContext):
    try:
        ...
        await message.reply(
            "Введите количество запросов, которое будет давать промокод (целое число).\nДля отмены нажмите кнопку Назад.",
            reply_markup=keyboard
        )
    except ValueError:
...
async def process_requests(message: types.Message, state: FSMContext):
    try:
        ...
        if requests_granted <= 0:
            await message.reply("❌ Количество запросов должно быть положительным числом.")
            return
        data = await state.get_data()
        code = data['code']
        max_uses = data['max_uses']
        created_by = message.from_user.id
        success = await create_promo_code(code, max_uses, requests_granted, created_by)
        if success:
            await message.reply(
                f"✅ Вы создали промокод!\n\n"
                f"Код: `{code}`\n"
                f"Активаций: {max_uses}\n"
                f"Запросов: {requests_granted}"
            )
        else:
            await message.reply("❌ Ошибка при создании промокода. Возможно, такой код уже существует.")
        await state.clear()
...
async def process_give_user_id(message: types.Message, state: FSMContext):
    try:
        ...
        await message.reply(
            "Введите количество запросов, которое хотите выдать (целое число).\nДля отмены нажмите кнопку Назад.",
            reply_markup=keyboard
        )
    except ValueError:
...
async def process_give_amount(message: types.Message, state: FSMContext):
    try:
        ...
        if not user_id:
            await message.reply("❌ Ошибка: ID пользователя не найден. Начните заново.")
            await state.clear()
            return
        user = await get_user(user_id)
        if not user:
        ...
        try:
            ...
            await bot.send_message(
                user_id,
                f"Вам выдали {amount} дополнительных запросов.\n"
                f"Теперь у вас доступно {available} запросов."
            )
        except Exception:
            pass
        await message.reply(
...
async def process_broadcast(message: types.Message, state: FSMContext):
    ...
    for user in users:
        try:
            ...
            print(f"Не удалось отправить сообщение {user['user_id']}: {e}")
...
# ===== ВЕБХУК ДЛЯ CRYPTOPAY =====
async def crypto_pay_webhook(request):
    """Обработчик вебхука от CryptoPay"""
    try:
...
async def main():
    ...
    print(f"🔗 Вебхук URL: https://qq-rb6p.onrender.com/webhook")
    try:
