...
async def collect_phone_data(query: str):
    ...
    if bigbase and isinstance(bigbase, dict):
        ...
        # Сбор всех полей из bigbase, включая адрес, карты, банки и остальное
...
# Другие сборщики (email, ip, vk, inn, fio) оставляем без изменений (они не используются для телефона)
...
# ===== HTML-ОТЧЁТ (только дополнительные данные) =====
def generate_html_report(data: dict, views: int, title: str = "Отчёт по номеру телефона") -> str:
    # Собираем поля для записи "Дополнительные данные"
    extra_fields = []
    ...
    if extra:
        for k, v in extra.items():
            if v:
                ...
                extra_fields.append((label, str(v)))

    # Если ничего нет – запись с сообщением
    if not extra_fields:
        extra_fields = [('', 'Ничего не найдено')]

    # Строим структуру отчёта (одна запись)
    structure_items = """
        <div class="client">
            ...
            <a href="#record1" class="clients_name">Дополнительные данные</a>
        </div>
        <div class="stick"></div>
    """
    ...
    accordion = f"""
    <div id="record1" class="accordion_inner">
    ...
    # Если есть хотя бы одно поле (кроме пустого), показываем запись, иначе заглушку
    if len(extra_fields) == 1 and extra_fields[0][0] == '':
        # Если только "Ничего не найдено" – показываем просто текст
        accordion = f"""
        <div class="accordion_inner">
            <div class="accordion open">
                ...
                <div class="accordion-body">
                    <div class="accordion-content">
                        <div class="row"><span>Ничего не найдено</span></div>
                    </div>
                </div>
            </div>
        </div>
