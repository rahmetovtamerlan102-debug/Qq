...
async def collect_phone_data(query: str):
    ...
    # Очистка email: оставляем только строки с '@'
    clean_emails = []
...
# ===== HTML-ОТЧЁТ (без дублирования записей) =====
def generate_html_report(data: dict, views: int, title: str = "Отчёт по номеру телефона") -> str:
    ...
    for key, value in extra.items():
        if isinstance(value, dict) and 'source' in value and 'data' in value:
            source = value['source']
            # Генерируем уникальный ключ записи для избежания дублирования
            record_key = source
...
async def universal_handler(message: types.Message):
    ...
    if search_type == "phone":
        ...
        # HTML-файл НЕ отправляется автоматически - только по кнопке
