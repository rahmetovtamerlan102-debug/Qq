...
async def referral_callback(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(text)  # без Markdown, чтобы избежать ошибки
    await callback.answer()
...
async def main():
    await init_db()
    print("🚀 Бот запущен (финальная версия с пулом подключений и корректным завершением)")

    # Настраиваем веб-сервер для health check
    app = web.Application()
    ...
    try:
        # Запускаем polling бота (будет работать, пока не остановят)
