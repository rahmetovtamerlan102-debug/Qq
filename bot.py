...
async def depsearch_search(query: str):
    ...
    try:
        ...
        # Пробуем без прокси
        return await depsearch_search_no_proxy(query)
...
async def depsearch_search_no_proxy(query: str):
    """DepSearch без прокси (запасной вариант)"""
    session = await get_http_session()
        ...
        async with session.get(
            ...
            if response.status == 200:
                try:
                    data = json.loads(text)
                    if data:
                        logger.info("DepSearch success without proxy!")
                        return data
                except:
                    pass
            
            logger.warning("DepSearch without proxy also failed: %s", response.status)
            return {}

    except Exception as e:
        logger.error("DepSearch without proxy error: %s", e)
        return {}
