...
async def buy_requests_callback(callback: types.CallbackQuery):
    keyboard = InlineKeyboardMarkup(inline_keyboard=[])
    for pkg in PACKAGES:
        # Используем разделитель | чтобы избежать проблем с точкой
...
async def buy_package_callback(callback: types.CallbackQuery):
    try:
        ...
        print(f"Ошибка парсинга callback: {callback.data}, {e}")
    ...
    async with pool.acquire() as conn:
        purchase = await conn.fetchrow(
            'SELECT invoice_id FROM purchases WHERE user_id = $1 ORDER BY created_at DESC LIMIT 1',
            user_id
        )
        invoice_id = purchase['invoice_id'] if purchase else None
