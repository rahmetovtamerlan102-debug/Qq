...
# ===== СБОР ДАННЫХ ДЛЯ TELEGRAM (TeleLog + BigBase + Jitler) =====
...
# ===== УНИВЕРСАЛЬНЫЙ СБОР ДЛЯ ВСЕГО ОСТАЛЬНОГО =====
async def collect_universal_data(query: str, search_type: str):
    tasks = [
        ...
        seon_search(query),
        snusbase_search(query),
        jitler_search_with_balancer(query, "sherlock") if search_type in ("person", "tag", "social") else asyncio.sleep(0, result={})
    ]
    results = await asyncio.gather(*tasks, return_exceptions=True)
    bigbase = results[0] if isinstance(results[0], dict) else {}
    ...
    result['gifts'] = list(dict.fromkeys([x for x in result['gifts'] if x]))
    result['emails'] = list(dict.fromkeys([e for e in result['emails'] if '@' in e]))[:10]
    result['phones'] = list(dict.fromkeys([p for p in result['phones'] if len(re.sub(r'\D', '', p)) >= 10]))[:10]
...
async def direction_callback(callback: types.CallbackQuery):
    ...
    try:
        if direction == "tg":
            ...
            data = await asyncio.wait_for(collect_universal_data(id_str, search_type), timeout=20.0)
    except asyncio.TimeoutError:
