...
async def collect_phone_data(query: str):
    ...
    result = {
        ...
        'banks': [],
        'extra': {},
        'sources': [],  # Список названий баз для подсчёта
        ...
        if isinstance(results_list, list):
            for item in results_list:
                if isinstance(item, dict):
                    source_name = item.get('🏫Источник', 'Unknown')
                    sources_set.add(source_name)
                    records_count += 1
                    record_data = {}
                    for k, v in item.items():
                        if k != '🏫Источник' and v and v not in (None, '', [], {}):
                            record_data[k] = v
                    if record_data:
                        result['extra'][f"Запись #{records_count}"] = {
                            'source': source_name,
                    ...
                    # Собираем основные данные из первой записи
                    if not result['fio']:
                        fio = (
                            item.get('👤ФИО') or
                            item.get('👤Имя') or
...
# Остальные сборщики (email, ip, vk, inn, fio) остаются без изменений
...
def format_value(value):
    """Форматирует значение для отображения в HTML"""
    if value is None:
        return ""
    if isinstance(value, str):
        return value
...
def format_phone_report(data: dict, views: int) -> str:
    ...
    # Информация о количестве записей и баз
    if data.get('records_count'):
        lines.append(f"\n📊 Количество записей: {data['records_count']}")
    if data.get('sources'):
        lines.append(f"📁 Количество баз: {len(data['sources'])}")
...
def generate_html_report(data: dict, views: int, title: str = "Отчёт по номеру телефона") -> str:
    """Генерирует HTML-отчёт с записями по источникам (утечкам)"""
    
    # Собираем записи из extra
    extra = data.get('extra', {})
    records = []
    sources_set = set()
    records_count = 0
    
    # Извлекаем записи из extra (они там в формате Запись #N: {source: ..., data: {...}})
    ...
    # Добавляем адрес, карты, банки как отдельную запись, если они есть
    extra_fields = []
    if data.get('address'):
    ...
    # Добавляем оставшиеся extra-поля (которые не вошли в записи)
