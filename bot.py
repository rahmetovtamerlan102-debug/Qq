...
async def get_user_created_date(user_id: int):
    pool = await get_pool()
    async with pool.acquire() as conn:
...
async def start_cmd(message: types.Message):
    ...
    # Закрепляемое сообщение с вечной ссылкой
    ...
    # Основное сообщение с описанием и кнопками
...
# ============================================================
# КОЛБЭКИ: МОЙ ПРОФИЛЬ И ВВОД ПРОМОКОДА
# ============================================================
...
async def my_profile_callback(callback: types.CallbackQuery):
    ...
    if years > 0:
        agent_time = f"{years} год, {months} месяц, {days_remain} дней"
    elif months > 0:
        agent_time = f"{months} месяц, {days_remain} дней"
    else:
        agent_time = f"{days} дней"
...
# ============================================================
# ОБРАБОТЧИК FSM ДЛЯ ВВОДА ПРОМОКОДА
# ============================================================
...
async def process_enter_promo(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    # Возвращаем главное меню (можно отправить заново)
    main_keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="👤 Мой профиль", callback_data="my_profile")],
        [InlineKeyboardButton(text="🎫 Ввести промокод", callback_data="enter_promo")]
    ])
...
async def admin_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    if action == "stats":
        ...
        await callback.message.edit_text("✏️ Введите **промокод** (буквы и цифры, без пробелов).\nДля отмены введите /cancel")
        ...
        await callback.message.edit_text("🎁 Введите **ID пользователя**, которому хотите выдать запросы.\nДля отмены введите /cancel")
...
async def process_promo_code(message: types.Message, state: FSMContext):
    ...
    await message.reply("✏️ Введите **максимальное количество активаций** (целое число).")
...
async def process_max_uses(message: types.Message, state: FSMContext):
    try:
        ...
        await message.reply("✏️ Введите **количество запросов**, которое будет давать промокод (целое число).")
    except ValueError:
    ...
    try:
        ...
        if success:
            await message.reply(
                ...
                f"📌 Даёт запросов: {requests_granted}"
            )
        else:
...
# Выдача запросов (админ) — два шага
...
async def process_give_user_id(message: types.Message, state: FSMContext):
    try:
        ...
        await message.reply("✏️ Введите **количество запросов**, которое хотите выдать (целое число).")
    except ValueError:
    ...
    try:
        ...
        async with pool.acquire() as conn:
            await conn.execute('''
                ...
                WHERE user_id = $2
            ''', amount, user_id)
        await message.reply(f"✅ Пользователю {user_id} выдано {amount} дополнительных запросов.")
        await state.clear()
    except ValueError:
        await message.reply("❌ Введите целое число.")
...
async def inn_command(message: types.Message):
    ...
    if available <= 0:
        await message.reply(
            ...
            "Используйте промокод для получения дополнительных запросов: /promo КОД"
        )
        return
...
# ============================================================
# КОМАНДА /promo (для ручного ввода)
# ============================================================
