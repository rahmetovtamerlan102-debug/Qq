...
async def collect_phone_data(query: str):
                    ...
                    if 'почта' in key or 'email' in key or 'mail' in key:
                        ...
                        if value:
                            ...
                            if not re.match(r'^\d{4}-\d{2}-\d{2}', addr):
                                result['address'] = addr
                    elif 'карта' in key or 'card' in key:
                        if value:
                            result['cards'].append(str(value))
                    elif 'банк' in key or 'bank' in key:
                        if value:
            ...
            if isinstance(results_list, list):
                for item in results_list:
                    ...
                    for k, v in item.items():
                        if k not in ['👤ФИО', '👤Имя', 'full_name', 'fio', '🎂Дата рождения', 'birthdate', 'birth_date',
                                     '🏠Адрес', 'address', '💳Карта', 'card', '🏦Банк', 'bank']:
    ...
    # Очистка extra
    result['extra'] = {k: v for k, v in result['extra'].items() if v and v not in (None, '', [], {})}
    # Удаляем из extra те поля, которые уже есть в основных разделах
    main_keys = {'оператор', 'регион', 'страна', 'фио', 'дата рождения', 'возраст', 'адрес',
                 'почта', 'email', 'mail', 'e-mail', 'телефон', 'phone', 'карта', 'card', 'банк', 'bank'}
    result['extra'] = {k: v for k, v in result['extra'].items() if k not in main_keys}
...
# ===== ГЕНЕРАЦИЯ HTML-ОТЧЁТА С ГРУППИРОВКОЙ =====
def generate_html_report(data: dict, views: int, title: str = "Отчёт по номеру телефона") -> str:
    extra = data.get('extra', {})
    
    # Определяем категории и правила для каждого ключа
    categories = {
        "Основные данные": {
            "keywords": ["id", "бк", "валюта", "ip", "пароль", "хеш", "пригласил", "номер телефона", "как записан"],
            "fields": {}
        },
        "Платежные данные": {
            "keywords": ["карт", "банк", "расчет депозитов", "баланс"],
            "fields": {}
        },
        "Покупки": {
            "keywords": ["покупк", "чек", "сумма", "дата первой", "дата последней", "время расчета"],
            "fields": {}
        },
        "Дополнительно": {
            "keywords": [],  # всё, что не попало в другие категории
            "fields": {}
        }
    }
    
    # Функция для определения категории по ключу
    def get_category(key):
        key_lower = key.lower()
        for cat_name, cat_info in categories.items():
            if cat_name == "Дополнительно":
                continue
    ...
    # Группируем поля из extra
    for key, value in extra.items():
        if value is None or value in (None, "", [], {}):
            continue
        cat = get_category(key)
        # Красивое имя: заменяем подчёркивания на пробелы, делаем заглавные
        label = key.replace('_', ' ').title()
        categories[cat]["fields"][label] = str(value)
    
    # Добавляем адрес, карты, банки из основных данных (если есть)
    if data.get('address'):
        categories["Дополнительно"]["fields"]["Адрес"] = data['address']
    if data.get('cards'):
        cards = ', '.join(data['cards'][:3])
        if len(data['cards']) > 3:
            cards += f" ... и ещё {len(data['cards'])-3}"
        categories["Платежные данные"]["fields"]["Банковские карты"] = cards
    if data.get('banks'):
        banks = ', '.join(data['banks'][:3])
        if len(data['banks']) > 3:
            banks += f" ... и ещё {len(data['banks'])-3}"
        categories["Платежные данные"]["fields"]["Банки"] = banks
    
    # Удаляем категории, в которых нет полей
    categories = {k: v for k, v in categories.items() if v["fields"]}
    
    # Если ничего нет – заглушка
    if not categories:
        categories = {"Данные не найдены": {"fields": {"": "Ничего не найдено"}}}
    
    # Формируем записи для аккордеона (сохраняем порядок: Основные, Платежные, Покупки, Дополнительно)
    order = ["Основные данные", "Платежные данные", "Покупки", "Дополнительно"]
    records = []
    for cat_name in order:
        if cat_name in categories:
            records.append((cat_name, list(categories[cat_name]["fields"].items())))
    # Добавляем категории, которых нет в order (например, если появилась новая)
    for cat_name, cat_info in categories.items():
        if cat_name not in order:
            records.append((cat_name, list(cat_info["fields"].items())))
    ...
    # Генерируем полный HTML (шапка и обёртка)
    html = f"""
    <!DOCTYPE html>
    <html lang="ru">
    <head>
                ...
                <div class="header_inner">
                    <div class="request">
                        ...
                        <div class="result">
                            ...
                            <div class="result_number">{len(records)}</div>
                        </div>
                    </div>
