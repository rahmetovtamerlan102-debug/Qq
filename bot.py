...
def normalize_list(values):
    ...
    for v in values:
        if isinstance(v, (str, int, float)):
            ...
            for k in ['value', 'username', 'id', 'email', 'name', 'full_name']:
                if k in v and v[k]:
                    cleaned = clean_value(v[k])
                    if cleaned:
                        result.append(str(cleaned))
                        break
...
def extract_birthdate_from_text(text):
    """Извлекает дату рождения из текста типа 'Nastya Takaya (6.5.1988)' или 'Лёха Лекс (1978-09-21)'"""
    if not text:
        return None
    # Ищем дату в скобках
...
def extract_phone_books(data):
    """Извлекает телефонные книги из данных"""
    books = []
...
async def collect_phone_data(query: str):
    ...
    for src in [nightsearch, seon, snusbase, jitler]:
        ...
        if vk:
            # Извлекаем дату рождения из VK
            if isinstance(vk, dict) and vk.get('name'):
                birth_from_vk = extract_birthdate_from_text(vk.get('name', ''))
                if birth_from_vk and not result['birthdate']:
                    result['birthdate'] = birth_from_vk
            result['vk'] = vk
        ...
        if ok:
            # Извлекаем дату рождения из Одноклассников
            if isinstance(ok, dict) and ok.get('name'):
                birth_from_ok = extract_birthdate_from_text(ok.get('name', ''))
                if birth_from_ok and not result['birthdate']:
                    result['birthdate'] = birth_from_ok
            result['ok'] = ok
...
async def main():
    await init_db()
    print("🚀 Бот запущен (с датой рождения и телефонными книгами)")
