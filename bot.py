    ...
    try:
        ...
        for fmt in ['%d.%m.%Y', '%Y-%m-%d', '%d/%m/%Y', '%Y/%m/%d', '%d-%m-%Y']:
            try:
                ...
                continue
        # Если не распарсили, пробуем через replace
...
async def collect_phone_data(query: str):
    ...
    # ---- 1. ПАРСИНГ BIGBASE (исправленный) ----
    if bigbase:
        ...
        for person in persons:
            ...
            # Дата рождения (все возможные ключи)
            for key in ('birthdate', 'birth_date', 'date_of_birth', 'dob'):
                ...
                for bd_item in birthdate_list:
                    if isinstance(bd_item, dict):
                        ...
                        value = bd_item
                    if value:
                        all_birthdates.append(str(value))
    ...
    for src in [nightsearch, seon, snusbase, jitler]:
        ...
        birthdate = deep_find(src, 'birthdate') or deep_find(src, 'birth_date') or deep_find(src, 'date_of_birth') or deep_find(src, 'dob')
        if birthdate:
...
async def main():
    await init_db()
    print("🚀 Бот запущен (исправленный парсинг BigBase + возраст)")
