...
def clean_value(value):
    # Если value – список, берём первый элемент (если он есть)
...
async def collect_phone_data(query: str):
    ...
    # ---- 2. ПАРСИНГ ДРУГИХ API (кроме телефонных книг) ----
...
async def number_handler(message: types.Message):
    ...
    if is_phone:
        ...
        vk_url = get_social_url(data.get('vk'))
        if vk_url:
            if not vk_url.startswith('http'):
                vk_url = f"https://{vk_url}"
            social_buttons.append(InlineKeyboardButton(text="📘 VK", url=vk_url))
        
        ok_url = get_social_url(data.get('ok'))
        if ok_url:
            if not ok_url.startswith('http'):
                ok_url = f"https://{ok_url}"
            social_buttons.append(InlineKeyboardButton(text="👥 OK", url=ok_url))
        
        tt_url = get_social_url(data.get('tiktok'))
        if tt_url:
            if not tt_url.startswith('http'):
                tt_url = f"https://{tt_url}"
            social_buttons.append(InlineKeyboardButton(text="🎵 TikTok", url=tt_url))
        
        inst_url = get_social_url(data.get('instagram'))
        if inst_url:
            if not inst_url.startswith('http'):
                inst_url = f"https://{inst_url}"
            social_buttons.append(InlineKeyboardButton(text="📷 Instagram", url=inst_url))
        
        if social_buttons:
            for i in range(0, len(social_buttons), 2):
                row = social_buttons[i:i+2]
                if len(row) == 1:
                    row.append(InlineKeyboardButton(text=" ", url="https://t.me"))
...
async def main():
    await init_db()
    print("🚀 Бот запущен (финальная версия, быстрый поиск)")
