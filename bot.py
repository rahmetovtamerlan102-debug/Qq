...
# ⚠️ ЭТОТ ТОКЕН НЕПРАВИЛЬНЫЙ ДЛЯ CRYPTOPAY – ЗАМЕНИТЕ НА ТОКЕН ПРИЛОЖЕНИЯ ИЗ @CryptoBot
...
# ===== СБОР ДАННЫХ (полные функции, я даю только заглушки для краткости, в реальном коде они должны быть полными) =====
async def collect_phone_data(query: str):
    # Полная реализация уже была в предыдущих кодах
    return {}
...
async def create_crypto_pay_invoice(user_id: int, amount_usdt: float, requests_count: int):
    session = await get_http_session()
            ...
            if resp.status == 200:
                ...
                if data.get('ok'):
                    ...
                    async with pool.acquire() as conn:
                        ...
                        ''', user_id, invoice_id, amount_usdt, requests_count)
...
async def check_payment_status(user_id: int, invoice_id: str) -> str:
    session = await get_http_session()
            ...
            if resp.status == 200:
                ...
                if data.get('ok'):
                    ...
                    if invoices and invoices[0].get('status') == 'paid':
                        ...
                        return invoices[0].get('status', 'pending')
            return "error"
    except Exception as e:
...
async def buy_requests_callback(callback: types.CallbackQuery):
    keyboard = InlineKeyboardMarkup(inline_keyboard=[])
    for pkg in PACKAGES:
        callback_data = f"buy_pkg|{pkg['requests']}|{pkg['usd']}"
...
@dp.callback_query(lambda c: c.data and c.data.startswith("buy_pkg|"))
async def buy_package_callback(callback: types.CallbackQuery):
    try:
        _, requests_str, usd_str = callback.data.split("|")
        requests_count = int(requests_str)
        amount_usdt = float(usd_str)
    except Exception:
        await callback.answer("Ошибка, попробуйте снова.")
        return

    user_id = callback.from_user.id
    pay_url, invoice_id = await create_crypto_pay_invoice(user_id, amount_usdt, requests_count)
    
    if not pay_url:
        error_text = invoice_id if invoice_id else "Неизвестная ошибка"
        await callback.message.edit_text(
            f"❌ Ошибка при создании счёта:\n{error_text}\n\n"
            "Проверьте токен CryptoPay и настройки приложения.",
...
@dp.callback_query(lambda c: c.data and c.data.startswith("check_payment|"))
async def check_payment_callback(callback: types.CallbackQuery):
    _, invoice_id = callback.data.split("|")
    user_id = callback.from_user.id
    status = await check_payment_status(user_id, invoice_id)
    if status == "paid":
        await callback.message.edit_text(
            f"✅ Оплата подтверждена! Запросы начислены.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        ...
        await callback.answer("❌ Ошибка проверки платежа. Попробуйте позже или свяжитесь с админом.", show_alert=True)
