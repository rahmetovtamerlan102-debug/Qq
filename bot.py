...
# ===== СБОР ДАННЫХ ДЛЯ ID (BIGBASE + JITLER) =====
async def collect_id_data(query: str, search_type: str):
    ...
    print(f"📊 Результат: телеграм={len(result['telegrams'])}, группы={len(result['groups'])}, история={len(result['username_history'])}, книги={len(result['phone_books'])}")
    return result
...
async def direction_callback(callback: types.CallbackQuery):
    ...
    try:
        data = await asyncio.wait_for(collect_id_data(id_str, search_type), timeout=10.0)
    except asyncio.TimeoutError:
    ...
    if any([data.get('telegrams'), data.get('groups'), data.get('username_history'), data.get('phone_books')]):
...
async def main():
    await init_db()
    print("🚀 Бот запущен (ID: BigBase + Jitler)")
