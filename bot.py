...
# === ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (сокращённо) ===
...
async def depsearch_search(query: str):
    ...
    try:
        async with session.get(url, timeout=aiohttp.ClientTimeout(total=API_TIMEOUTS["depsearch"])) as resp:
            ...
            logger.info(f"DepSearch: HTTP {resp.status}, response: {text[:500]}")
            if resp.status == 200:
...
# === СБОР ДАННЫХ (универсальный, парсит всё) ===
async def collect_general_data(query: str, search_type: str = "phone"):
    ...
    if depsearch:
        ...
        if ip_info:
            if ip_info.get('country') and not result['country']:
                result['country'] = str(ip_info['country'])
                sources_set.add("DepSearch (страна)")
            if ip_info.get('city') and ip_info.get('city') != 'Неизвестно' and not result['city']:
                result['city'] = str(ip_info['city'])
                sources_set.add("DepSearch (город)")
            if ip_info.get('provider') and not result['operator']:
        ...
        if isinstance(results_list, list):
            for idx, item in enumerate(results_list):
                ...
                # Собираем все поля, удаляя эмодзи из ключей
                record_data = {}
