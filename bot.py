...
# ===== СБОР ДАННЫХ ДЛЯ ID (ИСПРАВЛЕННЫЙ) =====
async def collect_id_data(query: str, search_type: str):
    ...
    if bigbase and isinstance(bigbase, dict):
        ...
        for record in records:
            ...
            for i in range(len(base_record)):
                if isinstance(base_record[i], list) and len(base_record[i]) >= 2:
                    key = str(base_record[i][0]).lower()
                    value = base_record[i][1]
                    
                    # Актуальный username
                    if 'актуальный username' in key:
                        if value and value not in result['telegrams']:
                                    ...
                                    for sub in item:
                                        if isinstance(sub, list) and len(sub) >= 2:
                                            ...
                                            if username and username not in result['telegrams']:
                                                result['telegrams'].append(str(username).strip())
                                ...
                                if isinstance(item, list):
                                    for sub in item:
                                        if isinstance(sub, list) and len(sub) >= 2:
                                            name = sub[1]
                                            if name:
                                                result['username_history'].append({'name': str(name)})
                                ...
                                if isinstance(group_item, list):
                                    ...
                                    for group_data in group_item:
                                        if isinstance(group_data, list) and len(group_data) >= 2:
                                            sub_key = str(group_data[0]).lower()
                                            sub_value = group_data[1]
                                            if 'название' in sub_key:
                                                group_name = str(sub_value)
                                            elif 'username' in sub_key:
                                                group_username = str(sub_value)
                                    if group_name and group_username:
                                        result['groups'].append(f"{group_name}: @{group_username}")
                                    elif group_name:
                                        result['groups'].append(group_name)
                    
                    # Интересы (из других полей)
                    elif 'интересы' in key or 'интерес' in key:
                        if isinstance(value, list):
                            for item in value:
                                if isinstance(item, str):
                                    result['interests'].append(item)
                                elif isinstance(item, list) and len(item) >= 2:
                                    result['interests'].append(str(item[1]))
    
    # ---- JITLER ----
    if jitler and isinstance(jitler, dict):
        jitler_data = jitler.get('response', jitler)
        
        # Телефонные книги (сохраняем как интересы)
        phonebooks = jitler_data.get('phonebooks', [])
        if phonebooks:
            for book in phonebooks[:10]:
                result['interests'].append(f"Телефонная книга: {book}")
    ...
    result['interests'] = list(dict.fromkeys([x for x in result['interests'] if x]))
    result['gift_ids'] = list(dict.fromkeys([x for x in result['gift_ids'] if x]))
...
async def main():
    await init_db()
    print("🚀 Бот запущен (исправлен поиск по ID)")
