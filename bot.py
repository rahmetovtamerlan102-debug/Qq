...
async def init_db():
    ...
    ''')
    # Для ID-отчётов ключ будет id + направление, например "123456:inn"
    await conn.execute('''
...
async def collect_data(query: str, search_type: str, is_phone: bool = False):
    ...
    # Логирование сырых ответов (для отладки, можно убрать после настройки)
    print("\n=== BIGBASE ===")
    ...
    # ---- Сбор ФИО, даты рождения, возраста, соцсетей, банков, email (для телефона и ID) ----
    fio = None
    birthdate = None
    age = None
    socials = {}
...
async def direction_callback(callback: types.CallbackQuery):
    ...
    # Формируем ключ кэша с учётом направления
    ...
    # Проверяем, есть ли реальные данные, чтобы сохранять только непустые
    has_data = any([
...
async def number_handler(message: types.Message):
    ...
    if is_phone:
        ...
        merged = data.get('merged', {})
        # Сохраняем только если есть данные (не пусто)
        has_data = any([
            merged.get('operator'),
            merged.get('region'),
            merged.get('country'),
        ...
        # ID – показываем выбор направления
...
async def main():
    await init_db()
    print("🚀 Бот запущен (исправленный, с полным сбором данных и кэшем по направлениям)")
