...
# ===== СБОР ДАННЫХ (BigBase только для доп. полей) =====
async def collect_phone_data(query: str):
    ...
    # ---- 1. ПАРСИНГ DEPSEARCH (ОСНОВНОЙ ДЛЯ ФИО, ДАТЫ, ОПЕРАТОРА, РЕГИОНА, СТРАНЫ) ----
        ...
        if isinstance(results_list, list):
            for item in results_list:
                if isinstance(item, dict):
                    ...
                    # Email из DepSearch тоже собираем
    ...
    # ---- 2. ДРУГИЕ API ДЛЯ ДОПОЛНИТЕЛЬНЫХ ДАННЫХ (BIGBASE, NIGHTSEARCH, SEON, SNUSBASE, JITLER) ----
    # Собираем email, telegram, соцсети из всех источников (кроме DepSearch, но DepSearch уже обработан)
    ...
    # ---- ТЕЛЕФОННЫЕ КНИГИ (только из JITLER и BIGBASE) ----
    phone_books = []
...
async def main():
    await init_db()
    print("🚀 Бот запущен (BigBase только для доп. данных)")
