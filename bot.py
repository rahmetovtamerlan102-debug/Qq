...
# ===== СБОР ДАННЫХ (только BigBase + JITLER) =====
async def collect_data(query: str, search_type: str, is_phone: bool = False):
    ...
    if not is_phone:
        # Извлекаем данные из BigBase и JITLER
        # BigBase может дать telegram_id, username, группы и историю
        if isinstance(bigbase, dict):
            if not telegram_id:
                telegram_id = bigbase.get('telegram_id') or bigbase.get('tg_id') or bigbase.get('id')
            if not reg_date:
                reg_date = bigbase.get('registration_date') or bigbase.get('reg_date') or bigbase.get('date')
            if not username:
                username = bigbase.get('username') or bigbase.get('nick')
            if not groups:
                groups = bigbase.get('groups') or bigbase.get('chats') or []
            if not interests:
                interests = bigbase.get('interests') or []
            # История
            hist = bigbase.get('username_history') or bigbase.get('history') or bigbase.get('name_history')
            if hist:
                    ...
                    for line in hist.split('\n'):
                        if line.strip():
                            ...
                            if len(parts) >= 2:
                                ...
                                history.append({'date': date_part, 'nick': nick, 'name': name, 'id': uid})
            # Подарки (gift_ids)
            gft = bigbase.get('gifts') or bigbase.get('presents')
            if gft:
        ...
        # Если BigBase не дал данные, можем взять из JITLER
        # JITLER может дать только телеграм-аккаунты
        # Но у нас уже есть telegram_raw, который мы парсим отдельно
    ...
    # Парсим телеграм-аккаунты из JITLER
...
def format_phone_report(merged: dict, full_data: dict, phone: str, views: int) -> str:
    ...
    age = None
    # Ищем в BigBase
    bigbase = full_data.get('bigbase', {})
    if bigbase:
        if isinstance(bigbase, dict):
            fio = bigbase.get('full_name') or bigbase.get('fio') or bigbase.get('name') or bigbase.get('ФИО')
            birthdate = bigbase.get('birthdate') or bigbase.get('birth_date') or bigbase.get('date_of_birth') or bigbase.get('дата рождения')
            age = bigbase.get('age')
        elif isinstance(bigbase, list) and bigbase:
            item = bigbase[0] if bigbase else {}
            if isinstance(item, dict):
    ...
    banks = []
    emails = []
    # Ищем в BigBase и JITLER
    for resp in [bigbase, full_data.get('jitler', {})]:
        if not resp:
            continue
...
# ===== ОБРАБОТЧИК ДЛЯ ЧИСЕЛ (с чётким разделением) =====
...
async def main():
    await init_db()
    print("🚀 Бот запущен (оптимизированный поиск, инлайн-кнопки)")
