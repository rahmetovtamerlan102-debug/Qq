...
CRYPTOPAY_BASE = "https://pay.crypt.bot"

# === ПРОКСИ (вшито) ===
PROXY_URL = "http://203.202.245.62:80"  # Можно менять на другой
# PROXY_URL = "http://103.200.113.40:8080"
# PROXY_URL = "http://103.213.186.226:8080"
...
async def parse_html_page(url: str, query: str = None):
    """Парсинг HTML страницы с поиском информации"""
    ...
    try:
        async with session.get(
            ...
            if response.status != 200:
                logger.warning(f"HTML parse error: HTTP {response.status}")
                return {'error': f'HTTP {response.status}'}
            
            # Проверяем на Cloudflare
            if 'Just a moment' in html or 'cf-browser-verification' in html:
                logger.warning("Cloudflare detected on HTML page")
            ...
            for a in soup.find_all('a', href=True):
                ...
                if href and not href.startswith('#') and not href.startswith('javascript'):
                    ...
                    result['links'].append({
                        'url': href,
                        'text': a.get_text(strip=True) or 'ссылка'
                    })
            ...
            # Поиск телефонов
            phones = re.findall(r'\+?\d[\d\-\(\)\s]{7,15}', text_content)
            ...
            for social, pattern in social_patterns.items():
                ...
                if matches:
                    result['socials'].extend(matches[:3])
            
            # Поиск по запросу (если указан)
            if query:
                ...
                for p in result['paragraphs']:
                    if query_lower in p.lower():
                        found_items.append({
                            'type': 'paragraph',
                            'content': p[:200] + '...' if len(p) > 200 else p
                        })
                ...
                result['found_data'] = found_items[:20]
            
            # Собираем основной текст
            result['full_text'] = soup.get_text(strip=True)[:5000]
            
            return result
            
    except asyncio.TimeoutError:
        logger.error("HTML parse timeout")
        return {'error': 'Timeout'}
    except Exception as e:
        logger.error(f"HTML parse error: {e}")
        return {'error': str(e)}

# === УНИВЕРСАЛЬНЫЙ ПАРСЕР DEPSEARCH ===
...
async def depsearch_search(query: str):
    """DepSearch API с прокси"""
    if not DEPSEARCH_TOKEN:
        logger.warning("DEPSEARCH_TOKEN не задан")
        return {}
    ...
    logger.info("DepSearch request: quest=%s", query)
    ...
    try:
        async with session.get(
            ...
            logger.info(
                "DepSearch HTTP=%s content_type=%s length=%s",
                response.status,
            ...
            if response.status != 200:
                logger.warning(
                    "DepSearch error body (first 1000 chars): %s",
                    text[:1000].replace("\n", " ")
                )
                return {}
            ...
            try:
                ...
                logger.error("DepSearch вернул невалидный JSON: %s", text[:500])
                return {}

            if not isinstance(data, dict):
                logger.warning("DepSearch response is not a dict")
                return {}

            results = data.get("results", [])

            logger.info("DepSearch keys: %s", list(data.keys()))
            logger.info("DepSearch results: %s", len(results) if isinstance(results, list) else 0)
        ...
        logger.warning("DepSearch timeout")
        return {}

    except aiohttp.ClientError as e:
        logger.error("DepSearch client error: %s", e)
        return {}

    except Exception as e:
        logger.exception("DepSearch error: %s", e)
        return {}
...
async def nightsearch_search(query: str):
    if not NIGHTSEARCH_API_KEY:
        ...
        return {}
    session = await get_http_session()
    url = f"{NIGHTSEARCH_BASE}/api/search"
    ...
    try:
        async with session.post(url, json=payload, headers=headers,
            ...
            if resp.status == 200:
                ...
                logger.info(f"NightSearch: found data")
                return data
            return {}
    except Exception as e:
...
async def seon_search(query: str):
    if not SEON_API_KEY:
        logger.warning("SEON_API_KEY не задан")
        return {}
    session = await get_http_session()
...
async def snusbase_search(query: str):
    if not SNUSBASE_API_KEY:
        logger.warning("SNUSBASE_API_KEY не задан")
        return {}
    session = await get_http_session()
    ...
    try:
        async with session.post(url, json=payload, headers=headers,
            ...
            if resp.status == 200:
                data = await resp.json()
                logger.info(f"Snusbase: found data")
                return data
            return {}
    except Exception as e:
    ...
    for item in items:
        if isinstance(item, dict):
            ...
            key_mapping = {
                ...
                'birthdate': 'Дата рождения', 'birth_date': 'Дата рождения', 'date_of_birth': 'Дата рождения',
                'age': 'Возраст', 'gender': 'Пол', 'operator': 'Оператор', 'carrier': 'Оператор',
                'source': 'Источник', 'database': 'База данных', 'username': 'Имя пользователя',
                'nickname': 'Никнейм', 'telegram': 'Telegram', 'vk': 'ВКонтакте',
                'instagram': 'Instagram', 'tiktok': 'TikTok', 'ok': 'Одноклассники',
            }
            for key, value in item.items():
                if value not in (None, "", [], {}):
                    russian_key = key_mapping.get(key, key)
                    fields[russian_key] = value
            if fields:
...
def parse_seon(data):
    ...
    if phone_info:
        ...
        key_mapping = {'phone': 'Телефон', 'country': 'Страна', 'carrier': 'Оператор',
                      'is_valid': 'Валидный', 'is_active': 'Активный', 'is_roaming': 'Роуминг'}
        for key, value in phone_info.items():
            if value:
                russian_key = key_mapping.get(key, key)
                russian_info[russian_key] = value
        if russian_info:
            parsed.append({"type": "phone_info", "data": russian_info})
    ...
    if data.get('social_media'):
        ...
        for platform, info in data['social_media'].items():
            social_data[f"{platform.title()}"] = info
        if social_data:
            parsed.append({"type": "social", "data": social_data})
    ...
    for item in results:
        if isinstance(item, dict):
            ...
            key_mapping = {
                ...
                'full_name': 'ФИО', 'first_name': 'Имя', 'last_name': 'Фамилия',
                'city': 'Город', 'state': 'Регион', 'country': 'Страна',
                'zip': 'Индекс', 'ip': 'IP адрес', 'date': 'Дата', 'source': 'Источник',
            }
            for key, value in item.items():
                if value not in (None, "", [], {}):
                    russian_key = key_mapping.get(key, key)
...
def parse_jitler(data):
    ...
    parsed = []
    jitler_data = data.get('response', data)
    
    phonebooks = jitler_data.get('phonebooks', [])
    for pb in phonebooks:
        if isinstance(pb, dict):
            fields = {}
            key_mapping = {'name': 'Имя', 'first_name': 'Имя', 'last_name': 'Фамилия',
                          'phone': 'Телефон', 'phone_number': 'Телефон', 'address': 'Адрес',
            ...
            for key, value in pb.items():
                if value not in (None, "", [], {}):
                    ...
                    fields[russian_key] = value
            if fields:
                parsed.append({"type": "phonebook", "data": fields})
    
    profiles = jitler_data.get('profiles', {})
    social_names = {'vk': 'ВКонтакте', 'ok': 'Одноклассники', 'instagram': 'Instagram',
                    'tiktok': 'TikTok', 'telegram': 'Telegram', 'facebook': 'Facebook',
                    'twitter': 'Twitter', 'youtube': 'YouTube'}
    
    for social, items in profiles.items():
                ...
                if isinstance(item, dict):
                    ...
                    for key, value in item.items():
                        if value not in (None, "", [], {}):
                            fields[key] = value
                    if fields:
                        parsed.append({"type": "profile", "social": social, 
                                      "social_name": social_names.get(social, social), "data": fields})
...
async def collect_general_data(query: str, search_type: str = "phone"):
    ...
    for name, task in tasks.items():
        try:
            timeout = 25.0 if name == "depsearch" else 12.0
            results[name] = await asyncio.wait_for(task, timeout=timeout)
        except asyncio.TimeoutError:
            results[name] = {}
            task.cancel()
            logger.warning(f"Timeout for {name} after {timeout}s")
        except Exception as e:
    ...
    logger.info(f"DepSearch RAW data: {depsearch}")
    
    dep_report = depsearch_to_report(depsearch)
    logger.info(f"DepSearch REPORT: {dep_report}")
    
    dep_results = dep_report.get("results", [])
    dep_phone_info = dep_report.get("phone_info", {})
    
    logger.info(f"DepSearch: найдено записей: {len(dep_results)}")
    if dep_results:
        logger.info(f"DepSearch первая запись: {dep_results[0]}")
    ...
    if ipdata:
        ...
        if extra_ip:
            records_count += 1
            result['extra'][f"IP информация"] = {
                'source': 'ip-api.com',
                'data': extra_ip
            }
            sources_set.add("ip-api.com")

    # ---- DepSearch (универсальный вывод) ----
    if dep_phone_info:
    ...
    for idx, item in enumerate(dep_results, 1):
        ...
        if record_key not in seen_records:
            ...
            records_count += 1
            result['extra'][f"Запись #{records_count} (DepSearch)"] = {
                'source': source_name,
                'data': item
            }
    ...
    for block in night_parsed:
        if block['type'] == 'result':
            ...
            if record_key not in seen_records:
                seen_records.add(record_key)
                source_name = fields.get('Источник', fields.get('База данных', 'NightSearch'))
                source_name = re.sub(r'[^\w\s\-\.]', '', source_name).strip()
                if not source_name:
                    source_name = 'NightSearch'
                sources_set.add(source_name)
                records_count += 1
                result['extra'][f"Запись #{records_count} (NightSearch)"] = {
                    'source': source_name,
                    'data': fields
                }
    ...
    for block in seon_parsed:
        if block['type'] == 'phone_info':
            ...
            if info.get('Страна'):
                ...
                sources_set.add("SEON")
            records_count += 1
            result['extra'][f"Информация о номере (SEON)"] = {
                'source': 'SEON',
                'data': info
            }
            ...
            fields = block['data']
            if fields.get('Email'):
                result['emails'].append(str(fields['Email']))
                sources_set.add("SEON")
    ...
    for block in snusbase_parsed:
        if block['type'] == 'result':
            ...
            if record_key not in seen_records:
                seen_records.add(record_key)
                source_name = fields.get('Источник', 'Snusbase')
                sources_set.add("Snusbase")
                records_count += 1
                result['extra'][f"Запись #{records_count} (Snusbase)"] = {
                    'source': 'Snusbase',
                    'data': fields
                }
    ...
    for block in jitler_parsed:
        if block['type'] == 'phonebook':
            fields = block['data']
            record_key = "|".join(str(v) for v in fields.values())
            if record_key not in seen_records:
                seen_records.add(record_key)
                records_count += 1
                result['extra'][f"Запись #{records_count} (Jitler)"] = {
                    'source': 'Jitler (телефонная книга)',
                    'data': fields
                }
                sources_set.add("Jitler")
                if 'Телефон' in fields:
                    result['phone_books'].append(str(fields['Телефон']))
                if 'Имя' in fields and not result['fio']:
                    result['fio'] = str(fields['Имя'])
        
        elif block['type'] == 'profile':
            fields = block['data']
            social_name = block.get('social_name', '')
            record_key = f"{social_name}:{fields.get('url', fields.get('id', ''))}"
            if record_key not in seen_records:
                seen_records.add(record_key)
                records_count += 1
                sources_set.add("Jitler")
                ...
                for key, value in fields.items():
                    if key in ['url', 'link']:
                        profile_data['Ссылка'] = value
                    elif key in ['name', 'username']:
                        profile_data['Имя'] = value
                    else:
                        profile_data[key] = value
                
                result['extra'][f"Профиль {social_name} #{records_count}"] = {
                    'source': f'Jitler ({social_name})',
                    'data': profile_data
                }
    ...
    logger.info(f"Final records_count: {records_count}, sources: {len(sources_set)}")
    logger.info(f"Final extra keys: {list(result['extra'].keys())}")
...
def generate_html_report(data: dict, views: int = 0) -> str:
    ...
    if data.get('phone_books'):
        main_info.append(("Телефоны", ', '.join(data['phone_books'][:10])))
    if data.get('emails'):
        main_info.append(("Email", ', '.join(data['emails'][:5])))
    if data.get('vk'):
        main_info.append(("ВКонтакте", data['vk']))
    if data.get('ok'):
    ...
    for idx, (key, rec) in enumerate(records.items(), start=1):
        ...
        for label, value in fields.items():
            if value and str(value).strip():
                rows_html += f'<div class="row"><strong>{label}:</strong><span>{value}</span></div>'
        accordions += f"""
        ...
        <div class="accordion_inner">
            <div class="accordion open">
                ...
                <div class="accordion-body">
                    <div class="accordion-content">
                        <div class="row">
                            <span>По вашему запросу ничего не найдено. Попробуйте другой номер.</span>
                        </div>
                    </div>
                </div>
            </div>
...
<head>
    ...
    <style>
        ...
        *, ::before, ::after {{ box-sizing: border-box; }}
        ...
        .accordion-content .row strong {{ font-weight: 500; font-size: 16px; line-height: 20px; color: #fff; }}
        .accordion-content .row span {{ font-weight: 600; font-size: 16px; line-height: 20px; color: #ff851f; width: 60%; text-align: right; }}
                ...
                <div class="block2 no_transform" id="printArea">
                    ...
                    <div id="main" class="accordion_inner">
                        <div class="accordion open">
                            ...
                            <div class="accordion-body">
                                <div class="accordion-content">
                                    {''.join(f'<div class="row"><strong>{label.upper()}:</strong><span>{value}</span></div>' for label, value in main_info) if main_info else '<div class="row"><span style="width:100%;text-align:center;">Нет данных</span></div>'}
                                </div>
                            </div>
                        </div>
                    </div>
                    {accordions if records else no_data_message}
                </div>
            </div>
        </div>
    </div>
...
# === НОВАЯ КОМАНДА ДЛЯ ПАРСИНГА HTML ===
@dp.message(Command("parse"))
async def parse_html_command(message: Message):
    """Парсинг HTML страницы"""
    if message.from_user.id not in ADMIN_IDS:
        await message.reply("❌ Доступ запрещён. Только для админов.")
        return
    ...
    if len(args) < 2:
        await message.reply(
            ...
            "С поиском: `/parse https://example.com текст`",
            parse_mode="Markdown"
        )
        return
    ...
    status_msg = await message.reply(f"🔍 Парсим страницу...")
    ...
    try:
        ...
        if result.get('description'):
            text += f"📝 Описание: {result['description'][:200]}...\n\n"
        
        # Статистика
        text += f"📊 **Статистика:**\n"
        text += f"  • Заголовков: {len(result.get('headers', []))}\n"
        text += f"  • Параграфов: {len(result.get('paragraphs', []))}\n"
        text += f"  • Ссылок: {len(result.get('links', []))}\n"
        text += f"  • Изображений: {len(result.get('images', []))}\n"
        text += f"  • Таблиц: {len(result.get('tables', []))}\n"
        text += f"  • Списков: {len(result.get('lists', []))}\n\n"
        ...
        if emails or phones or socials:
            text += f"📱 **Найденные контакты:**\n"
            if emails:
                text += f"  • Email: {', '.join(emails[:5])}\n"
            if phones:
                text += f"  • Телефоны: {', '.join(phones[:5])}\n"
            if socials:
                text += f"  • Соцсети: {', '.join(socials[:3])}\n"
            text += "\n"
        
        # Если есть поиск по запросу
        if query and result.get('found_data'):
            text += f"🔍 **Найдено по запросу '{query}':**\n"
            for item in result['found_data'][:10]:
                text += f"  • {item['type']}: {item['content'][:100]}...\n"
            text += "\n"
        
        # Заголовки
        if result.get('headers'):
            text += f"📌 **Заголовки:**\n"
            for h in result['headers'][:5]:
                text += f"  • {h}\n"
            text += "\n"
        ...
        text += f"📎 **Всего найдено:** {len(result.get('text', []))} элементов"
        
        # Сохраняем результат в файл
        html_file = BufferedInputFile(
            result.get('full_text', '').encode('utf-8'),
            filename=f"page_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        )
        
        await status_msg.edit_text(text[:4000])
        await message.reply_document(html_file, caption="📄 Полный текст страницы")
...
async def start_cmd(message: Message):
    ...
    main_text = (
        ...
        "📎 Дополнительно:\n"
        "└ /parse URL [запрос] — парсинг HTML страницы"
    )
...
@dp.message(Command("testdep"))
async def test_depsearch(message: Message):
    if message.from_user.id not in ADMIN_IDS:
    ...
    args = message.text.split()
    if len(args) < 2:
        await message.reply("Укажите номер: /testdep 77083706944")
        return
    
    phone = args[1]
    
    status_msg = await message.reply(f"🔍 Тестируем DepSearch для {phone}...")
    
    try:
        result = await depsearch_search(phone)
        
        if result and isinstance(result, dict):
            text = f"✅ DepSearch ответил!\n"
            text += f"📊 Результатов: {len(result.get('results', []))}\n"
            text += f"📱 Phone info: {'есть' if result.get('phone_info') else 'нет'}\n"
            text += f"🔑 Ключи: {', '.join(result.keys())}\n\n"
            
            if result.get('phone_info'):
                text += "📝 Информация о номере:\n"
                for key, value in result['phone_info'].items():
                    text += f"  {key}: {value}\n"
            ...
            await status_msg.edit_text("❌ DepSearch не вернул данных")
