...
async def collect_id_data(query: str, search_type: str):
    ...
    for gift in telelog_gifts:
        if isinstance(gift, dict):
            ...
            if gift.get('from_user_id') == int(query):
                gift_str = f"📤 {date_str} → {to_user}"
            elif gift.get('to_user_id') == int(query):
    ...
    # История юзернеймов — убираем дубликаты
    seen_usernames = set()
...
async def main():
    await init_db()
    print("🚀 Бот запущен (TeleLog: история имён, юзернеймов, подарки)")
