...
CRYPTOPAY_TOKEN = "626294:AAHhNmLsqJsYAqBWIEK52MZrJTk8B1Z27vy"  # Новый токен
        ...
        async with session.post(url, json=payload, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as resp:
            ...
            if resp.status == 200:
                ...
                if data.get('ok'):
                    ...
                    error_msg = data.get('error', {}).get('message', 'Неизвестная ошибка CryptoPay')
                    return None, error_msg
            else:
                return None, f"HTTP {resp.status}: {response_text[:100]}"
    except Exception as e:
        return None, str(e)
