...
# === КРИПТОТОКЕН ВСТАВЛЕН ПРЯМО В КОД ===
...
async def start_cmd(message: Message):
    ...
    # === 1. ОТПРАВЛЯЕМ "ВЕЧНАЯ ССЫЛКА" И ЗАКРЕПЛЯЕМ ===
    ...
    # === 2. ОТПРАВЛЯЕМ ОСНОВНОЕ ПРИВЕТСТВИЕ ===
    ...
    await message.reply(main_text, parse_mode="Markdown", reply_markup=main_keyboard)
...
async def pay_usdt_callback(callback: CallbackQuery):
    ...
    if not pay_url:
        ...
        await callback.answer()
        return
    # invoice_id определён
    invoice_id = pay_url.split('invoice_id=')[-1] if 'invoice_id=' in pay_url else None
    if not invoice_id:
        # если не удалось извлечь, используем заглушку, но обычно CryptoPay возвращает invoice_id отдельно
        # но мы уже сохранили в БД, можно получить его оттуда, но проще использовать переданный из create_crypto_pay_invoice
        # изменим create_crypto_pay_invoice, чтобы возвращал invoice_id
        # временно: получим из БД
        pool = await get_pool()
        async with pool.acquire() as conn:
            row = await conn.fetchrow('SELECT invoice_id FROM purchases WHERE user_id=$1 ORDER BY created_at DESC LIMIT 1', user_id)
