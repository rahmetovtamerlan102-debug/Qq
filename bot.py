...
async def collect_phone_data(query: str):
    ...
    if bigbase and isinstance(bigbase, dict):
        # Оператор, регион, страна из dossier.head
        ...
        # ФИО и дата рождения из connections.person
        ...
        # ---- EMAIL ИЗ BIGBASE (ГЛУБОКИЙ ПОИСК) ----
        # Из records
        ...
        for conn in bigbase.get('connections', {}).get('person', []):
            # email может быть вложен в другие поля
    ...
    # ---- 2. FALLBACK НА DEPSEARCH (ЕСЛИ BIGBASE НЕ ДАЛ ОСНОВНЫЕ ДАННЫЕ) ----
    ...
    # ---- 3. СОБИРАЕМ EMAIL ИЗ ДРУГИХ API (КРОМЕ BIGBASE, УЖЕ СОБРАЛИ) ----
    for src in [nightsearch, seon, snusbase, jitler, depsearch]:
        ...
        # Дополнительно из DepSearch по ключу "✉️Почта"
    ...
    # Преобразуем соцсети в чистый URL
...
async def main():
    await init_db()
    print("🚀 Бот запущен (BigBase основной, email из BigBase)")
