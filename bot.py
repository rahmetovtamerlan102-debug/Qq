...
async def telelog_get_stats_min(user_id: str):
    """Получает базовую статистику из TeleLog"""
    if not TELELOG_TOKEN:
        return {}
    ...
    try:
        ...
        print(f"❌ TeleLog stats_min error: {repr(e)}")
        return {}
...
async def collect_id_data(query: str, search_type: str):
    ...
    telelog_stats = results[5] if isinstance(results[5], dict) else {}
    ...
    for name_entry in telelog_names:
        if isinstance(name_entry, dict):
            date = name_entry.get('date') or name_entry.get('created_at') or name_entry.get('updated_at')
    ...
    for gift in telelog_gifts:
        if isinstance(gift, dict):
            # Может быть много разных полей
            gift_id = gift.get('id') or gift.get('gift_id') or gift.get('from_id') or gift.get('to_id')
            if gift_id:
                result['gifts'].append(str(gift_id))
            # Или может быть объект с from/to
            from_id = gift.get('from_id')
            to_id = gift.get('to_id')
            if from_id:
                result['gifts'].append(f"От: {from_id}")
            if to_id:
                result['gifts'].append(f"Кому: {to_id}")
