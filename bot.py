...
async def collect_phone_data(query: str):
    ...
    if bigbase and isinstance(bigbase, dict):
        # ===== УНИВЕРСАЛЬНЫЙ ПАРСЕР: ПРОВЕРЯЕМ ОБА ФОРМАТА =====
        # Формат 1: с dossier
        dossier = bigbase.get('dossier', {})
        # Формат 2: без dossier (прямо в корне)
            ...
            for record in records_list:
                ...
                for item in base_record:
                    if isinstance(item, list) and len(item) >= 2:
                        ...
                        value = item[1]
                        if 'оператор' in key and not operator:
                            operator = str(value)
                        elif 'регион' in key and not region:
                            region = str(value)
                        elif 'страна' in key and not country_info:
