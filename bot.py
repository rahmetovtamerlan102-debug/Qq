...
async def get_cached_or_fetch(query: str, fetch_func, func_name: str = None):
    if func_name is None:
        func_name = fetch_func.__name__
    cache_key = get_cache_key(func_name, query)
...
async def jitler_search_with_balancer(query: str, search_type: str = "number"):
    session = await get_http_session()
    for attempt in range(2):  # Уменьшено количество попыток
            ...
            async with session.post(url, json=payload, headers=headers, 
                ...
                if resp.status == 200:
                    ...
                    if data.get('result'):
                        if 'id' in data:
                            task_id = data['id']
                            for _ in range(2):  # Уменьшено количество попыток
                                await asyncio.sleep(0.2)  # Уменьшена задержка
                                try:
...
# ===== СБОР ДАННЫХ (ОПТИМИЗИРОВАННЫЙ) =====
async def collect_phone_data(query: str, progress_callback=None):
    # Проверяем кэш
    ...
    # Запускаем все API параллельно с разными таймаутами
    tasks = {
    ...
    for name, task in tasks.items():
        try:
            results[name] = await asyncio.wait_for(task, timeout=2.5)
            completed += 1
            if progress_callback:
                await progress_callback(completed, total, name)
        except asyncio.TimeoutError:
            results[name] = {}
            task.cancel()
            completed += 1
            if progress_callback:
                await progress_callback(completed, total, f"{name} (таймаут)")
    
    # Извлекаем результаты
    bigbase = results.get('bigbase', {})
    nightsearch = results.get('nightsearch', {})
    seon = results.get('seon', {})
    snusbase = results.get('snusbase', {})
    depsearch = results.get('depsearch', {})
    jitler = results.get('jitler', {})
...
async def collect_email_data(query: str):
    cache_key = get_cache_key("email", query)
    if cache_key in cache:
        cached_time, data = cache[cache_key]
        if datetime.now() - cached_time < CACHE_TTL:
            return data
...
async def collect_ip_data(query: str):
    cache_key = get_cache_key("ip", query)
    if cache_key in cache:
        cached_time, data = cache[cache_key]
        if datetime.now() - cached_time < CACHE_TTL:
            return data
...
async def collect_vk_data(query: str):
    cache_key = get_cache_key("vk", query)
    if cache_key in cache:
        cached_time, data = cache[cache_key]
        if datetime.now() - cached_time < CACHE_TTL:
            return data
...
async def collect_inn_data(query: str):
    cache_key = get_cache_key("inn", query)
    if cache_key in cache:
        cached_time, data = cache[cache_key]
        if datetime.now() - cached_time < CACHE_TTL:
            return data
...
async def collect_fio_data(query: str):
    cache_key = get_cache_key("fio", query)
    if cache_key in cache:
        cached_time, data = cache[cache_key]
        if datetime.now() - cached_time < CACHE_TTL:
            return data
...
async def universal_handler(message: Message):
    ...
    async def update_progress(completed, total, name):
        try:
            await status_msg.edit_text(
                f"⠛ Поиск по {search_type}:\n\n"
                f"💬 Поиск: {query}\n"
                f"⏱️ Сбор информации... {completed}/{total}\n"
                f"✅ Готово: {name}"
            )
        except Exception:
            pass
    ...
    if search_type == "phone":
        data = await collect_phone_data(query, update_progress)
