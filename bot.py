...
# ===== СБОР ДАННЫХ ДЛЯ ID (БЫСТРЫЙ И БЕЗ EMAIL) =====
async def collect_id_data(query: str, search_type: str):
    ...
    if bigbase and isinstance(bigbase, dict):
        ...
        for g in groups:
            if isinstance(g, dict):
                ...
                result['groups'].append(str(g))
        
        # Интересы
        result['interests'].extend(deep_find_all(bigbase, 'interests'))
        
        # История имени
        history = deep_find_all(bigbase, 'username_history') + deep_find_all(bigbase, 'history') + deep_find_all(bigbase, 'name_history')
        for entry in history:
            if isinstance(entry, dict):
                # Проверяем, есть ли дата и ник/имя
                if entry.get('date') and (entry.get('nick') or entry.get('username') or entry.get('name') or entry.get('id')):
                    result['username_history'].append(entry)
            else:
                result['username_history'].append(entry)
        
        # Подарки
        result['gift_ids'].extend(deep_find_all(bigbase, 'gifts') + deep_find_all(bigbase, 'presents'))
        
        # Соцсети
        vk = deep_find(bigbase, 'vk') or deep_find(bigbase, 'vkontakte')
        if vk:
            result['vk'] = get_social_url(vk)
        inst = deep_find(bigbase, 'instagram')
        if inst:
            result['instagram'] = get_social_url(inst)
        tt = deep_find(bigbase, 'tiktok')
        if tt:
            result['tiktok'] = get_social_url(tt)
        ok = deep_find(bigbase, 'ok') or deep_find(bigbase, 'odnoklassniki')
        if ok:
            result['ok'] = get_social_url(ok)
    ...
    if jitler and isinstance(jitler, dict):
        ...
        # Если есть raw — парсим ссылки
        raw = jitler_data.get('raw', '')
        if raw:
            # VK
            vk_match = re.search(r'https://vk\.com/[^\s<>"]+', raw)
            if vk_match and not result['vk']:
                result['vk'] = vk_match.group(0)
            
            # OK
            ok_match = re.search(r'https://ok\.ru/profile/[^\s<>"]+', raw)
            if ok_match and not result['ok']:
                result['ok'] = ok_match.group(0)
            
            # Instagram
            insta_match = re.search(r'https://instagram\.com/[^\s<>"]+', raw)
            if insta_match and not result['instagram']:
                result['instagram'] = insta_match.group(0)
            
            # TikTok
            tt_match = re.search(r'https://tiktok\.com/[^\s<>"]+', raw)
            if tt_match and not result['tiktok']:
                result['tiktok'] = tt_match.group(0)
            
            # Telegram (@username)
            tg_mentions = re.findall(r'@(\w+)', raw)
            for username in tg_mentions:
                tg_str = f"@{username}"
                if tg_str not in result['telegrams']:
                    result['telegrams'].append(tg_str)
            
            tg_links = re.findall(r't\.me/(\w+)', raw)
            for username in tg_links:
                tg_str = f"@{username}"
                if tg_str not in result['telegrams']:
                    result['telegrams'].append(tg_str)
        
        # Если есть явное поле telegram
        telegrams = jitler_data.get('telegram', [])
        for tg in telegrams:
            formatted = extract_telegram(tg)
        ...
        # Группы
        groups = deep_find_all(jitler_data, 'groups') + deep_find_all(jitler_data, 'chats')
        for g in groups:
            if isinstance(g, dict):
                name = g.get('name') or g.get('title')
                if name:
        ...
        # Интересы
        result['interests'].extend(deep_find_all(jitler_data, 'interests'))
        
        # История имени
        history = deep_find_all(jitler_data, 'username_history') + deep_find_all(jitler_data, 'history') + deep_find_all(jitler_data, 'name_history')
        for entry in history:
            if isinstance(entry, dict):
                if entry.get('date') and (entry.get('nick') or entry.get('username') or entry.get('name') or entry.get('id')):
                    result['username_history'].append(entry)
        ...
        # Подарки
        result['gift_ids'].extend(deep_find_all(jitler_data, 'gifts') + deep_find_all(jitler_data, 'presents'))
    
    # Нормализация
    result['telegrams'] = list(dict.fromkeys(result['telegrams']))
    result['groups'] = list(dict.fromkeys(result['groups']))
    result['interests'] = list(dict.fromkeys(result['interests']))
    result['gift_ids'] = list(dict.fromkeys(result['gift_ids']))
...
def format_id_report(data: dict, views: int) -> str:
    ...
    if data.get('interests'):
        lines.append(f"\n🧠 Интересы [{len(data['interests'])}]:")
        for interest in data['interests'][:10]:
            lines.append(str(interest))
    
    # Подарки
    if data.get('gift_ids'):
        lines.append(f"\n🎁 Подарочные связи: {', '.join(data['gift_ids'][:20])}")
...
async def main():
    await init_db()
    print("🚀 Бот запущен (поиск по ID: история, группы, интересы, подарки)")
