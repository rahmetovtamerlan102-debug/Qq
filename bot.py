...
# ===== УНИВЕРСАЛЬНЫЙ ПОИСК СОЦСЕТЕЙ (по ключам и ссылкам) =====
def extract_socials(data):
    """Рекурсивно ищет значения, которые являются ссылками на соцсети или поля с ключами vk/ok/instagram/tiktok."""
...
# ===== ОТЛАДОЧНАЯ ФУНКЦИЯ (закомментирована) =====
def print_structure(data, prefix=""):
    ...
    if isinstance(data, dict):
        ...
        for i, value in enumerate(data[:3]):
            print(f"{prefix}[{i}]: {type(value).__name__}")
            print_structure(value, prefix + "  ")
...
async def collect_phone_data(query: str):
    ...
    # ---- ОТЛАДКА: раскомментировать для просмотра структуры ----
    # print("\n=== STRUCTURE BIGBASE ===")
    # print_structure(bigbase)
    ...
    if bigbase and isinstance(bigbase, dict):
        ...
        # ---- СОЦСЕТИ ИЗ BIGBASE (УНИВЕРСАЛЬНЫЙ ПОИСК) ----
    ...
    # ---- 3. ДРУГИЕ API ----
    for src in [nightsearch, seon, snusbase, jitler, depsearch]:
        if not src:
            continue
    ...
    result['phone_books'] = normalize_list(result['phone_books'])
    
    # Дополнительная очистка соцсетей
    for key in ['vk', 'instagram', 'tiktok', 'ok']:
        if result.get(key):
...
async def main():
    await init_db()
    print("🚀 Бот запущен (универсальный поиск соцсетей)")
