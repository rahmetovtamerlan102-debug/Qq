...
# ===== СБОР ДАННЫХ (С ПРЯМЫМ ПАРСИНГОМ BIGBASE) =====
async def collect_data(query: str, search_type: str, is_phone: bool = False):
    ...
    # Если не нашли в BigBase, пробуем другие
    if not op:
...
async def main():
    await init_db()
    print("🚀 Бот запущен (финальная версия с прямым парсингом BigBase)")
