...
FUNSTAT_TOKEN = os.getenv("FUNSTAT_TOKEN", "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1aWQiOiIyMDMzMDI5NDc1IiwianRpIjoiODJmMjlmNzQtYmJlMi00ZGUwLWEwZDQtN2EzMDJhMWE5MDViIiwiZXhwIjoxODAxMDA4MzM4fQ.Mba4aX85YAMcaMLfhUBzXtCoNmEujfMe-6sGBbp3kT-T2SiLM_Ho0BBAFAQ8_C6Gz06PH9mAYhfBvlLSjb4oVd1Fm_vmb8MC-wuObU3qgfGrYdGzVF3ntJHv-LdNELq-jsqvQOY3jq9meso9dUoyj5SviDQWL6cvnRQ03kpHWxA")
...
# ===== FUNSTAT API =====
async def funstat_get_names(user_id: str):
    """Получает историю имён пользователя из FunStat"""
    if not FUNSTAT_TOKEN:
        return []
    
    url = f"https://api.funstat.org/api/v1/users/{user_id}/names"
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=5)) as resp:
                if resp.status == 200:
                    data = await resp.json()
                    print(f"📛 FunStat names для {user_id}: {data}")
                    return data.get('data', []) if isinstance(data, dict) else []
                return []
    except Exception as e:
        print(f"❌ FunStat names error: {repr(e)}")
        return []

async def funstat_get_usernames(user_id: str):
    """Получает историю юзернеймов пользователя из FunStat"""
    if not FUNSTAT_TOKEN:
        return []
    
    url = f"https://api.funstat.org/api/v1/users/{user_id}/usernames"
    headers = {"Authorization": f"Bearer {FUNSTAT_TOKEN}"}
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=5)) as resp:
                if resp.status == 200:
                    data = await resp.json()
                    print(f"👤 FunStat usernames для {user_id}: {data}")
                    return data.get('data', []) if isinstance(data, dict) else []
                return []
    except Exception as e:
        print(f"❌ FunStat usernames error: {repr(e)}")
        return []

async def funstat_get_gifts(user_id: str):
    """Получает подарки пользователя из FunStat"""
    if not FUNSTAT_TOKEN:
        return []
    
    url = f"https://api.funstat.org/api/v1/users/{user_id}/gifts_relation"
    headers = {"Authorization": f"Bearer {FUNSTAT_TOKEN}"}
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=5)) as resp:
                if resp.status == 200:
                    data = await resp.json()
                    print(f"🎁 FunStat gifts для {user_id}: {data}")
                    return data.get('data', []) if isinstance(data, dict) else []
                return []
    except Exception as e:
        print(f"❌ FunStat gifts error: {repr(e)}")
        return []
...
# ===== СБОР ДАННЫХ ДЛЯ ID (BIGBASE + JITLER + FUNSTAT) =====
async def collect_id_data(query: str, search_type: str):
    ...
    tasks = [
        ...
        funstat_get_names(query),
        funstat_get_usernames(query),
        funstat_get_gifts(query)
    ]
    ...
    funstat_names = results[2] if isinstance(results[2], list) else []
    funstat_usernames = results[3] if isinstance(results[3], list) else []
    funstat_gifts = results[4] if isinstance(results[4], list) else []
    
    print(f"✅ BigBase: {'есть' if bigbase else 'нет'}")
    print(f"✅ Jitler: {'есть' if jitler else 'нет'}")
    print(f"✅ FunStat names: {len(funstat_names)}")
    print(f"✅ FunStat usernames: {len(funstat_usernames)}")
    print(f"✅ FunStat gifts: {len(funstat_gifts)}")
    ...
    # ---- FUNSTAT ----
    # История имён
    for name_entry in funstat_names:
        if isinstance(name_entry, dict):
            date = name_entry.get('date', name_entry.get('created_at'))
            name = name_entry.get('name') or name_entry.get('first_name') or name_entry.get('last_name')
            if name:
                result['name_history'].append({'date': date, 'name': name})
        elif isinstance(name_entry, str):
            result['name_history'].append({'date': None, 'name': name_entry})
    
    # История юзернеймов (добавляем в telegrams)
    for username_entry in funstat_usernames:
        if isinstance(username_entry, dict):
            username = username_entry.get('username') or username_entry.get('name')
            if username:
                if not username.startswith('@'):
                    username = '@' + username
    ...
    # История имён — объединяем из BigBase и FunStat
    seen_names = set()
    unique_name_history = []
...
async def main():
    await init_db()
    print("🚀 Бот запущен (BigBase + Jitler + FunStat)")
