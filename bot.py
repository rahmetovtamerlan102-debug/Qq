...
# ВАЖНО: этот токен не работает для CryptoPay – замените на токен из @CryptoBot (Crypto Pay → Create App)
    ...
    try:
        async with session.post(url, json=payload, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as resp:
            ...
            if resp.status == 200:
                ...
                if data.get('ok'):
                    invoice = data['result']
                    invoice_id = str(invoice['invoice_id'])  # ПРЕОБРАЗОВАНИЕ В СТРОКУ
