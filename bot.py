...
async def collect_data(query: str, search_type: str, is_phone: bool = False):
    ...
    # ---- Оператор, регион, страна (BigBase) ----
    if is_phone:
        op = None
        reg = None
        country = None
        ...
        if not op or not reg or not country:
            for src in sources[1:]:
                if not src:
                    continue
                if not op:
                    op = deep_search(src, 'operator') or deep_search(src, 'оператор') or deep_search(src, 'oper')
                if not reg:
                    reg = deep_search(src, 'region') or deep_search(src, 'регион') or deep_search(src, 'reg')
                if not country:
                    country = deep_search(src, 'country') or deep_search(src, 'страна')
    ...
    # ---- ФИО, дата рождения, возраст (BigBase) ----
    fio = None
    birthdate = None
    age = None
