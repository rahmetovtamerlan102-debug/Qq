...
# ===== ТЕКСТОВЫЕ ФОРМАТТЕРЫ (БЕЗ MARKDOWN) =====
def format_phone_report(data: dict, views: int) -> str:
    ...
    if data.get('fio'):
        lines.append(f"- ЗАПИСАН В БАЗАХ: {data['fio']}")
    
    if data.get('birthdate'):
        lines.append(f"- ДАТА РОЖДЕНИЯ: {data['birthdate']}")
    
    phones = [data.get('query', '')]
    if data.get('phone_books'):
        for pb in data['phone_books'][:5]:
            if pb and pb not in phones and len(re.sub(r'\D', '', str(pb))) >= 10:
                phones.append(re.sub(r'\D', '', str(pb)))
    phones = list(dict.fromkeys([p for p in phones if p]))
    if phones:
        lines.append(f"- ТЕЛЕФОН: {', '.join(phones[:3])}")
    ...
    for key, value in extra.items():
        if isinstance(value, dict) and 'data' in value:
            for k, v in value['data'].items():
                if 'паспорт' in str(k).lower() or 'passport' in str(k).lower():
                    passport = v
                    break
    if not passport:
        for key, value in extra.items():
            if isinstance(value, dict) and 'data' in value:
                for k, v in value['data'].items():
                    if 'паспорт' in str(k).lower() and v:
                        passport = v
                        break
    if passport:
        lines.append(f"- ПАСПОРТ: {passport}")
    
    if data.get('vk'):
        vk_id = re.search(r'/(id\d+)', str(data['vk']))
        if vk_id:
            lines.append(f"- ВК: {vk_id.group(1)}")
        else:
            lines.append(f"- ВК: {data['vk']}")
    
    if data.get('ok'):
        ok_id = re.search(r'/(\d+)', str(data['ok']))
        if ok_id:
            lines.append(f"- OK: {ok_id.group(1)}")
        else:
    ...
    lines.append("")
    lines.append("Полная информация доступна в файле ниже")
...
def format_generic_report(data: dict, views: int, title: str) -> str:
    ...
    if data.get('tiktok'):
        lines.append(f"TikTok: {data['tiktok']}")
    if data.get('cards'):
        lines.append(f"Карты: {', '.join(data['cards'][:3])}")
    if data.get('groups'):
        lines.append(f"Группы: {', '.join(data['groups'][:5])}")
    if data.get('domains'):
        lines.append(f"Домены: {', '.join(data['domains'][:3])}")
    lines.append(f"Интересовались этим: {views}")
    return "\n".join(lines)
...
def generate_html_report(data: dict, views: int, title: str = "Отчёт по номеру телефона") -> str:
    ...
    # Соцсети и Email отдельно друг от друга
    social_fields = []
    if data.get('vk'):
        social_fields.append(("ВКОНТАКТЕ", data['vk']))
    if data.get('ok'):
        social_fields.append(("ОДНОКЛАССНИКИ", data['ok']))
    if data.get('instagram'):
        social_fields.append(("INSTAGRAM", data['instagram']))
    if data.get('tiktok'):
        social_fields.append(("TIKTOK", data['tiktok']))
    ...
    if social_fields:
        ...
        sources_set.add("Социальные сети")
        records_count += 1
    ...
    if data.get('emails'):
        ...
        email_fields.append(("E-MAIL", emails))
    ...
    if email_fields:
        ...
        sources_set.add("Email")
        records_count += 1
    
    # Остальные записи из extra
    for key, value in extra.items():
...
async def start_cmd(message: types.Message):
    ...
    text = """Dataseeker — твой бесплатный цифровой детектив.
...
async def universal_handler(message: types.Message):
    ...
    if search_type == "phone":
        ...
        if query:
            ...
            viber_url = f"viber://chat?number={query}"
            buttons.append([InlineKeyboardButton(text="Viber", url=viber_url)])
        ...
        if keyboard:
            await status_msg.edit_text(report, reply_markup=keyboard)
        else:
            await status_msg.edit_text(report)
        
        # Автоматически отправляем HTML-файл
        await message.reply_document(html_file, caption="Полный HTML-отчёт")
