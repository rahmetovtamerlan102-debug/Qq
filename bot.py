...
CRYPTOPAY_TOKEN = os.getenv("CRYPTOPAY_TOKEN")  # из .env, не хранить в коде
...
# === ГЕНЕРАТОР HTML-ОТЧЁТА (ваш дизайн) ===
        ...
        async with session.post(url, json=payload, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as resp:
            if resp.status == 200:
                ...
                if data.get('ok'):
                    ...
                    return None, data.get('error', {}).get('message', 'Ошибка')
            else:
        ...
        async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=5)) as resp:
            if resp.status == 200:
                ...
                if data.get('ok'):
                    ...
                    return {"status": "error", "message": data.get('error', {}).get('message', 'Ошибка API')}
            else:
