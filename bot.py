        ...
        async with session.get(url, timeout=aiohttp.ClientTimeout(total=API_TIMEOUTS["ipapi"])) as resp:
            if resp.status == 200:
                ...
                if data.get('status') == 'success':
                    ...
                    logger.warning(f"ip-api.com error: {data.get('message')}")
                    return {}
            return {}
...
async def collect_general_data(query: str, search_type: str = "phone"):
    ...
    logger.info(f"DepSearch result for {query}: {json.dumps(depsearch, ensure_ascii=False)[:500]}")
    ...
    if depsearch:
        search_type_from_api = depsearch.get('search_type', '')
        
        # Для IP
        if search_type_from_api == 'ip':
            ip_info = depsearch.get('ip_info', {})
            if ip_info:
                if ip_info.get('country'):
                    result['country'] = str(ip_info['country'])
                    sources_set.add("DepSearch (страна)")
                if ip_info.get('city') and ip_info.get('city') != 'Неизвестно':
                    result['city'] = str(ip_info['city'])
                    sources_set.add("DepSearch (город)")
                if ip_info.get('provider'):
                    result['operator'] = str(ip_info['provider'])
                    sources_set.add("DepSearch (провайдер)")
                if ip_info.get('organization'):
                    if not result['operator']:
                        result['operator'] = str(ip_info['organization'])
                    sources_set.add("DepSearch (организация)")
                extra_ip = {}
                for key in ['country', 'city', 'reverse_dns', 'provider', 'organization', 'timezone', 'coords', 'zip', 'is_proxy', 'is_hosting']:
                    if ip_info.get(key) and ip_info[key] not in ('Неизвестно', 'Нет данных', ''):
                        extra_ip[key] = ip_info[key]
                if extra_ip:
                    records_count += 1
                    result['extra'][f"IP информация (DepSearch)"] = {
                        'source': 'DepSearch (IP)',
                        'data': extra_ip
                    }
                    sources_set.add("DepSearch (IP)")

        # Для телефона
        elif search_type_from_api == 'phone':
        ...
        # Обработка results (общая для всех)
        ...
        if isinstance(results_list, list):
            for idx, item in enumerate(results_list):
                ...
                if record_data:
                    record_key = "|".join(str(v) for v in record_data.values())
                    ...
                    # Извлечение основных полей (если они есть)
                    if not result['fio']:
                        for key in ['👤ФИО', '👤Полное имя', 'full_name', 'fio', 'ФИО', 'Полное имя', '👤Имя', '🔸Никнейм']:
                    ...
                    # Телефон (из любой записи добавляем в phone_books)
                    phone_val = None
                    ...
                    if phone_val and phone_val not in result['phone_books']:
                        ...
                        sources_set.add("DepSearch (телефон из записи)")
...
# === ГЕНЕРАТОР HTML-ОТЧЁТА (УНИВЕРСАЛЬНЫЙ, БЕЗ ЭМОДЗИ) ===
