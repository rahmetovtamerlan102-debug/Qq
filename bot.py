...
# НОВЫЕ ТАРИФЫ (цена в звёздах скрыта в меню)
PACKAGES = [
...
# ===== НОВОЕ МЕНЮ ТАРИФОВ (ДВЕ КОЛОНКИ, БЕЗ ЗВЁЗД) =====
...
async def buy_requests_callback(callback: types.CallbackQuery):
    ...
    for pkg in PACKAGES:
        if pkg["requests"] == 1000:
            # Выгодный тариф – отдельная кнопка на всю ширину
    ...
    for btn in buttons:
        if isinstance(btn, list):
            # Это кнопка "Выгодный" – добавляем как отдельную строку
            if row:
