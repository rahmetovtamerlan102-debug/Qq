...
async def collect_phone_data(query: str):
    ...
    if bigbase and isinstance(bigbase, dict):
        # ===== ИСПРАВЛЕНО: Данные в корне, а не в dossier =====
        head = bigbase.get('head', {})
        
        # ===== ОБЯЗАТЕЛЬНО ИЗВЛЕКАЕМ ОПЕРАТОРА, РЕГИОН И СТРАНУ =====
        ...
        if country_info:
            ...
            # Пробуем найти страну в других полях
            country = head.get('country') or head.get('phone_country')
            if country:
        ...
        # ===== ИЗВЛЕКАЕМ ДАННЫЕ ИЗ CONNECTIONS =====
