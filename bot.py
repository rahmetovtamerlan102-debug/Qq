...
🚀 Просто отправь данные для поиска!"""
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🌐 Зеркало Dataseeker", url="https://t.me/Dataseekerboto")]
    ])

    sent_msg = await message.reply(text, parse_mode="Markdown", reply_markup=keyboard)

    # Если бот в группе/супергруппе и есть права — закрепляем
    try:
...
async def universal_handler(message: types.Message):
    ...
    # Если не распознано как id, то обычное определение
