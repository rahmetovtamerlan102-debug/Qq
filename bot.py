...
async def jitler_search_with_balancer(query: str, search_type: str = "number"):
    """
    Отправляет запрос в Jitler с оптимизированным ожиданием.
    """
...
# ===== СБОР ДАННЫХ ДЛЯ ID (ТОЛЬКО BIGBASE) =====
async def collect_id_data(query: str, search_type: str):
    """
    Сбор данных для ID только из BigBase (быстро).
    """
    print(f"🔍 Поиск ID: {query}, тип: {search_type}")
    
    try:
        bigbase = await asyncio.wait_for(
            bigbase_search(query),
            timeout=5.0
        )
        print(f"✅ BigBase: {'есть' if bigbase else 'нет'}")
    except asyncio.TimeoutError:
        print(f"⏱️ Таймаут BigBase для {query}")
        bigbase = {}
    ...
    print(f"📊 Результат: телеграм={len(result['telegrams'])}, группы={len(result['groups'])}, история={len(result['username_history'])}")
    return result
...
async def direction_callback(callback: types.CallbackQuery):
    ...
    try:
        data = await asyncio.wait_for(collect_id_data(id_str, search_type), timeout=6.0)
    except asyncio.TimeoutError:
    ...
    if any([data.get('telegrams'), data.get('groups'), data.get('username_history')]):
...
async def main():
    await init_db()
    print("🚀 Бот запущен (ID-поиск: только BigBase)")
