...
# ===== СБОР ДАННЫХ (приоритет DepSearch) =====
async def collect_phone_data(query: str):
                ...
                if isinstance(item, dict):
                    ...
                    if email:
                        if isinstance(email, list):
                            result['emails'].extend(email)
                        else:
                            result['emails'].append(str(email))
    
    # ---- 2. ПАРСИНГ BIGBASE (FALLBACK, если DepSearch не дал) ----
    if bigbase:
        # Если DepSearch не дал оператор/регион/страну – берём из BigBase
        if not result['operator'] or not result['region'] or not result['country']:
        ...
        # ФИО из BigBase (если DepSearch не дал)
        if not result['fio']:
        ...
        # Дата рождения из BigBase (если нет в DepSearch)
        if not all_birthdates:
            connections = bigbase.get('connections', {})
            persons = connections.get('person', [])
            for person in persons:
        ...
        # Email и другие поля из BigBase (дополняем)
        result['emails'].extend(
            deep_find_all(bigbase, 'email') +
            deep_find_all(bigbase, 'mail') +
            deep_find_all(bigbase, 'e-mail')
        )
        result['telegrams'].extend(
            deep_find_all(bigbase, 'telegram') +
            deep_find_all(bigbase, 'tg')
        )
        vk = deep_find(bigbase, 'vk') or deep_find(bigbase, 'vkontakte')
        if vk:
            result['vk'] = vk
        instagram = deep_find(bigbase, 'instagram')
        if instagram:
            result['instagram'] = instagram
        tiktok = deep_find(bigbase, 'tiktok')
        if tiktok:
    ...
    # ---- 3. ДРУГИЕ API (дополняют данные, если чего-то не хватает) ----
    for src in [nightsearch, seon, snusbase, jitler]:
        ...
        if not all_birthdates:
            birthdate = (
                deep_find(src, 'birthdate') or
                deep_find(src, 'birth_date') or
                deep_find(src, 'date_of_birth') or
                deep_find(src, 'dob')
            )
            if birthdate:
        ...
        result['telegrams'].extend(
            deep_find_all(src, 'telegram') +
            deep_find_all(src, 'tg')
        )
    ...
    phone_books = []
    if jitler:
        phone_books.extend(deep_find_all(jitler, 'phone_books'))
        phone_books.extend(deep_find_all(jitler, 'phonebook'))
        phone_books.extend(deep_find_all(jitler, 'phone_book'))
        phone_books.extend(deep_find_all(jitler, 'contacts'))
...
def format_phone_report(data: dict, views: int) -> str:
    ...
    if data.get('instagram') and clean_value(data['instagram']):
        lines.append(f"\n📷 Instagram: {clean_value(data['instagram'])}")
...
async def main():
    await init_db()
    print("🚀 Бот запущен (приоритет DepSearch, без банковских карт)")
