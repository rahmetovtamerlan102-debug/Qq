...
async def collect_phone_data(query: str):
    ...
    # ===== ОБЯЗАТЕЛЬНО ИЗВЛЕКАЕМ ОПЕРАТОРА, РЕГИОН И СТРАНУ ИЗ BigBase =====
    if bigbase and isinstance(bigbase, dict):
        head = bigbase.get('head', {})
        ...
        if operator:
            ...
            for key in ['operator', 'phone_operator', 'operator_name']:
                found = deep_find(bigbase, key)
                if found:
                    result['operator'] = str(found)
                    sources_set.add("BigBase (оператор)")
                    break
        ...
        if region:
            ...
            # Пробуем найти в других местах
            for key in ['region', 'phone_region', 'area', 'district']:
                found = deep_find(bigbase, key)
                if found:
                    result['region'] = str(found)
                    sources_set.add("BigBase (регион)")
                    break

        # ИЗВЛЕЧЕНИЕ СТРАНЫ из phone_country_info
        country_info = head.get('phone_country_info')
        if country_info:
            result['country'] = str(country_info)
            sources_set.add("BigBase (страна)")
        else:
            # Пробуем найти в других местах
            for key in ['country', 'phone_country', 'country_code']:
                found = deep_find(bigbase, key)
                if found:
                    result['country'] = str(found)
                    sources_set.add("BigBase (страна)")
                    break

        # ДОПОЛНИТЕЛЬНО ПАРСИМ СТРАНУ ИЗ phone_country_info (если она в формате "RU Россия (Москва)")
        if country_info and isinstance(country_info, str):
            # Извлекаем страну из формата "RU Россия (Москва)"
            country_match = re.search(r'[A-Z]{2}\s+([^\s(]+)', country_info)
            if country_match:
                result['country'] = country_match.group(1)
            else:
                # Или просто берем первую часть после кода страны
                parts = country_info.split()
                if len(parts) > 1:
                    # Пропускаем код страны (RU)
                    for part in parts[1:]:
                        if '(' not in part and len(part) > 2:
                            result['country'] = part
                            break

        # ===== ИЗВЛЕКАЕМ ФИО ИЗ CONNECTIONS =====
        ...
        # ===== ИЗВЛЕКАЕМ АДРЕС ИЗ CONNECTIONS =====
        for person in persons:
    ...
    # ===== DEPSEARCH - дополнительная информация =====
...
def format_phone_report(data: dict, views: int) -> str:
    ...
    if data.get('operator'):
        ...
        lines.append(f"├ Оператор: Не найден")
    if data.get('region'):
        lines.append(f"├ Регион: {data['region']}")
    else:
        lines.append(f"├ Регион: Не найден")
    if data.get('country'):
        lines.append(f"└ Страна: {data['country']}")
    else:
        lines.append(f"└ Страна: Не найдена")
...
def generate_html_report(data: dict, views: int, title: str = "Отчёт по номеру телефона") -> str:
    ...
    # Добавляем основную информацию как первую запись
    main_fields = []
    main_fields.append(("ТЕЛЕФОН", data.get('query', '')))
    main_fields.append(("ОПЕРАТОР", data.get('operator', 'Не найден')))
    main_fields.append(("РЕГИОН", data.get('region', 'Не найден')))
    main_fields.append(("СТРАНА", data.get('country', 'Не найдена')))
    
    if data.get('fio'):
        main_fields.append(("ФИО", data['fio']))
    if data.get('birthdate'):
        main_fields.append(("ДАТА РОЖДЕНИЯ", data['birthdate']))
    if data.get('age') is not None:
        main_fields.append(("ВОЗРАСТ", f"{data['age']} лет"))
    if data.get('address'):
        main_fields.append(("АДРЕС", data['address']))
    
    records.append({
        'title': "📱 Основная информация",
        'fields': main_fields
    })
    sources_set.add("Основная информация")
    records_count += 1
    
    # Добавляем остальные записи из extra
    ...
    # Добавляем телефоны и другие контакты
    extra_fields = []
    if data.get('phone_books'):
        books = ', '.join(data['phone_books'][:10])
        if len(data['phone_books']) > 10:
            books += f" ... и ещё {len(data['phone_books'])-10}"
        extra_fields.append(("ТЕЛЕФОННЫЕ КНИГИ", books))
    if data.get('emails'):
        emails = ', '.join(data['emails'][:5])
        if len(data['emails']) > 5:
            emails += f" ... и ещё {len(data['emails'])-5}"
        extra_fields.append(("EMAIL", emails))
    if data.get('telegrams'):
        extra_fields.append(("TELEGRAM", ', '.join(data['telegrams'])))
    if data.get('vk'):
        extra_fields.append(("ВКОНТАКТЕ", data['vk']))
    if data.get('ok'):
        extra_fields.append(("ОДНОКЛАССНИКИ", data['ok']))
    if data.get('instagram'):
        extra_fields.append(("INSTAGRAM", data['instagram']))
    if data.get('tiktok'):
        extra_fields.append(("TIKTOK", data['tiktok']))
    if data.get('cards'):
        cards = ', '.join(data['cards'][:3])
        if len(data['cards']) > 3:
            cards += f" ... и ещё {len(data['cards'])-3}"
        extra_fields.append(("БАНКОВСКИЕ КАРТЫ", cards))
...
async def start_cmd(message: types.Message):
        ...
        if referrer_id:
            try:
                ...
                await bot.send_message(
                    referrer_id,
                    f"🎉 Пользователь {username_display} перешёл по вашей реферальной ссылке!"
                )
            except Exception:
                pass

    # Кнопка на информационный канал
    ...
    # Отправляем основное приветствие без закрепления
