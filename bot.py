...
# ===== ПОИСК ТЕЛЕФОННЫХ КНИГ ТОЛЬКО ИЗ JITLER =====
def extract_phone_books_from_jitler(data):
    """
    Рекурсивно обходит ответ Jitler и собирает записи телефонной книги.
    Ищет объекты, содержащие ключи 'phone' или 'number', и, опционально, 'name'.
    Возвращает список строк вида "Имя: Телефон" или просто "Телефон".
    """
    results = []
    if data is None:
        return results
    ...
    if isinstance(data, dict):
        # Если это запись с телефоном
        phone = data.get('phone') or data.get('number') or data.get('tel') or data.get('phone_number')
        if phone:
            name = data.get('name') or data.get('full_name') or data.get('contact') or data.get('title')
            if name:
                results.append(f"{name}: {phone}")
            else:
                results.append(str(phone))
        # Рекурсивно обходим все значения
        for v in data.values():
            results.extend(extract_phone_books_from_jitler(v))
    elif isinstance(data, list):
        for item in data:
            results.extend(extract_phone_books_from_jitler(item))
    return results
...
async def collect_phone_data(query: str):
    ...
    for src in [nightsearch, seon, snusbase, depsearch]:  # Jitler исключён для email/telegram/соцсетей, т.к. мы берём только телефонные книги
        if not src:
            continue
    ...
    if jitler and isinstance(jitler, dict):
        # Извлекаем response (если он есть, иначе сам jitler)
        jitler_data = jitler.get('response', jitler)
        phone_books = extract_phone_books_from_jitler(jitler_data)
        # Убираем дубликаты
        result['phone_books'] = list(dict.fromkeys(phone_books))
    ...
    # Соцсети уже обработаны через get_social_url, дополнительные чистки не нужны
...
def format_phone_report(data: dict, views: int) -> str:
    ...
    # Соцсети – уже чистые URL, просто выводим
    vk = data.get('vk')
    if vk:
        lines.append(f"\n🧑‍💻 Вконтакте: {vk}")
    ok = data.get('ok')
    ...
    if data.get('phone_books'):
        lines.append("\n🔎 Телефонные книги:")
        for entry in data['phone_books'][:15]:  # ограничим 15 записей
...
async def main():
    await init_db()
    print("🚀 Бот запущен (исправлен вывод соцсетей и телефонных книг из Jitler)")
