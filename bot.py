...
async def collect_phone_data(query: str):
    ...
    sources_set = set()
    records_count = 0
    seen_records = set()  # для избежания дублирования записей
        ...
        if isinstance(results_list, list):
            for item in results_list:
                if isinstance(item, dict):
                    ...
                    # Собираем email из depsearch
                    email = item.get('✉️Почта') or item.get('email') or item.get('mail')
                    if email and '@' in str(email):
                        result['emails'].append(str(email))
                    # Собираем соцсети из depsearch
