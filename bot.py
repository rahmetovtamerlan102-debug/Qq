...
# ===== НОВЫЕ ФОРМАТТЕРЫ =====
def format_email_report(data: dict, views: int = 0) -> str:
    lines = ["📧"]
    ...
    if data.get('fio'):
        ...
        for i, fio in enumerate(fios):
            if i == len(fios) - 1 and len(fios) <= 1:
                lines.append(f"│   └ {fio}")
            elif i == len(fios) - 1:
                lines.append(f"│   └ {fio}")
            else:
...
def format_ip_report(data: dict, views: int = 0) -> str:
    lines = ["🌐"]
    lines.append(f"├ IP: {data.get('query', '')}")
    
    if data.get('location'):
        lines.append(f"├ 🗺️ Локация: {data['location']}")
    if data.get('isp'):
        lines.append(f"├ 📡 Провайдер: {data['isp']}")
    
    has_related = data.get('fio') or data.get('phone') or data.get('address') or data.get('email')
    if has_related:
        lines.append("├ 👤 Связанные данные:")
        if data.get('fio'):
            fios = data['fio'][:3]
            for i, fio in enumerate(fios):
                if i == len(fios) - 1 and not data.get('phone') and not data.get('address') and not data.get('email'):
        ...
        if data.get('phone'):
            phones = data['phone'][:3]
            for i, phone in enumerate(phones):
                if i == len(phones) - 1 and not data.get('address') and not data.get('email'):
                    lines.append(f"│   └ Телефон: {phone}")
                else:
                    lines.append(f"│   ├ Телефон: {phone}")
        if data.get('address'):
            addresses = data['address'][:3]
            for i, addr in enumerate(addresses):
                if i == len(addresses) - 1 and not data.get('email'):
...
def format_vk_report(data: dict, views: int = 0) -> str:
    lines = ["🧑‍💻"]
    ...
    has_contacts = data.get('phones') or data.get('emails') or data.get('telegrams')
    if has_contacts:
        lines.append("├ 📞 Контакты:")
        if data.get('phones'):
            phones = data['phones'][:3]
            for i, phone in enumerate(phones):
                if i == len(phones) - 1 and not data.get('emails') and not data.get('telegrams'):
                    lines.append(f"│   └ {phone}")
                else:
                    lines.append(f"│   ├ {phone}")
        if data.get('emails'):
            emails = data['emails'][:3]
            for i, email in enumerate(emails):
                if i == len(emails) - 1 and not data.get('telegrams'):
...
def format_fio_report(data: dict, views: int = 0) -> str:
    lines = ["👤"]
    ...
    if data.get('phones'):
        ...
        phones = data['phones'][:5]
        for i, phone in enumerate(phones):
            if i == len(phones) - 1 and not data.get('emails') and not data.get('telegrams') and not data.get('vk') and not data.get('ok'):
                lines.append(f"│   └ {phone}")
            else:
                lines.append(f"│   ├ {phone}")
    ...
    if data.get('emails'):
        ...
        for i, email in enumerate(emails):
            if i == len(emails) - 1 and not data.get('telegrams') and not data.get('vk') and not data.get('ok'):
                lines.append(f"│   └ {email}")
            else:
                lines.append(f"│   ├ {email}")
    ...
    if data.get('telegrams'):
        ...
        tgs = data['telegrams'][:3]
        for i, tg in enumerate(tgs):
            if i == len(tgs) - 1 and not data.get('vk') and not data.get('ok'):
                lines.append(f"│   └ {tg}")
            else:
                lines.append(f"│   ├ {tg}")
...
def format_inn_report(data: dict, views: int = 0) -> str:
    lines = ["🏛️"]
...
def generate_html_report(data: dict, views: int, title: str = "Отчёт по номеру телефона") -> str:
    ...
    extra_fields = []
    if data.get('address'):
        extra_fields.append(("АДРЕС", data['address']))
    if data.get('cards'):
    ...
    if data.get('banks'):
        ...
        extra_fields.append(("БАНКИ", banks))
    
    extra_all = data.get('extra', {})
    for key, value in extra_all.items():
        if not isinstance(value, dict) and value and value not in (None, '', [], {}):
            clean_key = re.sub(r'[^\w\s\-\.]', '', str(key)).strip()
            if clean_key and clean_key.lower() not in ['запись']:
                extra_fields.append((clean_key.upper(), format_value(value)))
    if extra_fields:
        records.append({
            'title': "Дополнительные данные",
            'fields': extra_fields
        })
        sources_set.add("Дополнительные данные")
        records_count += 1
...
async def inn_command(message: types.Message):
    ...
    views = 0
    report = format_inn_report(data, views)
...
async def universal_handler(message: types.Message):
    ...
    if search_type == "phone":
        ...
        views = 0
        report = format_email_report(data, views)
        await status_msg.edit_text(report, parse_mode="Markdown")
    elif search_type == "ip":
        data = await collect_ip_data(query)
        views = 0
        report = format_ip_report(data, views)
        await status_msg.edit_text(report, parse_mode="Markdown")
    elif search_type == "vk":
        data = await collect_vk_data(query)
        views = 0
        report = format_vk_report(data, views)
        await status_msg.edit_text(report, parse_mode="Markdown")
