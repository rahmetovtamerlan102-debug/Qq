...
# ===== НОВЫЙ HTML-ОТЧЁТ В СТИЛЕ ПРИМЕРА =====
def generate_html_report(data: dict, views: int, title: str = "Отчёт по номеру телефона") -> str:
    # Формируем список секций для аккордеонов
    sections = []

    # Секция "Основные данные"
    personal = []
    if data.get('operator'):
        personal.append(('Оператор', data['operator']))
    if data.get('region'):
        personal.append(('Регион', data['region']))
    if data.get('country'):
        personal.append(('Страна', data['country']))
    if data.get('fio'):
        personal.append(('ФИО', data['fio']))
    if data.get('birthdate'):
        personal.append(('Дата рождения', data['birthdate']))
    if data.get('age') is not None:
        personal.append(('Возраст', data['age']))
    if data.get('address'):
        personal.append(('Адрес', data['address']))
    if personal:
        sections.append(('Основные данные', personal))
    ...
    if data.get('phone_books'):
        ...
        sections.append(('Телефонные книги', [('Контакты', books)]))

    # Соцсети
    social = []
    if data.get('vk'):
        social.append(('ВКонтакте', data['vk']))
    if data.get('ok'):
        social.append(('Одноклассники', data['ok']))
    if data.get('instagram'):
        social.append(('Instagram', data['instagram']))
    if data.get('tiktok'):
        social.append(('TikTok', data['tiktok']))
    if social:
        sections.append(('Социальные сети', social))
    ...
    if data.get('emails'):
        ...
        sections.append(('E-mail', [('Почта', emails)]))

    # Telegram
    if data.get('telegrams'):
        sections.append(('Telegram', [('Аккаунты', ', '.join(data['telegrams']))]))
    ...
    if data.get('cards'):
        ...
        sections.append(('Банковские карты', [('Карты', cards)]))
    ...
    if data.get('banks'):
        ...
        sections.append(('Банки', [('Банки', banks)]))

    # Если нет ни одной секции – показать заглушку
    if not sections:
        sections.append(('Данные не найдены', [('', 'Ничего не найдено по этому номеру')]))

    # Подсчёт результатов (количество непустых секций)
    result_count = len([s for s in sections if s[1]])
    ...
    for idx, (sec_title, _) in enumerate(sections, 1):
        ...
        <div class="client">
            ...
            <a href="#section{idx}" class="clients_name">{sec_title}</a>
        </div>
        <div class="stick"></div>
        """
    ...
    # Аккордеоны
    accordions = ""
    for idx, (sec_title, rows) in enumerate(sections, 1):
        rows_html = ""
        for label, value in rows:
            if label and value:
                rows_html += f'<div class="row"><strong>{label.upper()}:</strong><span>{value}</span></div>'
        accordions += f"""
        <div id="section{idx}" class="accordion_inner">
