...
# ⚠️ ЭТОТ ТОКЕН НЕ ПОДХОДИТ ДЛЯ CRYPTOPAY! Это токен бота, а не приложения.
# Для оплаты создайте приложение в @CryptoBot -> Crypto Pay -> Create App
                ...
                if data.get('ok'):
                    ...
                    if invoices:
                        ...
                        if status == 'paid':
                            ...
                            return {"status": "pending", "message": "❌ Вы не оплатили. Оплатите счёт и нажмите снова."}
                        elif status == 'expired':
...
async def start_cmd(message: types.Message):
    ...
    msg_link = await message.reply("🔮 Вечная ссылка на информацию:", reply_markup=keyboard_info)
    try:
    ...
    text = """🕵️ Dataseeker — бесплатный цифровой детектив.

Телефон, Email, VK, IP, ИНН, ФИО — ищи всё в одном месте.
5 бесплатных запросов в сутки."""
...
async def buy_requests_callback(callback: types.CallbackQuery):
    ...
    for pkg in PACKAGES:
        keyboard.inline_keyboard.append([
            InlineKeyboardButton(text=f"{pkg['requests']} запросов — ${pkg['usd']} USD", callback_data=f"buy_pkg|{pkg['requests']}|{pkg['usd']}")
        ])
    keyboard.inline_keyboard.append([InlineKeyboardButton(text="🔙 Назад", callback_data="back_to_menu")])
    await callback.message.edit_text("💳 Выберите пакет:", parse_mode="Markdown", reply_markup=keyboard)
    await callback.answer()
...
async def buy_package_callback(callback: types.CallbackQuery):
    ...
    if not pay_url:
        await callback.message.edit_text("❌ Ошибка создания счёта.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
    ...
    await callback.message.edit_text(
        f"💳 Счёт: {requests_count} запросов, {amount_usd} USD\nНажмите «Оплатить», затем после оплаты — «Я оплатил».",
...
async def check_payment_callback(callback: types.CallbackQuery):
    ...
    pool = await get_pool()
    async with pool.acquire() as conn:
        purchase = await conn.fetchrow('SELECT * FROM purchases WHERE invoice_id=$1 AND user_id=$2', invoice_id, user_id)
        if not purchase:
            await callback.message.edit_text("❌ Платёж не найден.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🔙 Назад", callback_data="buy_requests")]
            ]))
            await callback.answer()
            return
    ...
    if result["status"] == "paid":
        ...
        await callback.message.edit_text("✅ Оплачено! Запросы начислены.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 В меню", callback_data="back_to_menu")]
        ]))
        await callback.answer()
    elif result["status"] == "pending":
        await callback.answer(result["message"], show_alert=True)  # "Вы не оплатили"
    else:
...
async def process_enter_promo(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await back_to_menu_callback(message)

async def back_to_menu_callback(message: types.Message):
    main_keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Мой профиль", callback_data="my_profile")],
        [InlineKeyboardButton(text="Реферальная система", callback_data="referral_system")],
        [InlineKeyboardButton(text="Пополнить запросы", callback_data="buy_requests")]
...
async def admin_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    if action == "stats":
        ...
        if not rows:
            ...
            text = "💰 Последние платежи:\n" + "\n".join([f"{p['user_id']} — {p['requests']} запросов, {p['amount']} USD, {p['status']}" for p in rows])
...
async def delete_promo_callback(callback: types.CallbackQuery):
    ...
    await admin_callback(callback, None)  # обновить список

# FSM для админки (кратко)
