...
async def collect_id_data(query: str, search_type: str):
        ...
        for record in records:
            ...
            for item in base_record:
                if isinstance(item, list) and len(item) >= 2:
                    ...
                    print(f"🔑 Ключ: '{key}' -> {str(value)[:50] if value else 'None'}")
                                ...
                                if isinstance(name_list, list):
                                    for name_item in name_list:
                                        if isinstance(name_item, list) and len(name_item) >= 2:
                                            ...
                                            if name:
                                                result['username_history'].append({'name': name})
    ...
    # Убираем дубликаты (для списков строк)
    ...
    # Для username_history (список словарей) — убираем дубликаты вручную
    seen = set()
    unique_history = []
    for item in result['username_history']:
        if isinstance(item, dict):
            key = item.get('name', '')
        else:
            key = str(item)
    ...
    result['username_history'] = unique_history
...
async def direction_callback(callback: types.CallbackQuery):
    ...
    try:
        data = await asyncio.wait_for(collect_id_data(id_str, search_type), timeout=8.0)
    except asyncio.TimeoutError:
...
async def main():
    await init_db()
    print("🚀 Бот запущен (исправлен BUTTON_USER_INVALID)")
