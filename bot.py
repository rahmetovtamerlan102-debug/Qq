...
async def jitler_search_with_balancer(query: str, search_type: str = "number"):
    create_timeout = aiohttp.ClientTimeout(total=2)
    poll_timeout = aiohttp.ClientTimeout(total=1.5)
    async with aiohttp.ClientSession() as session:
...
async def collect_phone_data(query: str):
    # Запускаем все API параллельно с короткими таймаутами
    async def fetch_bigbase():
        try:
            return await asyncio.wait_for(bigbase_search(query), timeout=3.0)
        except:
            return {}
    
    async def fetch_nightsearch():
        try:
            return await asyncio.wait_for(nightsearch_search(query), timeout=2.0)
        except:
            return {}
    
    async def fetch_seon():
        try:
            return await asyncio.wait_for(seon_search(query), timeout=2.0)
        except:
            return {}
    
    async def fetch_snusbase():
        try:
            return await asyncio.wait_for(snusbase_search(query), timeout=2.0)
        except:
            return {}
    
    async def fetch_depsearch():
        try:
            return await asyncio.wait_for(depsearch_search(query), timeout=3.0)
        except:
            return {}
    
    async def fetch_jitler():
        try:
            return await asyncio.wait_for(jitler_search_with_balancer(query, "number"), timeout=2.5)
        except:
            return {}
    
    # Запускаем все параллельно с общим таймаутом 6 секунд
    try:
    ...
    if jitler and isinstance(jitler, dict):
        ...
        # Извлекаем также данные из jitler записи
        records = jitler_data.get('records', [])
