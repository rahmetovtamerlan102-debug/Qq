...
async def collect_phone_data(query: str):
    ...
    # Обработка NightSearch, SEON, Snusbase для телефонных книг и соцсетей
    ...
    # Обработка Jitler (только для телефонных книг)
...
# Остальные сборщики (email, ip, vk, inn, fio) – без изменений
...
async def universal_handler(message: types.Message):
    ...
    if search_type == "phone":
        ...
        stats_text = []
        if data.get('records_count'):
            stats_text.append(f"📊 Количество записей: {data['records_count']}")
        if data.get('sources'):
            stats_text.append(f"📁 Количество баз: {len(data['sources'])}")
        caption = "\n".join(stats_text) if stats_text else "📄 Полный HTML-отчёт"
        ...
        await message.reply_document(html_file, caption=caption)
...
async def html_phone_callback(callback: types.CallbackQuery):
    ...
    await callback.message.reply_document(html_file, caption=caption)
