...
async def start_cmd(message: types.Message):
    ...
    # Кнопка на информационный канал (без рефералки)
    ...
    # Отправляем основное приветствие (без реферальной кнопки)
...
async def inn_command(message: types.Message):
    ...
    if daily >= 5:
        await message.reply("❌ Вы исчерпали лимит запросов на сегодня (5). Попробуйте завтра.")
        return
...
async def universal_handler(message: types.Message):
    ...
    # === ПРОВЕРКА ЛИМИТА (5 запросов в день) ===
