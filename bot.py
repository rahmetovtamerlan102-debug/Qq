...
async def collect_phone_data(query: str):
    ...
    # ---- 1. ПАРСИНГ DEPSEARCH (ЕДИНСТВЕННЫЙ ДЛЯ ОСНОВНЫХ ПОЛЕЙ) ----
    if depsearch and isinstance(depsearch, dict):
        ...
        if isinstance(results_list, list):
            for item in results_list:
                ...
                # Email из DepSearch тоже добавляем (дополнительное поле)
    ...
    # ---- 2. СОБИРАЕМ ДОПОЛНИТЕЛЬНЫЕ ДАННЫЕ ИЗ ВСЕХ API (КРОМЕ ОСНОВНЫХ ПОЛЕЙ) ----
    # Здесь BigBase используется только для соцсетей, email, телеграм и книг
    for src in [bigbase, nightsearch, seon, snusbase, jitler]:
        ...
        if ok and not result['ok']:
            result['ok'] = ok
        books = deep_find_all(src, 'phone_books') + deep_find_all(src, 'contacts')
        if books:
            result['phone_books'].extend(books)
    ...
    # Преобразуем соцсети в чистый URL (если они словари)
...
def format_phone_report(data: dict, views: int) -> str:
    ...
    # Соцсети – чистый URL (уже нормализованы в collect_phone_data)
    if data.get('vk'):
        lines.append(f"\n🧑‍💻 Вконтакте: {data['vk']}")
    if data.get('ok'):
...
async def main():
    await init_db()
    print("🚀 Бот запущен (DepSearch – основной для основных данных)")
