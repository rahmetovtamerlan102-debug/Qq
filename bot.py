...
async def collect_phone_data(query: str):
    ...
    jitler = results[4] if isinstance(results[4], dict) else {}
    ...
    # ---- 1. ПАРСИНГ BIGBASE (ОСНОВНОЙ ИСТОЧНИК) ----
    if bigbase:
        ...
        for person in persons:
            ...
            for key in ('birthdate', 'birth_date', 'date_of_birth', 'dob'):
                birthdate_list = person.get(key, [])
                if not isinstance(birthdate_list, list):
                    birthdate_list = [birthdate_list]
                for bd_item in birthdate_list:
                    if isinstance(bd_item, dict):
                        value = bd_item.get('value') or bd_item.get('date')
                    else:
                        value = bd_item
                    normalized = normalize_birthdate(value)
                    if normalized:
    ...
    # ---- 2. ДРУГИЕ API (только если нет ФИО из BigBase) ----
    if not result['fio']:
    ...
    # ---- 3. СБОР ДАТ РОЖДЕНИЯ ИЗ ДРУГИХ API ----
    for src in [nightsearch, seon, snusbase, jitler]:
        if not src:
            continue
        birthdate = (
            deep_find(src, 'birthdate')
            or deep_find(src, 'birth_date')
            or deep_find(src, 'date_of_birth')
            or deep_find(src, 'dob')
        )
        ...
        if vk:
            if isinstance(vk, dict) and vk.get('name'):
                ...
                if birth_from_vk:
                    normalized_vk = normalize_birthdate(birth_from_vk)
                    if normalized_vk:
                        all_birthdates.append(normalized_vk)
            result['vk'] = vk
        ...
        if ok:
            if isinstance(ok, dict) and ok.get('name'):
                ...
                if birth_from_ok:
                    normalized_ok = normalize_birthdate(birth_from_ok)
                    if normalized_ok:
                        all_birthdates.append(normalized_ok)
            result['ok'] = ok
        
        result['banks'].extend(deep_find_all(src, 'bank') + deep_find_all(src, 'banks') + deep_find_all(src, 'account'))
    
    # ---- ТЕЛЕФОННЫЕ КНИГИ ТОЛЬКО ИЗ JITLER ----
    phone_books = []
    jitler_response = jitler.get('response')
    if isinstance(jitler_response, list) and jitler_response:
        first = jitler_response[0]
        if isinstance(first, dict):
            books = first.get('phone_books') or first.get('contacts')
            if books:
                if isinstance(books, list):
                    phone_books.extend(books)
                else:
                    phone_books.append(str(books))
    result['phone_books'] = normalize_list(phone_books)
    
    # ---- ВЫБИРАЕМ ЛУЧШУЮ ДАТУ И ВОЗРАСТ ----
    if all_birthdates:
...
def format_phone_report(data: dict, views: int) -> str:
    ...
    # Вконтакте – кликабельная ссылка
    vk_data = data.get('vk')
    if vk_data:
        if isinstance(vk_data, dict):
            vk_name = vk_data.get('name')
            vk_url = vk_data.get('url')
        else:
            vk_name = None
            vk_url = str(vk_data) if isinstance(vk_data, str) else None
        if vk_url:
            if not vk_url.startswith('http'):
                vk_url = f"https://{vk_url}"
            display_name = vk_name if vk_name else "Вконтакте"
            lines.append(f"\n🧑‍💻 Вконтакте: [{display_name}]({vk_url})")
        elif vk_name:
            lines.append(f"\n🧑‍💻 Вконтакте: {vk_name}")
    
    # Одноклассники – кликабельная ссылка
    ok_data = data.get('ok')
    if ok_data:
        if isinstance(ok_data, dict):
            ok_name = ok_data.get('name')
            ok_url = ok_data.get('url')
        else:
            ok_name = None
            ok_url = str(ok_data) if isinstance(ok_data, str) else None
        if ok_url:
            if not ok_url.startswith('http'):
                ok_url = f"https://{ok_url}"
            display_name = ok_name if ok_name else "Одноклассники"
            lines.append(f"\n👨‍🦳 Одноклассники: [{display_name}]({ok_url})")
        elif ok_name:
            lines.append(f"\n👨‍🦳 Одноклассники: {ok_name}")
    
    # TikTok – кликабельная ссылка
    tt_data = data.get('tiktok')
    if tt_data:
        tt_clean = clean_value(tt_data)
        if tt_clean:
            if not tt_clean.startswith('http'):
                tt_clean = f"https://{tt_clean}"
            lines.append(f"\n👩‍🦲 TikTok: [TikTok]({tt_clean})")
...
async def main():
    await init_db()
    print("🚀 Бот запущен (финальная версия с кликабельными ссылками)")
