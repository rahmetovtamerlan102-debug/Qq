...
async def collect_general_data(query: str, search_type: str = "phone"):
    ...
    for block in dep_parsed:
        if block['type'] == 'phone_info':
            ...
            if record_key not in seen_records:
                ...
                # Извлечение ключевых полей для основного блока
                if not result['fio']:
                    for key in ['👤ФИО', '👤Полное имя', 'full_name', 'fio', 'ФИО', 'Полное имя', '👤Имя', '🔸Никнейм']:
                        if key in fields:
                            result['fio'] = str(fields[key]).strip()
                            break
                    ...
                    for key in ['📍Адрес', 'Адрес', 'address']:
                        if key in fields:
                            ...
                            if not re.match(r'^\d{4}-\d{2}-\d{2}', addr):
                                result['address'] = addr
                            break
                for key in ['✉️Почта', 'email']:
                    if key in fields and '@' in str(fields[key]):
                        result['emails'].append(str(fields[key]))
                        break
                for key in ['🪪 Паспорт', 'паспорт', 'passport_numbers']:
                    if key in fields and not result['passport']:
                        result['passport'] = str(fields[key])
                        break
                for key in ['📄Инн', 'инн', 'inns', 'inn']:
                    if key in fields and not result['inn']:
                        result['inn'] = str(fields[key])
                        break
                for key in ['📄Снилс', 'снилс', 'snils']:
                    if key in fields and not result['snils']:
                        result['snils'] = str(fields[key])
                        break
                ...
                for key in ['🧑‍💻Вконтакте', 'vk']:
                    if key in fields and not result['vk']:
                        result['vk'] = get_social_url(fields[key])
                        break
                for key in ['👨‍🦳Одноклассники', 'ok']:
                    if key in fields and not result['ok']:
                        result['ok'] = get_social_url(fields[key])
                        break
                for key in ['📷Instagram', 'instagram']:
                    if key in fields and not result['instagram']:
                        result['instagram'] = get_social_url(fields[key])
                        break
                for key in ['👩‍🦲TikTok', 'tiktok']:
                    if key in fields and not result['tiktok']:
    ...
    # ---- SEON / Snusbase (извлекаем только ключевые поля) ----
