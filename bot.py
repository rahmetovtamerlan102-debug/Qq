...
async def collect_phone_data(query: str):
    ...
    # ---- ТЕЛЕФОННЫЕ КНИГИ (ТОЛЬКО JITLER) ----
    phone_books = []
    if jitler:
    ...
    # Очистка соцсетей (чистый URL или None)
...
def format_phone_report(data: dict, views: int) -> str:
    ...
    # Соцсети – принудительно извлекаем URL из словаря
    def get_social_url(value):
        if isinstance(value, dict):
            return value.get('url', str(value))
        return str(value) if value else None
    
    vk = get_social_url(data.get('vk'))
    if vk:
        lines.append(f"\n🧑‍💻 Вконтакте: {vk}")
    
    ok = get_social_url(data.get('ok'))
    if ok:
        lines.append(f"\n👨‍🦳 Одноклассники: {ok}")
    
    tiktok = get_social_url(data.get('tiktok'))
    if tiktok:
        lines.append(f"\n👩‍🦲 TikTok: {tiktok}")
    
    instagram = get_social_url(data.get('instagram'))
    if instagram:
        lines.append(f"\n📷 Instagram: {instagram}")
...
async def main():
    await init_db()
    print("🚀 Бот запущен (исправлен вывод соцсетей)")
