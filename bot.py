...
# ⚠️ ВАЖНО: замените на свой токен из @CryptoBot (Crypto Pay → Create App)
...
# Пакеты: количество запросов, цена в USD, цена в звёздах
PACKAGES = [
    {"requests": 5, "usd": 1.50, "stars": 10},
    {"requests": 10, "usd": 2.50, "stars": 18},
    {"requests": 25, "usd": 5.00, "stars": 40},
    {"requests": 50, "usd": 9.00, "stars": 70},
    {"requests": 100, "usd": 15.00, "stars": 120},
]
...
async def create_stars_invoice(user_id: int, stars_price: int, requests_count: int):
    pool = await get_pool()
    # Создаём запись в purchases с временным invoice_id
    ...
    # Отправляем инвойс
    prices = [LabeledPrice(label=f"{requests_count} запросов", amount=stars_price * 100)]  # в XTR цена в целых звёздах
    try:
        ...
        # Удаляем запись, если не удалось отправить
        async with pool.acquire() as conn:
...
async def process_stars_payment(charge_id: str, user_id: int, payload: str):
    ...
    async with pool.acquire() as conn:
        ...
        if not purchase or purchase['status'] != 'pending':
            return
        # Обновляем invoice_id на реальный charge_id
        await conn.execute('''
...
async def buy_requests_callback(callback: types.CallbackQuery):
    ...
    for pkg in PACKAGES:
        keyboard.inline_keyboard.append([
            InlineKeyboardButton(
                text=f"{pkg['requests']} запросов — ${pkg['usd']} / {pkg['stars']} ⭐",
    ...
    await callback.message.edit_text("Выберите пакет запросов:", reply_markup=keyboard)
    await callback.answer()

@dp.callback_query(lambda c: c.data and c.data.startswith("pkg_"))
async def package_selected_callback(callback: types.CallbackQuery):
    # Разбираем данные: pkg_запросы_usd_звезды
    ...
    # Сохраняем выбранный пакет в контекст (временно)
    # Показываем выбор способа оплаты
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Оплатить звёздами", callback_data=f"pay_stars_{requests_count}_{stars_price}")],
...
async def pay_stars_callback(callback: types.CallbackQuery):
    ...
    # Инвойс отправлен, сообщение в чате заменим на инструкцию
    await callback.message.edit_text(
...
async def check_usdt_payment_callback(callback: types.CallbackQuery):
    ...
    if result["status"] == "paid":
        ...
        # Показываем "Счёт ещё не оплачен" с кнопками
        await callback.message.edit_text(
            "Счёт ещё не оплачен",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
# ===== ОБРАБОТЧИКИ ДЛЯ PAYMENTS (STARS) =====
@dp.pre_checkout_query()
async def pre_checkout_handler(pre_checkout_query: PreCheckoutQuery):
    # Всегда одобряем для цифровых товаров
...
# ===== ОСТАЛЬНЫЕ ОБРАБОТЧИКИ (ПРОМОКОДЫ, НАЗАД, ПОДПИСКА) =====
...
async def delete_promo_callback(callback: types.CallbackQuery):
    ...
    await admin_callback(callback, None)
