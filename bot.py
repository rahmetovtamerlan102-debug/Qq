...
async def depsearch_search(query: str):
    ...
    url = f"https://api.depsearch.lol/quest={query}&token={DEPSEARCH_TOKEN}&lang=ru"
    ...
    headers = {
        ...
        "Referer": "https://depsearch.lol/",
        "Origin": "https://depsearch.lol",
    ...
    try:
        async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=API_TIMEOUTS["depsearch"])) as resp:
            ...
            if resp.status == 200:
                try:
                    ...
                    logger.info(f"DepSearch parsed JSON, keys: {list(data.keys()) if isinstance(data, dict) else 'not dict'}")
                ...
                logger.warning(f"DepSearch HTTP error {resp.status}")
                return {}
    except asyncio.TimeoutError:
        logger.error(f"DepSearch timeout")
        return {}
