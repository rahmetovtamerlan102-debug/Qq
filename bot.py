            ...
            for item in base_record:
                if isinstance(item, list) and len(item) >= 2:
                    ...
                    if value and value not in (None, '', [], {}):
                        # Обработка специальных ключей для красоты
                        if 'БК' in key:
                            key = 'БК'
...
# ===== HTML-ОТЧЁТ (с полным паролем хеш и исправленным БК) =====
        ...
        if isinstance(value, dict) and 'source' in value and 'data' in value:
            ...
            for k, v in value['data'].items():
                if v and v not in (None, '', [], {}):
                    ...
                    if clean_key:
                        # Если ключ содержит "БК", показываем только "1win" без "БК:"
...
async def universal_handler(message: types.Message):
    ...
    if search_type == "phone":
        ...
        # Генерируем HTML, но не отправляем автоматически
        ...
        # HTML-файл НЕ отправляем автоматически – только по кнопке
...
async def html_phone_callback(callback: types.CallbackQuery):
    ...
    # Отправляем HTML-файл только по кнопке
