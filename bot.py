...
async def depsearch_search(query: str):
    """DepSearch как в curl - GET запрос"""
    ...
    try:
        async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as resp:
            ...
            logger.info(f"DepSearch: HTTP {resp.status}, length: {len(text)}")
            logger.info(f"DepSearch response preview: {text[:300]}")
            
            if resp.status == 200:
                try:
                    data = json.loads(text)
                    logger.info(f"DepSearch SUCCESS! Keys: {list(data.keys()) if isinstance(data, dict) else 'not dict'}")
