...
async def collect_phone_data(query: str):
    ...
    # Логируем ответ DepSearch для отладки
    print("\n=== DEPSEARCH ===")
    print(json.dumps(depsearch, ensure_ascii=False, indent=2)[:2000])
    ...
    # ---- 2. FALLBACK НА BIGBASE (если DepSearch не дал основные данные) ----
    if not result['operator'] or not result['region'] or not result['country'] or not result['fio'] or not all_birthdates:
    ...
    # ---- 3. ДОПОЛНИТЕЛЬНЫЕ ДАННЫЕ ИЗ ВСЕХ API (кроме основных полей) ----
    for src in [bigbase, nightsearch, seon, snusbase, jitler]:
        if not src:
            continue
        ...
        if vk:
            # Если vk – словарь с url, оставляем его для чистого вывода
            result['vk'] = vk
    ...
    # ---- ТЕЛЕФОННЫЕ КНИГИ (из BigBase и JITLER) ----
    phone_books = []
    for src in [bigbase, jitler]:
        if not src:
            continue
        phone_books.extend(deep_find_all(src, 'phone_books'))
        phone_books.extend(deep_find_all(src, 'phonebook'))
        phone_books.extend(deep_find_all(src, 'phone_book'))
        phone_books.extend(deep_find_all(src, 'contacts'))
...
async def main():
    await init_db()
    print("🚀 Бот запущен (с fallback на BigBase)")
