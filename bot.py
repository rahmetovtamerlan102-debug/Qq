...
# CRYPTOPAY_TOKEN - НЕ ХРАНИТЬ В КОДЕ! Используй .env
CRYPTOPAY_TOKEN = os.getenv("CRYPTOPAY_TOKEN")
if not CRYPTOPAY_TOKEN:
    print("⚠️ CRYPTOPAY_TOKEN не задан в .env!")
    ...
    try:
        async with session.post(url, json=payload, headers=headers, 
            ...
            if resp.status == 200:
                ...
                logger.info(f"NightSearch ответ для {query}: {json.dumps(data, ensure_ascii=False)[:500]}")
                return data
            logger.warning(f"NightSearch ошибка {resp.status} для {query}")
            return {}
    except Exception as e:
        logger.error(f"NightSearch ошибка: {e}")
        return {}
...
async def depsearch_search(query: str):
    if not DEPSEARCH_TOKEN or not DEPSEARCH_BASE:
        logger.warning("DEPSEARCH_TOKEN или DEPSEARCH_BASE не задан")
        return {}
    session = await get_http_session()
    ...
    try:
        async with session.get(url, timeout=aiohttp.ClientTimeout(total=API_TIMEOUTS["depsearch"])) as resp:
            if resp.status == 200:
                data = await resp.json()
                logger.info(f"DepSearch ответ для {query}: {json.dumps(data, ensure_ascii=False)[:500]}")
                return data
            logger.warning(f"DepSearch ошибка {resp.status} для {query}")
            return {}
    except Exception as e:
...
async def jitler_search_with_balancer(query: str, search_type: str = "number"):
    ...
    for attempt in range(2):
        ...
        try:
            ...
            logger.error(f"Jitler ошибка: {e}")
            continue
    return {}
...
async def collect_phone_data(query: str):
    ...
    logger.info(f"DepSearch data: {json.dumps(depsearch, ensure_ascii=False)[:200] if depsearch else 'empty'}")
    logger.info(f"NightSearch data: {json.dumps(nightsearch, ensure_ascii=False)[:200] if nightsearch else 'empty'}")
    ...
    if depsearch:
        ...
        if not phone_info:
            # Возможно данные в другом ключе
            for key in ['phone', 'info', 'data']:
                if key in depsearch and isinstance(depsearch[key], dict):
                    phone_info = depsearch[key]
                    break
        ...
        if not results_list:
            ...
            for key in ['data', 'items', 'records']:
                if key in depsearch and isinstance(depsearch[key], list):
                    results_list = depsearch[key]
                    break

        if isinstance(results_list, list):
            for item in results_list:
                if isinstance(item, dict):
                    source_name = item.get('🏫Источник', item.get('Источник', item.get('source', 'Unknown')))
                    ...
                    for k, v in item.items():
                        if k not in ['🏫Источник', 'Источник', 'source'] and v and v not in (None, '', [], {}):
    ...
    if nightsearch:
        ...
        # Результаты из NightSearch
        results_list = nightsearch.get('results', [])
        if not results_list:
            # Проверяем другие ключи
            for key in ['data', 'items']:
                if key in nightsearch and isinstance(nightsearch[key], list):
                    results_list = nightsearch[key]
                    break
        ...
        if isinstance(results_list, list):
            for item in results_list:
                if isinstance(item, dict):
                    ...
                    if not fields:
                        # Проверяем есть ли прямые ключи
                        direct_fields = {k: v for k, v in item.items() if k not in ['database', 'source', 'description']}
                        if direct_fields:
                            fields = [{'key': k, 'value': v} for k, v in direct_fields.items()]
                    ...
                    if not result['fio']:
                        for field in fields:
                            if field.get('key') in ['ФИО', 'Имя', 'full_name', 'name']:
                                if field.get('value'):
                    ...
                    if not all_birthdates:
                        for field in fields:
                            if field.get('key') in ['Дата рождения', 'birthdate', 'dob']:
                                if field.get('value'):
                    ...
                    if not result['address']:
                        for field in fields:
                            if field.get('key') in ['Адрес', 'address', '📍Адрес', '🏠Адрес']:
                                if field.get('value'):
                    ...
                    # Поиск документов в NightSearch
                    for field in fields:
                        if isinstance(field, dict):
                            value = field.get('value', '')
                            if isinstance(value, str):
...
async def create_crypto_pay_invoice(user_id: int, amount_usd: float, requests_count: int):
    if not CRYPTOPAY_TOKEN:
        return None, "CRYPTOPAY_TOKEN не настроен"
    session = await get_http_session()
    ...
    try:
        async with session.post(url, json=payload, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as resp:
            ...
            logger.info(f"CryptoPay ответ: {response_text}")
            if resp.status == 200:
                data = await resp.json()
                if data.get('ok'):
                    invoice = data['result']
        ...
        logger.error(f"CryptoPay ошибка: {e}")
        return None, str(e)
...
async def html_phone_callback(callback: CallbackQuery):
    ...
    report = format_phone_report(data, views)
    await callback.message.reply(report)
    await callback.answer()
