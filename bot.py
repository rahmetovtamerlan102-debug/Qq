...
async def collect_data(query: str, search_type: str, is_phone: bool = False):
    ...
    print("\n=== BIGBASE ===")
    print(json.dumps(bigbase, ensure_ascii=False, indent=2)[:2000])
    print("\n=== NIGHTSEARCH ===")
    print(json.dumps(nightsearch, ensure_ascii=False, indent=2)[:2000])
    print("\n=== SEON ===")
    print(json.dumps(seon, ensure_ascii=False, indent=2)[:2000])
    print("\n=== JITLER ===")
    print(json.dumps(jitler, ensure_ascii=False, indent=2)[:2000])

    # ---- ПРЯМОЙ ПАРСИНГ BIGBASE ----
    op = None
    reg = None
    country = None
    fio = None
    ...
    if bigbase and isinstance(bigbase, dict):
        ...
        op = head.get('phone_operator')
        reg = head.get('phone_region')
    ...
    if not op:
        op = deep_search(nightsearch, 'operator') or deep_search(seon, 'operator') or deep_search(jitler, 'operator')
    if not reg:
        reg = deep_search(nightsearch, 'region') or deep_search(seon, 'region') or deep_search(jitler, 'region')
    if not country:
        country = deep_search(nightsearch, 'country') or deep_search(seon, 'country') or deep_search(jitler, 'country')
    
    if not fio:
        for src in [nightsearch, seon, jitler]:
            if src:
                fio = fio or deep_search(src, 'full_name') or deep_search(src, 'fio') or deep_search(src, 'name')
                if fio:
                    break
    ...
    if not birthdate:
        for src in [nightsearch, seon, jitler]:
            if src:
                birthdate = birthdate or deep_search(src, 'birthdate') or deep_search(src, 'birth_date') or deep_search(src, 'date_of_birth')
                if birthdate:
                    break
    ...
    contacts = []
    telegram_raw = None
    for src in [bigbase, nightsearch, seon, jitler]:
        if not src:
            continue
    ...
    if not is_phone:
        for src in [bigbase, nightsearch, seon]:
            if not src:
                continue
            if not telegram_id:
                telegram_id = deep_search(src, 'telegram_id') or deep_search(src, 'tg_id') or deep_search(src, 'user_id')
            if not username:
                username = deep_search(src, 'username') or deep_search(src, 'user_name') or deep_search(src, 'nick')
            if not reg_date:
...
# ===== ФОРМАТИРОВАНИЕ С ВЫВОДОМ ДИАГНОСТИКИ =====
def format_phone_report(merged: dict, full_data: dict, phone: str, views: int) -> str:
    ...
    # === ДИАГНОСТИКА ===
    # Выводим всё, что есть в merged, даже если данных нет
    debug_data = {
        ...
        'birthdate': merged.get('birthdate'),
        'age': merged.get('age'),
        'contacts': len(merged.get('contacts', [])),
        'telegrams': len(merged.get('telegrams', [])),
        'socials': len(merged.get('socials', {})),
        'banks': len(merged.get('banks', [])),
        'emails': len(merged.get('emails', []))
    }
    lines.append("\n🔍 **ДИАГНОСТИКА (внутренние данные):**")
    lines.append(f"├ merged: {json.dumps(debug_data, ensure_ascii=False)}")
    
    # Проверяем, есть ли ФИО в bigbase напрямую
    if full_data.get('bigbase'):
        bigbase = full_data['bigbase']
        connections = bigbase.get('connections', {})
        persons = connections.get('person', [])
        if persons:
            fio_from_bigbase = persons[0].get('fio', [{}])[0].get('value') if persons else None
            lines.append(f"├ ФИО из bigbase: {fio_from_bigbase}")
        head = bigbase.get('dossier', {}).get('head', {})
        if head:
            lines.append(f"├ Оператор из bigbase: {head.get('phone_operator')}")
            lines.append(f"├ Регион из bigbase: {head.get('phone_region')}")
            lines.append(f"├ Страна из bigbase: {head.get('phone_country_info')}")
