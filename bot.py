...
async def create_user(user_id: int, username: str = None, referred_by: int = None):
    ...
    balance = 5  # не используется, но оставим для совместимости
    if referred_by:
        # бонус рефереру не начисляем, но запись о реферале сохраняем
...
async def deduct_request(user_id: int) -> bool:
    # Всегда возвращаем True – безлимит
    return True
...
async def add_mirror_bonus(user_id: int):
    # Бонус не нужен, но оставим пустым
    pass
...
# ===== СБОР ДАННЫХ (BigBase + Nightsearch + Seon + JITLER) =====
async def collect_data(query: str, search_type: str, is_phone: bool = False):
    ...
    if is_phone:
        ...
        for src in sources:
            ...
            if isinstance(src, dict):
                data_items.append(src)
            elif isinstance(src, list):
                data_items.extend(src)
            for item in data_items:
                if isinstance(item, dict):
                    if not op:
                        op = item.get('operator') or item.get('оператор') or item.get('oper')
                    if not reg:
                        reg = item.get('region') or item.get('регион') or item.get('reg')
                    if not country:
                        country = item.get('country') or item.get('страна')
    ...
    for src in sources:
        if not src:
            continue
        if isinstance(src, dict):
            books = src.get('phone_books') or src.get('contacts') or src.get('телефонные_книги')
            if books:
                if isinstance(books, list):
                    contacts.extend(books)
                else:
                    contacts.append(str(books))
            tg = src.get('telegram') or src.get('tg')
            if tg and not telegram_raw:
                telegram_raw = tg
        elif isinstance(src, list):
            for item in src:
                if isinstance(item, dict):
                    books = item.get('phone_books') or item.get('contacts')
                    if books:
                        if isinstance(books, list):
                            contacts.extend(books)
                        else:
                            contacts.append(str(books))
                    tg = item.get('telegram') or item.get('tg')
                    if tg and not telegram_raw:
                        telegram_raw = tg
...
def format_phone_report(merged: dict, full_data: dict, phone: str, views: int) -> str:
    ...
    birthdate = None
    age = None
    for src in [full_data.get('bigbase'), full_data.get('nightsearch'), full_data.get('seon'), full_data.get('jitler')]:
        if not src:
            continue
        if isinstance(src, dict):
            if not fio:
                fio = src.get('full_name') or src.get('fio') or src.get('name') or src.get('ФИО')
            if not birthdate:
                birthdate = src.get('birthdate') or src.get('birth_date') or src.get('date_of_birth') or src.get('дата рождения')
            if not age:
                age = src.get('age')
        elif isinstance(src, list):
            for item in src:
        ...
        if isinstance(src, dict):
            for key in ['vk', 'vkontakte', 'instagram', 'tiktok', 'ok', 'odnoklassniki', 'twitter', 'facebook']:
                ...
                if val and key not in socials:
                    socials[key] = val
            bank = src.get('bank') or src.get('banks') or src.get('account')
            if bank:
                if isinstance(bank, list):
                    banks.extend(bank)
                else:
                    banks.append(str(bank))
            email = src.get('email') or src.get('mail') or src.get('e-mail')
            if email:
...
async def start_cmd(message: types.Message):
    ...
    # Основное меню с тремя кнопками (без "Купить" и "Поиск по неполным")
...
async def add_bot_cmd(message: types.Message):
    ...
    try:
        ...
        await add_mirror_bonus(message.from_user.id)  # ничего не делает
        await message.reply(f"✅ Бот @{username} добавлен как зеркало! Запросы бесконечные, бонус не нужен.")
...
async def main():
    await init_db()
    print("🚀 Бот запущен (бесконечные запросы, без лишних кнопок)")
