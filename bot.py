...
Токен TeleLog (можно заменить через .env)
...
===== СБОР ДАННЫХ ТОЛЬКО ДЛЯ TELEGRAM (быстрый) =====

async def collect_telelog_only(query: str):
print(f"🔍 Быстрый поиск Telegram ID: {query}")

telelog_tasks = await asyncio.gather(  
...
telelog_names = telelog_tasks[0] if isinstance(telelog_tasks[0], list) else []  
telelog_usernames = telelog_tasks[1] if isinstance(telelog_tasks[1], list) else []  
telelog_gifts = telelog_tasks[2] if isinstance(telelog_tasks[2], list) else []  
...
# Обработка TeleLog данных с удалением дубликатов  
seen_names = set()  
...
===== СБОР ДАННЫХ ДЛЯ ID (ПОЛНЫЙ, БЕЗ TELELOG) =====

async def collect_id_data(query: str, search_type: str):
print(f"🔍 Полный поиск ID: {query}, тип: {search_type}")
...
# BigBase, Jitler, DepSearch, NightSearch, SEON, Snusbase  
api_tasks = [  
...
bigbase = api_results[0] if isinstance(api_results[0], dict) else {}  
jitler = api_results[1] if isinstance(api_results[1], dict) else {}  
depsearch = api_results[2] if isinstance(api_results[2], dict) else {}  
nightsearch = api_results[3] if isinstance(api_results[3], dict) else {}  
seon = api_results[4] if isinstance(api_results[4], dict) else {}  
snusbase = api_results[5] if isinstance(api_results[5], dict) else {}  

print(f"✅ BigBase: {'есть' if bigbase else 'нет'}")  
print(f"✅ Jitler: {'есть' if jitler else 'нет'}")  
print(f"✅ DepSearch: {'есть' if depsearch else 'нет'}")  
print(f"✅ NightSearch: {'есть' if nightsearch else 'нет'}")  
print(f"✅ SEON: {'есть' if seon else 'нет'}")  
print(f"✅ Snusbase: {'есть' if snusbase else 'нет'}")  
...
# ---- ОБРАБОТКА СПЕЦИФИЧНЫХ ТИПОВ (ИНН, ПАСПОРТ, ВУ) ----  
# Здесь можно добавить парсинг для этих типов, если API возвращает структурированные данные.  
# Пока просто сохраняем сырой ответ в отдельное поле для отладки.  
if search_type in ("inn", "passport", "driver"):  
    if jitler and isinstance(jitler, dict):  
        # Сохраняем raw для отображения  
        raw_text = jitler.get('response', {}).get('raw', '')  
        if raw_text:  
            result['raw_data'] = raw_text  
...
# Удаляем дубликаты в истории имён и юзернеймов  
seen_names = set()  
unique_name_history = []  
...
print(f"📊 Результат: телеграм={len(result['telegrams'])}, группы={len(result['groups'])}, история имён={len(result['name_history'])}, история юзернеймов={len(result['username_history'])}, книги={len(result['phone_books'])}, подарки={len(result['gifts'])}")  
return result
...
# Если есть сырые данные для ИНН/паспорт/ВУ — покажем их  
if data.get('raw_data'):  
    lines.append("📄 Результат поиска:")  
    lines.append(f"  {data['raw_data'][:500]}")  
    lines.append("")  
...
# Сопоставление направления → тип запроса  
SEARCH_TYPES = {  
    "vk": "vks",  
    "inn": "inn",  
    "passport": "passport",  
    "driver": "driver",  
    "tg": "telegram",   # для быстрого поиска через TeleLog  
}  
...
try:  
    if direction == "tg":  
        # Быстрый поиск только TeleLog + BigBase  
        data = await asyncio.wait_for(collect_telelog_only(id_str), timeout=10.0)  
    else:  
        # Полный поиск со всеми API  
        data = await asyncio.wait_for(collect_id_data(id_str, search_type), timeout=20.0)  
except asyncio.TimeoutError:  
...
if is_phone:  
    ...
    # Для номера используем collect_phone_data (ваша существующая функция)  
...
async def main():
await init_db()
print("🚀 Бот запущен (разделены TeleLog и полный поиск, правильные типы)")
