...
def format_phone_report(data: dict, views: int) -> str:
    ...
    # Карты скрываем в текстовом отчёте
    if data.get('cards'):
        lines.append(f"\n💳 Карты: скрыты (доступны в HTML-отчёте)")
...
async def universal_handler(message: types.Message):
    ...
    # Определяем тип запроса (порядок важен)
    # 1. Телефон (с + или без)
    ...
    # 4. VK ссылка
        ...
        search_type = "vk"
    # 5. Telegram (только @username или idXXXXX)
    elif text.startswith('@'):
        search_type = "telegram"
    elif text.lower().startswith("id") and len(text) > 2:
        digits = text[2:].strip()
        if digits.isdigit():
            query = digits
            search_type = "telegram"
    # 6. ФИО (три слова на кириллице)
    elif re.search(r'[А-Яа-я]+\s+[А-Яа-я]+\s+[А-Яа-я]+', text):
        search_type = "fio"
    # 7. Если ничего не подошло – неизвестно
    ...
    if search_type == "phone":
        ...
        # Telegram (если есть username из данных)
        ...
        # Не отправляем HTML автоматически
