...
async def collect_phone_data(query: str):
    ...
    result = {
        ...
        'region': None,
        'country': None,
        'fio': None,          # ФИО из BigBase (основной)
        'extra_names': [],    # Дополнительные имена из других API
        'birthdate': None,
        'age': None,
        'emails': [],
        'telegrams': [],
    ...
    for src in [nightsearch, seon, snusbase, jitler]:
        ...
        # ФИО из других API (если в BigBase нет)
        if not result['fio']:
            fio_from_src = deep_find(src, 'full_name') or deep_find(src, 'fio') or deep_find(src, 'name')
            if fio_from_src:
                result['fio'] = fio_from_src
        else:
            # Если ФИО уже есть, собираем дополнительные имена для вывода
            extra = deep_find(src, 'full_name') or deep_find(src, 'fio') or deep_find(src, 'name')
            if extra and extra != result['fio']:
                result['extra_names'].append(extra)
    ...
    result['extra_names'] = normalize_list(result['extra_names'])
...
def format_phone_report(data: dict, views: int) -> str:
    ...
    if has_personal:
        ...
        if data.get('extra_names'):
            lines.append(f"├ Доп. имена: {', '.join(data['extra_names'][:3])}")
...
async def main():
    await init_db()
    print("🚀 Бот запущен (ФИО из всех API, дата рождения отдельно)")
